...
async function main() {
	...
	const YOUContracts = await dh.deployYOUContractsHardhat(accounts[0])

	const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } =
		coreContracts

	await dh.connectCoreContracts(coreContracts, YOUContracts)
	await dh.connectYOUContractsToCore(YOUContracts, coreContracts)
	...
	const YOUmount = toBN(toWei("2500")) // borrower wants to withdraw 2500 U
	...
	const liquidationReserve = await troveManager.U_GAS_COMPENSATION()
	const expectedFee = await troveManager.getBorrowingFeeWithDecay(YOUmount)

	// Total debt of the new trove = U amount drawn, plus fee, plus the liquidation reserve
	const expectedDebt = YOUmount.add(expectedFee).add(liquidationReserve)
	...
	await borrowerOperations.openTrove(maxFee, YOUmount, upperHint, lowerHint, {
		value: ETHColl,
	})
	...
	await borrowerOperations.adjustTrove(maxFee, 0, URepayment, false, upperHint, lowerHint, {
		value: collIncrease,
	})
	...
	const redemptionhint = await hintHelpers.getRedemptionHints(YOUmount, price, 50)
	...
	/* Finally, perform the on-chain redemption, passing the truncated U amount, the correct hints, and the expected
