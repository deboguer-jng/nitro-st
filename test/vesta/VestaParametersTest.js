...
contract("VestaParameters", async accounts => {
	...
	const YOU_GAS_COMPENSATION_SAFETY_MAX = toBN(dec(400, 18))
	const YOU_GAS_COMPENSATION_SAFETY_MIN = toBN(dec(1, 18))
	...
	describe("Vesta Parameters", async () => {
		beforeEach(async () => {
			...
			GAS_COMPENSATION = await vestaParameters.U_GAS_COMPENSATION_DEFAULT()
			...
			await deploymentHelper.connectYOUContractsToCore(YOUContracts, contracts, false, false)
		})
		...
		it("Formula Checks: Call every function with default value, Should match default values", async () => {
			...
			await vestaParameters.setUGasCompensation(ZERO_ADDRESS, dec(30, 18))
			...
			assert.equal(
				(await vestaParameters.PERCENT_DIVISOR(ZERO_ADDRESS)).toString(),
				PERCENT_DIVISOR
			)
			assert.equal(
				(await vestaParameters.BORROWING_FEE_FLOOR(ZERO_ADDRESS)).toString(),
				BORROWING_FEE_FLOOR
			)
			assert.equal(
				(await vestaParameters.MAX_BORROWING_FEE(ZERO_ADDRESS)).toString(),
				MAX_BORROWING_FEE
			)
			assert.equal(
				(await vestaParameters.U_GAS_COMPENSATION(ZERO_ADDRESS)).toString(),
				GAS_COMPENSATION
			)
			assert.equal((await vestaParameters.MIN_NET_DEBT(ZERO_ADDRESS)).toString(), MIN_NET_DEBT)
			assert.equal(
				(await vestaParameters.REDEMPTION_FEE_FLOOR(ZERO_ADDRESS)).toString(),
		...
		it("Try to edit Parameters has User, Revert Transactions", async () => {
			...
			await assertRevert(
				vestaParameters.setUGasCompensation(ZERO_ADDRESS, GAS_COMPENSATION, { from: user })
			)
			await assertRevert(
				vestaParameters.setMinNetDebt(ZERO_ADDRESS, MIN_NET_DEBT, { from: user })
			)
			await assertRevert(
				vestaParameters.setPercentDivisor(ZERO_ADDRESS, PERCENT_DIVISOR, { from: user })
			)
			await assertRevert(
				vestaParameters.setBorrowingFeeFloor(ZERO_ADDRESS, BORROWING_FEE_FLOOR, { from: user })
			)
			await assertRevert(
				vestaParameters.setMaxBorrowingFee(ZERO_ADDRESS, MAX_BORROWING_FEE, { from: user })
			)
			await assertRevert(
				vestaParameters.setRedemptionFeeFloor(ZERO_ADDRESS, REDEMPTION_FEE_FLOOR, {
					from: user,
				})
			)
		})
		...
		it("sanitizeParameters: User call sanitizeParameters on Non-Configured Collateral - Set Default Values", async () => {
			...
			assert.equal(
				...
				await vestaParameters.U_GAS_COMPENSATION(ZERO_ADDRESS)
			)
			assert.equal(MIN_NET_DEBT.toString(), await vestaParameters.MIN_NET_DEBT(ZERO_ADDRESS))
			assert.equal(
				PERCENT_DIVISOR.toString(),
				await vestaParameters.PERCENT_DIVISOR(ZERO_ADDRESS)
			)
			assert.equal(
				BORROWING_FEE_FLOOR.toString(),
				await vestaParameters.BORROWING_FEE_FLOOR(ZERO_ADDRESS)
			)
			assert.equal(
				MAX_BORROWING_FEE.toString(),
				await vestaParameters.MAX_BORROWING_FEE(ZERO_ADDRESS)
			)
			assert.equal(
				REDEMPTION_FEE_FLOOR.toString(),
				await vestaParameters.REDEMPTION_FEE_FLOOR(ZERO_ADDRESS)
			)
		})
		...
		it("sanitizeParameters: User call sanitizeParamaters on Configured Collateral - Ignore it", async () => {
			...
			const newGasComp = YOU_GAS_COMPENSATION_SAFETY_MAX
		...
		it("setUGasCompensation: Owner change parameter - Failing SafeCheck", async () => {
			...
			await assertRevert(
				vestaParameters.setUGasCompensation(
					ZERO_ADDRESS,
					YOU_GAS_COMPENSATION_SAFETY_MIN.sub(toBN(1))
				)
			)
			await assertRevert(
				vestaParameters.setUGasCompensation(
		...
		it("setUGasCompensation: Owner change parameter - Valid SafeCheck", async () => {
			...
			assert.equal(
				YOU_GAS_COMPENSATION_SAFETY_MIN.toString(),
				await vestaParameters.U_GAS_COMPENSATION(ZERO_ADDRESS)
			)

			await vestaParameters.setUGasCompensation(ZERO_ADDRESS, YOU_GAS_COMPENSATION_SAFETY_MAX)
			assert.equal(
				YOU_GAS_COMPENSATION_SAFETY_MAX.toString(),
				await vestaParameters.U_GAS_COMPENSATION(ZERO_ADDRESS)
			)
		})
		...
		it("setMinNetDebt: Owner change parameter - Valid SafeCheck", async () => {
			...
			assert.equal(
				...
				await vestaParameters.MIN_NET_DEBT(ZERO_ADDRESS)
			)
		...
		it("setPercentDivisor: Owner change parameter - Failing SafeCheck", async () => {
			...
			await assertRevert(
				vestaParameters.setPercentDivisor(
					ZERO_ADDRESS,
					PERCENT_DIVISOR_SAFETY_MIN.sub(toBN(1))
				)
			)
			await assertRevert(
				vestaParameters.setPercentDivisor(
		...
		it("setRedemptionFeeFloor: Owner change parameter - Valid SafeCheck", async () => {
			...
			await vestaParameters.setRedemptionFeeFloor(
				ZERO_ADDRESS,
				REDEMPTION_FEE_FLOOR_SAFETY_MIN
			)
			assert.equal(
				expectedMin.toString(),
				await vestaParameters.REDEMPTION_FEE_FLOOR(ZERO_ADDRESS)
			...
			await vestaParameters.setRedemptionFeeFloor(
				ZERO_ADDRESS,
				REDEMPTION_FEE_FLOOR_SAFETY_MAX
			)
			assert.equal(
				expectedMax.toString(),
				await vestaParameters.REDEMPTION_FEE_FLOOR(ZERO_ADDRESS)
		...
		it("openTrove(): Borrowing at zero base rate charges minimum fee with different borrowingFeeFloor", async () => {
			...
			assert.equal(
				...
				await vestaParameters.BORROWING_FEE_FLOOR(erc20.address)
			)
			...
			const _USDVFee_Asset = toBN(
				th.getEventArgByName(txC_Asset, "UBorrowingFeePaid", "_UFee")
			)

			const expectedFee = (await vestaParameters.BORROWING_FEE_FLOOR(ZERO_ADDRESS))
				.mul(toBN(USDVRequest))
				.div(toBN(dec(1, 18)))
			const expectedFee_Asset = (await vestaParameters.BORROWING_FEE_FLOOR(erc20.address))
				.mul(toBN(USDVRequest))
				.div(toBN(dec(1, 18)))
