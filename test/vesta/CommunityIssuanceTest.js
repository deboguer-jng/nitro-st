...
contract("CommunityIssuance", async accounts => {
	...
	describe("Community Issuance", async () => {
		...
		it("addFundToStabilityPool: Called by user, valid inputs, revert transaction", async () => {
			...
			await assertRevert(
				communityIssuance.addFundToStabilityPool(stabilityPool.address, dec(100, 18), {
					from: user,
				})
			)
		})
		...
		it("addFundToStabilityPool: Called by owner, valid inputs, add stability pool", async () => {
			...
			await communityIssuance.addFundToStabilityPool(
				stabilityPoolERC20.address,
				dec(100, 18),
				{ from: treasury }
			)
			assert.equal(
				(await communityIssuance.YOUSupplyCaps(stabilityPool.address)).toString(),
				dec(100, 18)
			)
			assert.equal(
				(await communityIssuance.YOUSupplyCaps(stabilityPoolERC20.address)).toString(),
				dec(100, 18)
			)
		})
		...
		it("addFundToStabilityPool: Called by owner twice, double total supply, don't change deploy time", async () => {
			...
			const deployTimePoolERC20 = await communityIssuance.lastUpdateTime(
				stabilityPoolERC20.address
			)
			...
			const deployTimePoolAfterERC20 = await communityIssuance.lastUpdateTime(
				stabilityPoolERC20.address
			)
		...
		it("addFundToStabilityPool: Called by owner, valid inputs, change total supply", async () => {
			...
			await communityIssuance.addFundToStabilityPool(stabilityPool.address, supply, {
				from: treasury,
			})
			await communityIssuance.addFundToStabilityPool(stabilityPool.address, supply, {
				from: treasury,
			})
			await communityIssuance.addFundToStabilityPool(stabilityPoolERC20.address, supply, {
				from: treasury,
			})
		...
		it("removeFundFromStabilityPool: Called by user, valid inputs, then reverts", async () => {
			...
			await assertRevert(
				communityIssuance.removeFundFromStabilityPool(stabilityPool.address, dec(50, 18), {
					from: user,
				})
			)
		})
		...
		it("removeFundFromStabilityPool: Called by owner, invalid inputs, then reverts", async () => {
			...
			await assertRevert(
				communityIssuance.removeFundFromStabilityPool(stabilityPool.address, dec(101, 18), {
					from: treasury,
				})
			)
		})
		...
		it("removeFundFromStabilityPool: Called by owner, max supply, then disable pool", async () => {
			...
			await communityIssuance.removeFundFromStabilityPool(stabilityPool.address, supply, {
				from: treasury,
			})
			...
			assert.equal(
				(await communityIssuance.lastUpdateTime(stabilityPool.address)).toString(),
				0
			)
			assert.equal(
				(await communityIssuance.totalYOUIssued(stabilityPool.address)).toString(),
				0
			)
		})
		...
		it("transferFundToAnotherStabilityPool : Called by owner, valid pools, issuedYOU, transfer over caps, revert transaction", async () => {
			const supply = toBN(dec(100, 18))
			...
			await communityIssuance.unprotectedIssueYOU(stabilityPool.address)

			const issued = await communityIssuance.totalYOUIssued(stabilityPool.address)
		...
		it("transferFundToAnotherStabilityPool : Called by owner, valid inputs, 50% balance, transfer YOU", async () => {
			const supply = toBN(dec(100, 18))
			...
			assert.equal(
				(await communityIssuance.YOUSupplyCaps(stabilityPoolERC20.address)).toString(),
				supply.add(supplyTransfered)
			)
		})

		it("transferFundToAnotherStabilityPool : Called by owner, valid inputs, 100% balance, transfer YOU and close pool", async () => {
			const supply = toBN(dec(100, 18))
		...
		it("transferFundToAnotherStabilityPool : Called by owner, valid inputs, issued YOU, 100% left over, transfer YOU and close pool", async () => {
			const supply = toBN(dec(100, 18))
