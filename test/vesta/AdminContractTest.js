...
contract("AdminContract", async accounts => {
	...
	describe("Admin Contract", async () => {
		...
		it("AddNewCollateral: As Owner - Create new Stability Pool - Verify All Systems", async () => {
			...
			assert.notEqual(
				(await contracts.youParameters.redemptionBlock(ZERO_ADDRESS)).toString(),
				0
			)
			assert.notEqual(
				await stabilityPoolManager.unsafeGetAssetStabilityPool(ZERO_ADDRESS),
				ZERO_ADDRESS
			)
			assert.equal(
				(await vstaToken.balanceOf(VSTAContracts.communityIssuance.address)).toString(),
				dec(100, 18)
			)
