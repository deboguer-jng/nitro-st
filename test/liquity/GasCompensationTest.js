...
contract("Gas compensation tests", async accounts => {
	...
	beforeEach(async () => {
		...
		contracts.troveManager.setVestaParameters(contracts.youParameters.address)
   ...
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 VST */
	...
	it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
		...
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: dec(300, 18),
			extraParams: { from: carol },
		})
		await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: A_totalDebt,
			extraParams: { from: dennis },
		})
		await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: B_totalDebt.add(C_totalDebt),
			extraParams: { from: erin },
		})
	...
	it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
		...
		await openTrove({
			ICR: toBN(dec(120, 16)),
			extraVSTAmount: dec(5000, 18),
			extraParams: { from: bob },
		})
		await openTrove({
			ICR: toBN(dec(60, 18)),
			extraVSTAmount: dec(600, 18),
			extraParams: { from: carol },
		})
		await openTrove({
			ICR: toBN(dec(80, 18)),
			extraVSTAmount: dec(1, 23),
			extraParams: { from: dennis },
		})
		await openTrove({
			ICR: toBN(dec(80, 18)),
	...
	it("gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder", async () => {
		...
		await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: dec(2000, 18),
			extraParams: { from: alice },
		})
		await openTrove({
			ICR: toBN(dec(1875, 15)),
			extraVSTAmount: dec(8000, 18),
			extraParams: { from: bob },
		})
		await openTrove({
			ICR: toBN(dec(2, 18)),
