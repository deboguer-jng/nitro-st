...
contract("Gas compensation tests", async accounts => {
	...
	beforeEach(async () => {
		...
		community = YOUContracts.communityIssuance
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		...
		assert.equal(
			(await troveManager.getCollGasCompensation(ZERO_ADDRESS, dec(1, "ether"))).toString(),
			dec(5, 15)
		)
		assert.equal(
			(await troveManager.getCollGasCompensation(erc20.address, dec(1, "ether"))).toString(),
			dec(5, 15)
		)
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		...
		assert.equal(
			(await troveManager.getCollGasCompensation(ZERO_ADDRESS, dec(100, "finney"))).toString(),
			dec(5, 14)
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, dec(100, "finney"))
			).toString(),
			dec(5, 14)
		)
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		...
		assert.equal(
			(await troveManager.getCollGasCompensation(ZERO_ADDRESS, "5000000000")).toString(),
			"25000000"
		)
		assert.equal(
			(await troveManager.getCollGasCompensation(erc20.address, "5000000000")).toString(),
			"25000000"
		)
	})
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "9999000000000000000")
			).toString(),
			"49995000000000000"
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "9999000000000000000")
			).toString(),
			"49995000000000000"
		)
     ...
     -> Expect 0.5% of collaterall as gas compensation */
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "55000000000000000")
			).toString(),
			dec(275, 12)
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "55000000000000000")
			).toString(),
			dec(275, 12)
		)
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "6092324088087235800")
			).toString(),
			"30461620440436179"
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "6092324088087235800")
			).toString(),
			"30461620440436179"
		)
	})
    ...
    -> Expect 0.5% of collaterall as gas compensation */
		assert.equal(
			(await troveManager.getCollGasCompensation(ZERO_ADDRESS, dec(10, "ether"))).toString(),
			"50000000000000000"
		)
		assert.equal(
			(await troveManager.getCollGasCompensation(erc20.address, dec(10, "ether"))).toString(),
			"50000000000000000"
		)
	})
    ...
    -> Expect $100 gas compensation, i.e. 0.5 ETH */
		assert.equal(
			(await troveManager.getCollGasCompensation(ZERO_ADDRESS, dec(100, "ether"))).toString(),
			dec(500, "finney")
		)
		assert.equal(
			(await troveManager.getCollGasCompensation(erc20.address, dec(100, "ether"))).toString(),
			dec(500, "finney")
		)
    ...
    -> Expect $100 gas compensation, i.e.  0.050005  ETH */
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "10001000000000000000")
			).toString(),
			"50005000000000000"
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "10001000000000000000")
			).toString(),
			"50005000000000000"
		)
    ...
    -> Expect $37.5 gas compensation i.e.  0.1875  ETH */
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "37500000000000000000")
			).toString(),
			"187500000000000000"
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "37500000000000000000")
			).toString(),
			"187500000000000000"
		)
    ...
    -> Expect $21473894.8385808 gas compensation, i.e.  473.7911529115490  ETH */
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "94758230582309850000000"),
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getCollGasCompensation(erc20.address, "94758230582309850000000"),
				"473791152911549000000"
			),
			1000000
		)
    ...
    -> Expect $150000000000 gas compensation, i.e. 1500000 ETH */
		...
		assert.equal(
			(
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, "300000000000000000000000000")
			).toString(),
			"1500000000000000000000000"
		)
		assert.equal(
			(
				await troveManager.getCollGasCompensation(erc20.address, "300000000000000000000000000")
			).toString(),
			"1500000000000000000000000"
		)
	})
    ...
    debt = 10 U
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 U*/
     ...
     debt = 0 U
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 U*/
		...
		// debt = 200 U
		// 0.5% of coll = 0.004995 ETH. USD value: $6.09
		// -> Expect  composite debt =  200 + 200 = 400  U */
		assert.equal(
			await troveManager.getCompositeDebt(ZERO_ADDRESS, dec(200, 18)),
			"400000000000000000000"
		)
		assert.equal(
			await troveManager.getCompositeDebt(erc20.address, dec(200, 18)),
			"400000000000000000000"
		)
	})
    ...
    debt = 123.45 U
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 U  */
		assert.equal(
			await troveManager.getCompositeDebt(ZERO_ADDRESS, "123450000000000000000"),
			"323450000000000000000"
		)
		assert.equal(
			await troveManager.getCompositeDebt(erc20.address, "123450000000000000000"),
			"323450000000000000000"
		)
	})
    ...
    coll = 100 ETH  
    debt = 2000 U
    -> Expect composite debt = (2000 + 200) = 2200 U  */
		assert.equal(
			(await troveManager.getCompositeDebt(ZERO_ADDRESS, dec(2000, 18))).toString(),
			"2200000000000000000000"
		)
		assert.equal(
			(await troveManager.getCompositeDebt(erc20.address, dec(2000, 18))).toString(),
			"2200000000000000000000"
		)
    ...
    -> Expect composite debt = (200 + 200) = 400 U  */
		assert.equal(
			(await troveManager.getCompositeDebt(ZERO_ADDRESS, dec(200, 18))).toString(),
			"400000000000000000000"
		)
		assert.equal(
			(await troveManager.getCompositeDebt(erc20.address, dec(200, 18))).toString(),
			"400000000000000000000"
		)
    ...
    coll = 37.5 ETH  
    debt = 500 U
    -> Expect composite debt = (500 + 200) = 700 U  */
		assert.equal(
			(await troveManager.getCompositeDebt(ZERO_ADDRESS, dec(500, 18))).toString(),
			"700000000000000000000"
		)
		assert.equal(
			(await troveManager.getCompositeDebt(erc20.address, dec(500, 18))).toString(),
			"700000000000000000000"
		)
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion U
    -> Expect composite debt = (1000000000 + 200) = 1000000200 U  */
		await priceFeed.setPrice("45323545420000000000000")
		assert.isAtMost(
			th.getDifference(
				(await troveManager.getCompositeDebt(ZERO_ADDRESS, dec(1, 27))).toString(),
		...
		assert.isAtMost(
			th.getDifference(
				(await troveManager.getCompositeDebt(erc20.address, dec(1, 27))).toString(),
				"1000000200000000000000000000"
			),
			100000000000
		)
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 U
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 U */
		await priceFeed.setPrice(dec(1, 24))
		assert.equal(
			(
				await troveManager.getCompositeDebt(ZERO_ADDRESS, "54321123456789000000000")
			).toString(),
			"54521123456789000000000"
		)
		assert.equal(
			(
				await troveManager.getCompositeDebt(erc20.address, "54321123456789000000000")
			).toString(),
			"54521123456789000000000"
		)
	})
	...
	it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
		...
		const flyn_ICRERC20 = (
			await troveManager.getCurrentICR(erc20.address, flyn, price)
		).toString()
		...
		// E opens with 4405.45 ETH, 32598.35 U
		...
		const erin_ICRERC20 = (
			await troveManager.getCurrentICR(erc20.address, erin, price)
		).toString()
		...
		const harriet_ICR = (
			await troveManager.getCurrentICR(ZERO_ADDRESS, harriet, price)
		).toString()
		const harriet_ICRERC20 = (
			await troveManager.getCurrentICR(erc20.address, harriet, price)
		).toString()
	...
	it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
		...
		await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: B_totalDebt.add(C_totalDebt),
			extraParams: { from: erin },
		})
		...
		console.log((await community.YOUSupplyCaps(stabilityPool.address)).toString())
		console.log((await community.YOUSupplyCaps(stabilityPoolERC20.address)).toString())

		// D, E each provide U to SP
		...
		const UinSP_0 = await stabilityPool.getTotalUDeposits()
		const UinSP_0ERC20 = await stabilityPoolERC20.getTotalUDeposits()
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		const aliceCollERC20 = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		...
		// Check SP U has decreased due to the liquidation
		const UinSP_A = await stabilityPool.getTotalUDeposits()
		const UinSP_AERC20 = await stabilityPoolERC20.getTotalUDeposits()
		assert.isTrue(UinSP_A.lte(UinSP_0))
		assert.isTrue(UinSP_AERC20.lte(UinSP_0ERC20))

		// Check ETH in SP has received the liquidation
		assert.equal(
			(await stabilityPool.getAssetBalance()).toString(),
			aliceColl.sub(_0pt5percent_aliceColl)
		) // 1 ETH - 0.5%
		assert.equal(
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		// Check SP U has decreased due to the liquidation of B
		const UinSP_B = await stabilityPool.getTotalUDeposits()
		const UinSP_BERC20 = await stabilityPool.getTotalUDeposits()

		assert.isTrue(UinSP_B.lt(UinSP_A))
		assert.isTrue(UinSP_BERC20.lt(UinSP_AERC20))
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		const carolCollERC20 = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		...
		// Check SP U has decreased due to the liquidation of C
		assert.isTrue((await stabilityPool.getTotalUDeposits()).lt(UinSP_B))
		assert.isTrue((await stabilityPoolERC20.getTotalUDeposits()).lt(UinSP_BERC20))
	...
	it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
		...
		await openTrove({
			ICR: toBN(dec(60, 18)),
			extraUAmount: dec(600, 18),
			extraParams: { from: carol },
		})
		await openTrove({
			ICR: toBN(dec(80, 18)),
			extraUAmount: dec(1, 23),
			extraParams: { from: dennis },
		})
		await openTrove({
			ICR: toBN(dec(80, 18)),
		...
		// D, E each provide 10000 U to SP
    ...
    and (1 - 0.05000025000125001) = 0.94999974999875 ETH remainder liquidated */
		...
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, alice, price_1)).lt(mv._MCR)
		)
		...
		// Check SP U has decreased due to the liquidation of A
		const UinSP_A = await stabilityPool.getTotalUDeposits()
		const UinSP_AERC20 = await stabilityPoolERC20.getTotalUDeposits()
		assert.isTrue(UinSP_A.lte(UinSP_0))
		assert.isTrue(UinSP_AERC20.lte(UinSP_0ERC20))
    ...
    and (15 - 0.666666666666666666) ETH remainder liquidated */
		...
		// Check SP U has decreased due to the liquidation of B
		assert.isTrue((await stabilityPool.getTotalUDeposits()).lt(UinSP_A))
		assert.isTrue((await stabilityPoolERC20.getTotalUDeposits()).lt(UinSP_AERC20))
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		const aliceDebtERC20 = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		assert.isFalse(await th.checkRecoveryMode(contracts, erc20.address))
		// Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
		const liquidationTxB = await troveManager.liquidate(ZERO_ADDRESS, bob, {
			from: liquidator,
			gasPrice: 0,
		})
    ...
    and (1 - 0.05000025000125001) = 0.94999974999875 ETH remainder liquidated */
		...
		const aliceCollValueInUSDERC20 = await borrowerOperations.getUSDValue(
			aliceCollERC20,
			price_1
		)
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
	...
	it("liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder", async () => {
		...
		const dennisCollERC20 = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		...
		const collGasCompensationERC20 = await troveManager.getCollGasCompensation(
			erc20.address,
			price
		)
    ...
    */
		...
		// Check U in SP has decreased
		assert.isTrue((await stabilityPool.getTotalUDeposits()).lt(UinSP_0))
		assert.isTrue((await stabilityPoolERC20.getTotalUDeposits()).lt(UinSP_0ERC20))
	...
	it("liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
		...
		const UinDefaultPool_0 = await defaultPool.getUDebt(ZERO_ADDRESS)
		const UinDefaultPool_0ERC20 = await defaultPool.getUDebt(erc20.address)
    ...
    */
		...
		// Check U in DefaultPool has decreased
		assert.isTrue((await defaultPool.getUDebt(ZERO_ADDRESS)).gt(UinDefaultPool_0))
		assert.isTrue((await defaultPool.getUDebt(erc20.address)).gt(UinDefaultPool_0))
		...
		assert.isAtMost(
			th.getDifference(expectedLiquidatedCollERC20, toBN(ETHinDefaultPoolERC20)),
			1000
		)
	})
    ...
    */
		...
		// Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 U
    ...
    */
	...
	it("Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
		...
		while (price < 210) {
			...
			const ICRList = []
			const coll_firstTrove = (await troveManager.Troves(_10_accounts[0], ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			const gasComp_firstTrove = (
				await troveManager.getCollGasCompensation(ZERO_ADDRESS, coll_firstTrove)
			).toString()

			const ICRListERC20 = []
			const coll_firstTroveERC20 = (await troveManager.Troves(_10_accounts[0], erc20.address))[
				th.TROVE_COLL_INDEX
			]
			const gasComp_firstTroveERC20 = (
				await troveManager.getCollGasCompensation(erc20.address, coll_firstTroveERC20)
			).toString()
			...
			for (account of _10_accounts) {
				...
				const gasCompensation = (
					await troveManager.getCollGasCompensation(ZERO_ADDRESS, coll)
				).toString()

				const collERC20 = (await troveManager.Troves(account, erc20.address))[
					th.TROVE_COLL_INDEX
				]
				const gasCompensationERC20 = (
					await troveManager.getCollGasCompensation(erc20.address, collERC20)
				).toString()
	...
	it("Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
		...
		let coll = 50
		// create 20 troves, increasing collateral, constant debt = 100U
