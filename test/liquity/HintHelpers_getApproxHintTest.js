...
contract("HintHelpers", async accounts => {
	...
	/* Open a Trove for each account. U debt is 200 U each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

	// Open Troves in parallel, then withdraw U in parallel
	const makeTrovesInParallel = async (accounts, n) => {
		...
		const withdrawUpromises = activeAccounts.map(account => withdrawUfromTrove(account))
		await Promise.all(withdrawUpromises)
	...
	const withdrawUfromTrove = async account => {
		await borrowerOperations.withdrawU(th._100pct, "100000000000000000000", account, account, {
			from: account,
		})
	}

	// Sequentially add coll and withdraw U, 1 account at a time
