...
const logYOUBalanceAndError = (YOUBalance_A, expectedYOUBalance_A) => {
	console.log(
		`Expected final balance: ${expectedYOUBalance_A}, \n
    Actual final balance: ${YOUBalance_A}, \n
    Abs. error: ${expectedYOUBalance_A.sub(YOUBalance_A)}`
	)
}

const repeatedlyIssueYOU = async (stabilityPool, timeBetweenIssuances, duration) => {
	const startTimestamp = th.toBN(await th.getLatestBlockTimestamp(web3))
	let timePassed = 0

	// while current time < 1 month from deployment, issue YOU every minute
	while (timePassed < duration) {
		await th.fastForwardTime(timeBetweenIssuances, web3.currentProvider)
		await stabilityPool._unprotectedTriggerYOUIssuance()
...
contract("YOU community issuance arithmetic tests", async accounts => {
	...
	it("Cumulative issuance is correct after a week", async () => {
		...
		const issuanceFractionAfterERC20 =
			await communityIssuanceTester.getLastUpdateTokenDistribution(stabilityPoolERC20.address)
		assert.isAtMost(
	...
	it("Cumulative issuance is correct after two week", async () => {
		...
		assert.isAtMost(
			issuanceFractionAfterERC20.div(toBN(ERC_WEEKLY).mul(toBN(2))).toNumber(),
			2
		)
	})
	...
	it("Give full supply after 4 weeks, forwards 4 weeks, totalIssueVsta should be max supply", async () => {
		...
		await communityIssuanceTester.unprotectedIssueYOU(stabilityPool.address)
		const totalIssued = await communityIssuanceTester.totalYOUIssued(stabilityPool.address)
	...
	it("Give full supply after 4 weeks, forwards 8 weeks, totalIssueVsta should be max supply", async () => {
		...
		await th.fastForwardTime(
			toBN(timeValues.SECONDS_IN_ONE_WEEK).mul(toBN(8)),
			web3.currentProvider
		)
	...
	it("Total YOU tokens issued is correct after a week", async () => {
		...
		const initialIssuanceERC20 = await communityIssuanceTester.totalYOUIssued(
			stabilityPoolERC20.address
		)
		...
		// Issue YOU
		await communityIssuanceTester.unprotectedIssueYOU(stabilityPool.address)
		const totalYOUIssued = await communityIssuanceTester.totalYOUIssued(stabilityPool.address)

		await communityIssuanceTester.unprotectedIssueYOU(stabilityPoolERC20.address)
		const totalYOUIssuedERC20 = await communityIssuanceTester.totalYOUIssued(
			stabilityPoolERC20.address
		)

		assert.isAtMost(th.getDifference(totalYOUIssued, expectedReward), 1000000000000000)
		assert.isAtMost(th.getDifference(totalYOUIssuedERC20, expectedReward), 1000000000000000)
	})

	it("Total YOU tokens issued is correct after a month", async () => {
		const distribution = toBN(dec(8_000_000, 18))
		await communityIssuanceTester.setWeeklyVstaDistribution(
			stabilityPool.address,
			distribution
		...
		assert.isAtMost(
			th.getDifference(totalYOUIssued, distribution.mul(toBN(4))),
			1000000000000000
		)
		assert.isAtMost(
			th.getDifference(totalYOUIssuedERC20, distribution.mul(toBN(4))),
			1000000000000000
		)
	})
})
