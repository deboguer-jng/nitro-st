...
contract("TroveManager", async accounts => {
	...
	it("liquidate(): closes a Trove that has ICR < MCR", async () => {
		...
		assert.equal((await youParameters.MCR(ZERO_ADDRESS)).toString(), "1100000000000000000")
		assert.equal((await youParameters.MCR(erc20.address)).toString(), "1100000000000000000")
	...
	it("liquidate(): decreases ActivePool ETH and VSTDebt by correct amounts", async () => {
		...
		const activePooL_VSTDebt_Before_Asset = (
		...
		th.assertIsApproximatelyEqual(
			activePooL_VSTDebt_Before_Asset,
    ...
    leaving Alice’s ether and VST debt in the ActivePool. */
		...
		const activePooL_VSTDebt_After_Asset = (
			await activePool.getVSTDebt(erc20.address)
		).toString()
	...
	it("liquidate(): increases DefaultPool ETH and VST debt by correct amounts", async () => {
		...
		const defaultPooL_VSTDebt_Before_Asset = (
		...
		const defaultPooL_VSTDebt_After_Asset = (
			await defaultPool.getVSTDebt(erc20.address)
		).toString()
	...
	it("liquidate(): updates the L_ETH and L_VSTDebt reward-per-unit-staked totals", async () => {
		...
		const L_VSTDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(
		...
		const L_VSTDebt_expected_1_Asset = C_totalDebt_Asset.mul(mv._1e18BN).div(
		...
		assert.isAtMost(
			th.getDifference(L_VSTDebt_AfterCarolLiquidated, L_VSTDebt_expected_1),
			100
		)
		assert.isAtMost(
		...
		assert.isAtMost(
			th.getDifference(L_VSTDebt_AfterCarolLiquidated_Asset, L_VSTDebt_expected_1_Asset),
			100
		)

		// Bob now withdraws VST, bringing his ICR to 1.11
		const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawVST({
			ICR: toBN(dec(111, 16)),
			extraParams: { from: bob },
		})
		const { increasedTotalDebt: B_increasedTotalDebt_Asset } = await withdrawVST({
			asset: erc20.address,
			ICR: toBN(dec(111, 16)),
			extraParams: { from: bob },
		})
   ...
   L_VSTDebt = (180 / 20) + (890 / 10) = 98 VST */
		...
		const L_VSTDebt_expected_2 = L_VSTDebt_expected_1.add(
			B_totalDebt.add(B_increasedTotalDebt)
				.add(B_collateral.mul(L_VSTDebt_expected_1).div(mv._1e18BN))
				.mul(mv._1e18BN)
				.div(A_collateral)
		)
	...
	it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
		...
		await openTrove({
			ICR: toBN(dec(218, 16)),
			extraVSTAmount: toBN(dec(100, 18)),
			extraParams: { from: carol },
		})
		await openTrove({
			asset: erc20.address,
	...
	it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
		...
		const bob_Deposit_Before_Asset = (
			await stabilityPoolERC20.getCompoundedVSTDeposit(bob)
		).toString()
	...
	it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
     ...
     Check Bob' SP deposit has been reduced to 50 VST, and his ETH gain has increased to 1.5 ETH. */
		...
		const alice_Deposit_After_Asset = (
			await stabilityPoolERC20.getCompoundedVSTDeposit(alice)
		).toString()
	...
	it("liquidate(): does not alter the liquidated user's token balance", async () => {
		await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
		const { VSTAmount: A_VSTAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(300, 18)),
			extraParams: { from: alice },
		})
		const { VSTAmount: B_VSTAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(200, 18)),
			extraParams: { from: bob },
		})
		const { VSTAmount: C_VSTAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(100, 18)),
			extraParams: { from: carol },
		})
		...
		const { VSTAmount: A_VSTAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(300, 18)),
			extraParams: { from: alice },
		})
		const { VSTAmount: B_VSTAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(200, 18)),
			extraParams: { from: bob },
		})
		const { VSTAmount: C_VSTAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: toBN(dec(100, 18)),
			extraParams: { from: carol },
		...
		assert.equal(
			(await VSTToken.balanceOf(alice)).toString(),
			A_VSTAmount.add(A_VSTAmount_Asset)
		)
		assert.equal(
			(await VSTToken.balanceOf(bob)).toString(),
			B_VSTAmount.add(B_VSTAmount_Asset)
		)
		assert.equal(
			(await VSTToken.balanceOf(carol)).toString(),
			C_VSTAmount.add(C_VSTAmount_Asset)
		)
	})
   ...
   (because his pending rewards bring his ICR < MCR) */
	...
	it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
		...
		await openTrove({
			ICR: toBN(dec(100, 18)),
			extraVSTAmount: toBN(dec(500, 18)),
			extraParams: { from: whale },
		})
		await openTrove({
			asset: erc20.address,
	...
	it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
		...
		const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: A_deposit,
			extraParams: { from: alice },
		})
		const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraVSTAmount: B_deposit,
			extraParams: { from: bob },
		})
		const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
    ...
    Total ETH gain: 4.975 ETH */
		...
		assert.isAtMost(
			th.getDifference(total_VSTinSP_Asset, totalDeposits_Asset.sub(liquidatedDebt_Asset)),
			1000
		)
		assert.isAtMost(
	...
	it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
		...
		th.assertIsApproximatelyEqual(
			(await stabilityPool.getTotalVSTDeposits()).toString(),
			spDeposit.sub(A_debt).sub(B_debt)
		)
		th.assertIsApproximatelyEqual(
			(await stabilityPoolERC20.getTotalVSTDeposits()).toString(),
	...
	it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
		...
		const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(310, 16)),
			extraVSTAmount: partialRedemptionAmount,
			extraParams: { from: alice },
		})
	...
	it("redeemCollateral(): cancels the provided VST with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
		...
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(310, 16)),
			extraVSTAmount: dec(10, 18),
			extraParams: { from: alice },
		})
		const { netDebt: B_netDebt } = await openTrove({
			ICR: toBN(dec(290, 16)),
			extraVSTAmount: dec(8, 18),
			extraParams: { from: bob },
		})
		...
		await openTrove({
			ICR: toBN(dec(100, 18)),
			extraVSTAmount: redemptionAmount,
			extraParams: { from: dennis },
		})
		await openTrove({
			asset: erc20.address,
    ...
    It leaves her with (3) VST debt + 50 for gas compensation. */
		...
		assert.equal(
			...
			dennis_VSTBalance_Before.sub(redemptionAmount).sub(redemptionAmount_Asset)
		)
	})
	...
	it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
		...
		const { VSTAmount: F_VSTAmount } = await openTrove({
			ICR: toBN(dec(200, 18)),
			extraVSTAmount: redemptionAmount.mul(toBN(2)),
			extraParams: { from: flyn },
		})
		const { VSTAmount: F_VSTAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 18)),
			extraVSTAmount: redemptionAmount.mul(toBN(2)),
			extraParams: { from: flyn },
		...
		th.assertIsApproximatelyEqual(
			flynBalance,
			F_VSTAmount.add(F_VSTAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
		)
	...
	it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveVSTAmount(dec(20000, 18), ZERO_ADDRESS),
			B,
			B,
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveVSTAmount(dec(30000, 18), ZERO_ADDRESS),
			C,
			C,
		...
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveVSTAmount(dec(10000, 18), erc20.address),
			A,
			A,
			{ from: A }
		)
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveVSTAmount(dec(20000, 18), erc20.address),
			B,
			B,
			{ from: B }
		)
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveVSTAmount(dec(30000, 18), erc20.address),
			C,
			C,
			{ from: C }
		)
	...
	it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveVSTAmount(dec(6000, 18), ZERO_ADDRESS),
			A,
			A,
			{ from: A, value: dec(1000, "ether") }
		)
		...
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveVSTAmount(dec(6000, 18), erc20.address),
			A,
			A,
			{ from: A }
		)
	...
	it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
		...
		const expectedTotalETHDrawn = fullfilledRedemptionAmount
			...
			.div(toBN(200)) // redempted VST converted to ETH, at ETH:USD price 200
	...
	it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
		...
		const { VSTAmount: B_VSTAmount, totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(133, 16)),
			extraVSTAmount: A_debt,
			extraParams: { from: bob },
		})
		...
		const { VSTAmount: B_VSTAmount_Asset, totalDebt: B_totalDebt_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(133, 16)),
			extraVSTAmount: A_debt,
			extraParams: { from: bob },
	...
	it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
		...
		const { totalDebt: D_totalDebt_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(195, 16)),
			extraVSTAmount: redemptionAmount_Asset,
			extraParams: { from: dennis },
		})
	...
	it("redeemCollateral(): reverts if max fee > 100%", async () => {
		...
		await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(30, 18),
			extraParams: { from: C },
		})
		await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(40, 18),
			extraParams: { from: D },
		})
	...
	it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(80, 18),
			extraParams: { from: A },
		})
		const { totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(90, 18),
			extraParams: { from: B },
		})
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(100, 18),
			extraParams: { from: C },
		})
	...
	it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraVSTAmount: dec(9500, 18),
			extraParams: { from: A },
		})
		const { totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(395, 16)),
			extraVSTAmount: dec(9000, 18),
			extraParams: { from: B },
		})
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(390, 16)),
			extraVSTAmount: dec(10000, 18),
			extraParams: { from: C },
		})
	...
	it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
		...
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(195, 16)),
			extraVSTAmount: dec(200, 18),
			extraParams: { from: carol },
		})
		const { totalDebt: D_totalDebt } = await openTrove({
			ICR: toBN(dec(190, 16)),
			extraVSTAmount: dec(400, 18),
			extraParams: { from: dennis },
		})
		...
		await VSTToken.transfer(erin, redemptionAmount.add(redemptionAmount_Asset), {
			from: alice,
		})
		...
		const carol_SPDeposit_before = (
			await stabilityPool.getCompoundedVSTDeposit(carol)
		).toString()
		...
		const carol_SPDeposit_before_Asset = (
			await stabilityPoolERC20.getCompoundedVSTDeposit(carol)
		).toString()
    ...
    */
	...
	it("redeemCollateral(): reverts when requested redemption amount exceeds caller's VST token balance", async () => {
		...
		try {
			...
			assert.include(
				error.message,
				"Requested redemption amount must be <= user's VST token balance"
			)
		}
	...
	it("redeemCollateral(): value of issued ETH == face value of redeemed VST (assuming 1 VST has value of $1)", async () => {
		...
		const { collateral: B_coll } = await openTrove({
			ICR: toBN(dec(300, 16)),
			extraVSTAmount: dec(1590, 18),
			extraParams: { from: bob },
		})
		const { collateral: C_coll } = await openTrove({
			ICR: toBN(dec(600, 16)),
			extraVSTAmount: dec(1090, 18),
			extraParams: { from: carol },
		})
    ...
    Total active ETH = 280 - 0.6 = 279.4 ETH */
		...
		assert.equal(
			...
			activeETH_0.sub(toBN(_120_VST).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_1_Asset.toString(),
			activeETH_0_Asset.sub(toBN(_120_VST).mul(mv._1e18BN).div(price))
		)
    ...
    Total active ETH = 279.4 - 1.865 = 277.535 ETH */
		...
		assert.equal(
			...
			activeETH_1.sub(toBN(_373_VST).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_2_Asset.toString(),
			activeETH_1_Asset.sub(toBN(_373_VST).mul(mv._1e18BN).div(price))
		)
    ...
    Total active ETH = 277.535 - 4.75 = 272.785 ETH */
		...
		assert.equal(
			...
			activeETH_2.sub(toBN(_950_VST).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_3_Asset.toString(),
			activeETH_2_Asset.sub(toBN(_950_VST).mul(mv._1e18BN).div(price))
		)
	})
	...
	it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
		...
		th.assertIsApproximatelyEqual(
			(await activePool.getVSTDebt(ZERO_ADDRESS)).toString(),
			totalDebt
		)
		th.assertIsApproximatelyEqual(
			(await activePool.getVSTDebt(erc20.address)).toString(),
			totalDebt_Asset
		)
	...
	it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
		...
		// Check A's balance has decreased by 10 VST
		assert.equal(
			A_balanceBefore.sub(await VSTToken.balanceOf(A)).toString(),
    ...
    */
	...
	it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
		// --- SETUP ---
		const { VSTAmount } = await openTrove({
			ICR: toBN(dec(200, 16)),
			extraVSTAmount: dec(1, 24),
			extraParams: { from: alice },
		})
		...
		const { VSTAmount: VSTAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraVSTAmount: dec(1, 24),
			extraParams: { from: alice },
		...
		for (let i = 0; i < 2; i++) {
			...
			await borrowerOperations.adjustTrove(
				...
				alice,
				alice,
				{ from: alice, value: VSTAmount.mul(mv._1e18BN).div(price) }
			)
		}
		...
		for (let i = 0; i < 2; i++) {
			...
			await borrowerOperations.adjustTrove(
				erc20.address,
				VSTAmount_Asset.mul(mv._1e18BN).div(price),
				th._100pct,
				0,
				VSTAmount_Asset,
				true,
	...
	it("getPendingVSTDebtReward(): Returns 0 if there is no pending VSTDebt reward", async () => {
		...
		const carolSnapshot_L_VSTDebt = (
			await troveManager.rewardSnapshots(carol, ZERO_ADDRESS)
		)[1]
		assert.equal(carolSnapshot_L_VSTDebt, 0)

		const carolSnapshot_L_VSTDebt_Asset = (
			await troveManager.rewardSnapshots(carol, erc20.address)
		)[1]
		assert.equal(carolSnapshot_L_VSTDebt_Asset, 0)

		const carol_PendingVSTDebtReward = await troveManager.getPendingVSTDebtReward(
			ZERO_ADDRESS,
			carol
		)
		assert.equal(carol_PendingVSTDebtReward, 0)

		const carol_PendingVSTDebtReward_Asset = await troveManager.getPendingVSTDebtReward(
			erc20.address,
			carol
		)
