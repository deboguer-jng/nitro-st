...
contract("TroveManager", async accounts => {
	...
	const withdrawU = async params => th.withdrawU(contracts, params)
	...
	it("liquidate(): closes a Trove that has ICR < MCR", async () => {
		...
		// Alice increases debt to 180 U, lowering her ICR to 1.11
		...
		await withdrawU({ ICR: targetICR, extraParams: { from: alice } })
		await withdrawU({ asset: erc20.address, ICR: targetICR, extraParams: { from: alice } })

		const ICR_AfterWithdrawal = await troveManager.getCurrentICR(ZERO_ADDRESS, alice, price)
		const ICR_AfterWithdrawal_Asset = await troveManager.getCurrentICR(
			erc20.address,
			alice,
			price
		)
		...
		// price drops to 1ETH:100U, reducing Alice's ICR below MCR
		...
		const status_Asset = (await troveManager.Troves(alice, erc20.address))[
	...
	it("liquidate(): decreases ActivePool ETH and UDebt by correct amounts", async () => {
		// --- SETUP ---
		...
		// check ActivePool ETH and U debt before
		...
		const activePooL_UDebt_Before = (await activePool.getUDebt(ZERO_ADDRESS)).toString()
		...
		const activePooL_UDebt_Before_Asset = (await activePool.getUDebt(erc20.address)).toString()
		...
		th.assertIsApproximatelyEqual(activePooL_UDebt_Before, A_totalDebt.add(B_totalDebt))
		...
		th.assertIsApproximatelyEqual(
			activePooL_UDebt_Before_Asset,
			A_totalDebt_Asset.add(B_totalDebt_Asset)
		)

		// price drops to 1ETH:100U, reducing Bob's ICR below MCR
		await priceFeed.setPrice("100000000000000000000")
    ...
    leaving Alice’s ether and U debt in the ActivePool. */
		...
		const activePooL_UDebt_After_Asset = (await activePool.getUDebt(erc20.address)).toString()

		assert.equal(activePool_ETH_After, A_collateral)
		assert.equal(activePool_RawEther_After, A_collateral)
		th.assertIsApproximatelyEqual(activePooL_UDebt_After, A_totalDebt)
		...
		th.assertIsApproximatelyEqual(activePooL_UDebt_After_Asset, A_totalDebt_Asset)
	})

	it("liquidate(): increases DefaultPool ETH and U debt by correct amounts", async () => {
		// --- SETUP ---
		await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
		const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(21, 17)),
		...
		// check DefaultPool ETH and U debt before
		...
		const defaultPooL_UDebt_Before = (await defaultPool.getUDebt(ZERO_ADDRESS)).toString()
		...
		const defaultPooL_UDebt_After = (await defaultPool.getUDebt(ZERO_ADDRESS)).toString()
		...
		const defaultPooL_UDebt_After_Asset = (
			await defaultPool.getUDebt(erc20.address)
		).toString()
		...
		th.assertIsApproximatelyEqual(defaultPooL_UDebt_After, B_totalDebt)
		...
		th.assertIsApproximatelyEqual(defaultPooL_UDebt_After_Asset, B_totalDebt_Asset)
	})
    ...
    Check all remaining troves in the array are in the correct order */
		...
		const whale_arrayIndex = (await troveManager.Troves(whale, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		]
		const alice_arrayIndex = (await troveManager.Troves(alice, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		]
		const bob_arrayIndex = (await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_ARRAY_INDEX]
		const dennis_arrayIndex = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		]
		const erin_arrayIndex = (await troveManager.Troves(erin, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		]

		const whale_arrayIndex_Asset = (await troveManager.Troves(whale, erc20.address))[
			th.TROVE_ARRAY_INDEX
		]
		const alice_arrayIndex_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_ARRAY_INDEX
		]
		const bob_arrayIndex_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_ARRAY_INDEX
		]
		const dennis_arrayIndex_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_ARRAY_INDEX
		]
		const erin_arrayIndex_Asset = (await troveManager.Troves(erin, erc20.address))[
			th.TROVE_ARRAY_INDEX
		]
	...
	it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
		...
		const totalStakesSnapshot_Before = (
			await troveManager.totalStakesSnapshot(ZERO_ADDRESS)
		).toString()
		const totalCollateralSnapshot_Before = (
			await troveManager.totalCollateralSnapshot(ZERO_ADDRESS)
		).toString()
		const totalStakesSnapshot_Before_Asset = (
			await troveManager.totalStakesSnapshot(erc20.address)
		).toString()
		const totalCollateralSnapshot_Before_Asset = (
			await troveManager.totalCollateralSnapshot(erc20.address)
		).toString()
		...
		// close Bob's Trove.  His ether*0.995 and U should be added to the DefaultPool.
		await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
		await troveManager.liquidate(erc20.address, bob, { from: owner })
	...
	it("liquidate(): updates the L_ETH and L_UDebt reward-per-unit-staked totals", async () => {
		// --- SETUP ---
		const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(8, 18)),
			extraParams: { from: alice },
		...
		// price drops to 1ETH:100U, reducing Carols's ICR below MCR
		await priceFeed.setPrice("100000000000000000000")
		...
		// Carol's ether*0.995 and U should be added to the DefaultPool.
		const L_ETH_AfterCarolLiquidated = await troveManager.L_ASSETS(ZERO_ADDRESS)
		const L_UDebt_AfterCarolLiquidated = await troveManager.L_UDebts(ZERO_ADDRESS)
		const L_ETH_AfterCarolLiquidated_Asset = await troveManager.L_ASSETS(erc20.address)
		const L_UDebt_AfterCarolLiquidated_Asset = await troveManager.L_UDebts(erc20.address)

		const L_ETH_expected_1 = th
			.applyLiquidationFee(C_collateral)
			.mul(mv._1e18BN)
			.div(A_collateral.add(B_collateral))
		const L_UDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
		const L_ETH_expected_1_Asset = th
			.applyLiquidationFee(C_collateral_Asset)
			.mul(mv._1e18BN)
			.div(A_collateral_Asset.add(B_collateral_Asset))
		const L_UDebt_expected_1_Asset = C_totalDebt_Asset.mul(mv._1e18BN).div(
		...
		assert.isAtMost(th.getDifference(L_UDebt_AfterCarolLiquidated, L_UDebt_expected_1), 100)
		assert.isAtMost(
			th.getDifference(L_ETH_AfterCarolLiquidated_Asset, L_ETH_expected_1_Asset),
			100
		)
		assert.isAtMost(
			th.getDifference(L_UDebt_AfterCarolLiquidated_Asset, L_UDebt_expected_1_Asset),
			100
		)

		// Bob now withdraws U, bringing his ICR to 1.11
		const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawU({
			ICR: toBN(dec(111, 16)),
			extraParams: { from: bob },
		})
		const { increasedTotalDebt: B_increasedTotalDebt_Asset } = await withdrawU({
			asset: erc20.address,
			ICR: toBN(dec(111, 16)),
			extraParams: { from: bob },
		})
		...
		// price drops to 1ETH:50U, reducing Bob's ICR below MCR
   ...
   L_UDebt = (180 / 20) + (890 / 10) = 98 U */
		const L_ETH_AfterBobLiquidated = await troveManager.L_ASSETS(ZERO_ADDRESS)
		const L_UDebt_AfterBobLiquidated = await troveManager.L_UDebts(ZERO_ADDRESS)
		...
		const L_UDebt_expected_2 = L_UDebt_expected_1.add(
			B_totalDebt.add(B_increasedTotalDebt)
				.add(B_collateral.mul(L_UDebt_expected_1).div(mv._1e18BN))
				.mul(mv._1e18BN)
				.div(A_collateral)
		)
		assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100)
		assert.isAtMost(th.getDifference(L_UDebt_AfterBobLiquidated, L_UDebt_expected_2), 100)
	})
	...
	it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
		...
		// Alice creates a single trove with 0.7 ETH and a debt of 70 U, and provides 10 U to SP
		...
		const alice_ICR_Asset = (
			await troveManager.getCurrentICR(erc20.address, alice, price)
		).toString()
		...
		const activeTrovesCount_Before_Asset = await troveManager.getTroveOwnersCount(
			erc20.address
		)
	...
	it("liquidate(): does nothing if trove has >= 110% ICR", async () => {
		...
		await assertRevert(
			troveManager.liquidate(ZERO_ADDRESS, bob),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidate(erc20.address, bob),
			"TroveManager: nothing to liquidate"
		)
	...
	it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
		// Whale provides U to SP
		const spDeposit = toBN(dec(100, 24))
		await openTrove({
			ICR: toBN(dec(4, 18)),
			extraUAmount: spDeposit,
			extraParams: { from: whale },
		})
		await openTrove({
			asset: erc20.address,
	...
	it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
		...
		await UToken.transfer(dennis, spDeposit.mul(toBN(2)), { from: bob })

		//Dennis provides U to SP
		...
		const dennis_Deposit_Before = (
			await stabilityPool.getCompoundedUDeposit(dennis)
		).toString()
		const dennis_ETHGain_Before = (
			await stabilityPool.getDepositorAssetGain(dennis)
		).toString()
		const dennis_Deposit_Before_Asset = (
			await stabilityPoolERC20.getCompoundedUDeposit(dennis)
		).toString()
		const dennis_ETHGain_Before_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(dennis)
		).toString()
		assert.isAtMost(
		...
		const dennis_Deposit_After = (await stabilityPool.getCompoundedUDeposit(dennis)).toString()
	...
	it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
		...
		const bob_Deposit_Before = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
		const bob_ETHGain_Before = (await stabilityPool.getDepositorAssetGain(bob)).toString()

		const bob_Deposit_Before_Asset = (
			await stabilityPoolERC20.getCompoundedUDeposit(bob)
		).toString()
		const bob_ETHGain_Before_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(bob)
		).toString()
		...
		assert.isAtMost(
			th.getDifference(bob_Deposit_Before_Asset, spDeposit.sub(liquidatedDebt_Asset)),
			1000000
		)
		assert.isAtMost(
		...
		const bob_Deposit_After = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
	...
	it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
		...
		// Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
		const bob_Deposit_Before = await stabilityPool.getCompoundedUDeposit(bob)
		const bob_ETHGain_Before = await stabilityPool.getDepositorAssetGain(bob)

		const bob_Deposit_Before_Asset = await stabilityPoolERC20.getCompoundedUDeposit(bob)
		...
		assert.isAtMost(
			th.getDifference(bob_Deposit_Before_Asset, B_spDeposit.sub(C_debt_Asset)),
			1000000
		)
		assert.isAtMost(
			th.getDifference(
		...
		assert.isFalse(await sortedTroves.contains(ZERO_ADDRESS, bob))
		assert.isFalse(await sortedTroves.contains(erc20.address, bob))
		const bob_Trove_Status = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_STATUS_INDEX
		].toString()
		const bob_Trove_Status_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_STATUS_INDEX
		].toString()
		assert.equal(bob_Trove_Status, 3)
		assert.equal(bob_Trove_Status_Asset, 3)

		/* Alice's U Loss = (300 / 400) * 200 = 150 U
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's ULoss = (100 / 400) * 200 = 50 U
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 U, and his ETH gain has increased to 1.5 ETH. */
		const alice_Deposit_After = (await stabilityPool.getCompoundedUDeposit(alice)).toString()
		const alice_ETHGain_After = (await stabilityPool.getDepositorAssetGain(alice)).toString()

		const alice_Deposit_After_Asset = (
			await stabilityPoolERC20.getCompoundedUDeposit(alice)
		).toString()
		...
		const bob_Deposit_After = await stabilityPool.getCompoundedUDeposit(bob)
		const bob_ETHGain_After = await stabilityPool.getDepositorAssetGain(bob)

		const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedUDeposit(bob)
		...
		assert.isAtMost(
			th.getDifference(
				...
				bob_Deposit_Before_Asset.sub(
					B_debt.mul(bob_Deposit_Before_Asset).div(totalDeposits_Asset)
				)
			),
			1000000
		)
	...
	it("liquidate(): does not alter the liquidated user's token balance", async () => {
		await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
		const { UAmount: A_UAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(300, 18)),
			extraParams: { from: alice },
		})
		const { UAmount: B_UAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(200, 18)),
			extraParams: { from: bob },
		})
		const { UAmount: C_UAmount } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(100, 18)),
			extraParams: { from: carol },
		})
		...
		const { UAmount: A_UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(300, 18)),
			extraParams: { from: alice },
		})
		const { UAmount: B_UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(200, 18)),
			extraParams: { from: bob },
		})
		const { UAmount: C_UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraUAmount: toBN(dec(100, 18)),
			extraParams: { from: carol },
		...
		// Liquidate A, B and C
		await activePool.getUDebt(ZERO_ADDRESS)
		await defaultPool.getUDebt(ZERO_ADDRESS)
		await activePool.getUDebt(erc20.address)
		await defaultPool.getUDebt(erc20.address)
		...
		assert.equal((await UToken.balanceOf(alice)).toString(), A_UAmount.add(A_UAmount_Asset))
		assert.equal((await UToken.balanceOf(bob)).toString(), B_UAmount.add(B_UAmount_Asset))
		assert.equal((await UToken.balanceOf(carol)).toString(), C_UAmount.add(C_UAmount_Asset))
	})
	...
	it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
		...
		// Defaulter opens with 60 U, 0.6 ETH
		...
		const alice_ICR_Before_Asset = await troveManager.getCurrentICR(
			erc20.address,
			alice,
			price
		)
		const bob_ICR_Before_Asset = await troveManager.getCurrentICR(erc20.address, bob, price)
		const carol_ICR_Before_Asset = await troveManager.getCurrentICR(
			erc20.address,
			carol,
			price
		)
    ...
    Therefore Alice and Bob above the MCR, Carol is below */
		...
		/* Liquidate defaulter. 30 U and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 U, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 U, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 U, and (0.3*1/4) = 0.075 ETH
    */
		await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
		await troveManager.liquidate(erc20.address, defaulter_1)
    ...
    check that Bob's raw coll and debt has not changed, and that his "raw" ICR is above the MCR */
		...
		await assertRevert(
			troveManager.liquidate(ZERO_ADDRESS, alice),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidate(erc20.address, alice),
			"TroveManager: nothing to liquidate"
		)
		await troveManager.liquidate(ZERO_ADDRESS, bob)
		await troveManager.liquidate(ZERO_ADDRESS, carol)
   ...
   (because his pending rewards bring his ICR < MCR) */
		...
		assert.equal(
			(await troveManager.Troves(alice, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"1"
		)
		assert.equal(
			(await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		assert.equal(
			(await troveManager.Troves(carol, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)

		assert.equal(
			(await troveManager.Troves(alice, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"1"
		)
		assert.equal(
			(await troveManager.Troves(bob, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		assert.equal(
			(await troveManager.Troves(carol, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
	})

	it("liquidate(): when SP > 0, triggers YOU reward event - increases the sum G", async () => {
		...
		assert.equal(await stabilityPool.getTotalUDeposits(), dec(100, 18))

		await stabilityPoolERC20.provideToSP(dec(100, 18), { from: B })
		assert.equal(await stabilityPoolERC20.getTotalUDeposits(), dec(100, 18))
		...
		// Price drops to 1ETH:100U, reducing defaulters to below MCR
		await priceFeed.setPrice(dec(100, 18))
		await priceFeed.getPrice()
		assert.isFalse(await th.checkRecoveryMode(contracts))
		assert.isFalse(await th.checkRecoveryMode(contracts, erc20.address))
		...
		// Expect G has increased from the YOU reward event triggered
	...
	it("liquidate(): when SP is empty, doesn't update G", async () => {
		...
		// Check SP is empty
		assert.equal(await stabilityPool.getTotalUDeposits(), "0")
		assert.equal(await stabilityPoolERC20.getTotalUDeposits(), "0")
	...
	it("liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
		...
		// A adds 10 U to the SP, but less than C's debt
		...
		await borrowerOperations.repayU(ZERO_ADDRESS, dec(1, 18), D, D, { from: D })
		await borrowerOperations.repayU(ZERO_ADDRESS, dec(1, 18), E, E, { from: E })
		await borrowerOperations.repayU(erc20.address, dec(1, 18), D, D, { from: D })
		await borrowerOperations.repayU(erc20.address, dec(1, 18), E, E, { from: E })
		...
		const pendingUDebt_C = await troveManager.getPendingUDebtReward(ZERO_ADDRESS, C)
		const defaultPoolETH = await defaultPool.getAssetBalance(ZERO_ADDRESS)
		const defaultPoolUDebt = await defaultPool.getUDebt(ZERO_ADDRESS)

		const pendingETH_C_Asset = await troveManager.getPendingAssetReward(erc20.address, C)
		const pendingUDebt_C_Asset = await troveManager.getPendingUDebtReward(erc20.address, C)
		const defaultPoolETH_Asset = await defaultPool.getAssetBalance(erc20.address)
		const defaultPoolUDebt_Asset = await defaultPool.getUDebt(erc20.address)

		assert.isTrue(pendingETH_C.lte(defaultPoolETH))
		assert.isTrue(pendingUDebt_C.lte(defaultPoolUDebt))

		assert.isTrue(pendingETH_C_Asset.lte(defaultPoolETH_Asset))
		assert.isTrue(pendingUDebt_C_Asset.lte(defaultPoolUDebt_Asset))
		//Check only difference is dust
		assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000)
		assert.isAtMost(th.getDifference(pendingUDebt_C, defaultPoolUDebt), 1000)

		assert.isAtMost(th.getDifference(pendingETH_C_Asset, defaultPoolETH_Asset), 1000)
		assert.isAtMost(th.getDifference(pendingUDebt_C_Asset, defaultPoolUDebt_Asset), 1000)
	...
	it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
		...
		// Price drops to 1ETH:100U, reducing Bob and Carol's ICR below MCR
		await priceFeed.setPrice(dec(100, 18))
		const price = await priceFeed.getPrice()
		...
		assert.isTrue(
			(await troveManager.getCurrentICR(ZERO_ADDRESS, harriet, price)).gte(mv._MCR)
		)
		assert.isTrue((await troveManager.getCurrentICR(ZERO_ADDRESS, ida, price)).gte(mv._MCR))

		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, graham, price)).gte(mv._MCR)
		)
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, harriet, price)).gte(mv._MCR)
		)
		...
		assert.equal(
			(await troveManager.Troves(erin, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		assert.equal(
			(await troveManager.Troves(flyn, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		...
		assert.equal(
			(await troveManager.Troves(erin, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		assert.equal(
			(await troveManager.Troves(flyn, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
	...
	it("liquidateTroves(): liquidates  up to the requested number of undercollateralized troves", async () => {
		...
		const aliceTroveStatus = (
			await troveManager.getTroveStatus(ZERO_ADDRESS, alice)
		).toString()
		const bobTroveStatus = (await troveManager.getTroveStatus(ZERO_ADDRESS, bob)).toString()
		const carolTroveStatus = (
			await troveManager.getTroveStatus(ZERO_ADDRESS, carol)
		).toString()

		const aliceTroveStatus_Asset = (
			await troveManager.getTroveStatus(erc20.address, alice)
		).toString()
		const bobTroveStatus_Asset = (
			await troveManager.getTroveStatus(erc20.address, bob)
		).toString()
		const carolTroveStatus_Asset = (
			await troveManager.getTroveStatus(erc20.address, carol)
		).toString()
		...
		const dennisTroveStatus = (
			await troveManager.getTroveStatus(ZERO_ADDRESS, dennis)
		).toString()
		const erinTroveStatus = (await troveManager.getTroveStatus(ZERO_ADDRESS, erin)).toString()

		const dennisTroveStatus_Asset = (
			await troveManager.getTroveStatus(erc20.address, dennis)
		).toString()
		const erinTroveStatus_Asset = (
			await troveManager.getTroveStatus(erc20.address, erin)
		).toString()
	...
	it("liquidateTroves(): does nothing if all troves have ICR > 110%", async () => {
		...
		await assertRevert(
			troveManager.liquidateTroves(ZERO_ADDRESS, 10),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidateTroves(erc20.address, 10),
			"TroveManager: nothing to liquidate"
		)
    ...
    Therefore Alice and Bob above the MCR, Carol is below */
		...
		// Liquidate defaulter. 30 U and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 U, 0.1 ETH
		await troveManager.liquidate(ZERO_ADDRESS, defaulter_1)
		await troveManager.liquidate(erc20.address, defaulter_1)
	...
	it("liquidateTroves(): reverts if n = 0", async () => {
		...
		await assertRevert(
			troveManager.liquidateTroves(ZERO_ADDRESS, 0),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidateTroves(erc20.address, 0),
			"TroveManager: nothing to liquidate"
		)
	...
	it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
		...
		const D_balanceBefore = await UToken.balanceOf(dennis)
		const E_balanceBefore = await UToken.balanceOf(erin)
		const F_balanceBefore = await UToken.balanceOf(flyn)
		...
		assert.equal((await UToken.balanceOf(dennis)).toString(), D_balanceBefore)
		assert.equal((await UToken.balanceOf(erin)).toString(), E_balanceBefore)
		assert.equal((await UToken.balanceOf(flyn)).toString(), F_balanceBefore)
	})

	it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
		// Whale provides 500 U to SP
		await openTrove({
			ICR: toBN(dec(100, 18)),
			extraUAmount: toBN(dec(500, 18)),
			extraParams: { from: whale },
		})
		await openTrove({
			asset: erc20.address,
		...
		// Check pool has 500 U
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), dec(500, 18))
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), dec(500, 18))
		...
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), "0")
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), "0")
	...
	it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
		...
		assert.isAtMost(
			th.getDifference(TCR_Before_Asset, totalColl_Asset.mul(price).div(totalDebt)),
			1000
		)
	...
	it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
		// Whale provides 400 U to the SP
		const whaleDeposit = toBN(dec(40000, 18))
		await openTrove({
			ICR: toBN(dec(100, 18)),
			extraUAmount: whaleDeposit,
			extraParams: { from: whale },
		})
		await openTrove({
			asset: erc20.address,
		...
		const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: A_deposit,
			extraParams: { from: alice },
		})
		const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
			extraUAmount: B_deposit,
			extraParams: { from: bob },
		})
		const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
			ICR: toBN(dec(2, 18)),
		...
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), totalDeposits)
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), totalDeposits)
    ...
    Total U in Pool: 800 U

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 U
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale U loss: 650 * (400/800) = 325 U
    alice U loss:  650 *(100/800) = 81.25 U
    bob U loss: 650 * (300/800) = 243.75 U

    whale remaining deposit: (400 - 325) = 75 U
    alice remaining deposit: (100 - 81.25) = 18.75 U
    bob remaining deposit: (300 - 243.75) = 56.25 U
    ...
    Total remaining deposits: 150 U
    Total ETH gain: 4.975 ETH */

		// Check remaining U Deposits and ETH gain, for whale and depositors whose troves were liquidated
		const whale_Deposit_After = await stabilityPool.getCompoundedUDeposit(whale)
		const alice_Deposit_After = await stabilityPool.getCompoundedUDeposit(alice)
		const bob_Deposit_After = await stabilityPool.getCompoundedUDeposit(bob)
		...
		const whale_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedUDeposit(whale)
		const alice_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedUDeposit(alice)
		const bob_Deposit_After_Asset = await stabilityPoolERC20.getCompoundedUDeposit(bob)
		...
		const total_UinSP = (await stabilityPool.getTotalUDeposits()).toString()
		const total_ETHinSP = (await stabilityPool.getAssetBalance()).toString()

		const total_UinSP_Asset = (await stabilityPoolERC20.getTotalUDeposits()).toString()
		const total_ETHinSP_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

		assert.isAtMost(th.getDifference(total_UinSP, totalDeposits.sub(liquidatedDebt)), 1000)
		assert.isAtMost(
			th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)),
			1000
		)
		assert.isAtMost(
			th.getDifference(total_UinSP_Asset, totalDeposits_Asset.sub(liquidatedDebt_Asset)),
			1000
		)
		assert.isAtMost(
			th.getDifference(total_ETHinSP_Asset, th.applyLiquidationFee(liquidatedColl_Asset)),
			1000
		)
	})

	it("liquidateTroves(): when SP > 0, triggers YOU reward event - increases the sum G", async () => {
		await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(100, 18)),
	...
	it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
		...
		// Price drops to 1ETH:100U, reducing A, B, C ICR below MCR
		await priceFeed.setPrice(dec(100, 18))
		const price = await priceFeed.getPrice()
		...
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, dennis, price)).gte(mv._MCR)
		)
	...
	it("batchLiquidateTroves(): does not liquidate troves that are not in the given array", async () => {
		...
		assert.equal(
			(await troveManager.Troves(dennis, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"1"
		)
		assert.equal(
			(await troveManager.Troves(erin, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
		...
		assert.equal(
			(await troveManager.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"1"
		)
		assert.equal(
			(await troveManager.Troves(erin, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
	...
	it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
		...
		th.assertIsApproximatelyEqual(
			(await stabilityPool.getTotalUDeposits()).toString(),
			spDeposit.sub(A_debt).sub(B_debt)
		)
		th.assertIsApproximatelyEqual(
			(await stabilityPoolERC20.getTotalUDeposits()).toString(),
	...
	it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
		...
		await UToken.transfer(carol, dec(200, 18), { from: whale })
		...
		const txCarolClose_Asset = await borrowerOperations.closeTrove(erc20.address, {
			from: carol,
		})
	...
	it("batchLiquidateTroves: when SP > 0, triggers YOU reward event - increases the sum G", async () => {
		await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(100, 18)),
	...
	it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
		...
		const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(310, 16)),
			extraUAmount: partialRedemptionAmount,
			extraParams: { from: alice },
		})
	...
	it("getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations", async () => {
		...
		// Get hints for a redemption of 170 + 30 + some extra U. At least 3 iterations are needed
	...
	it("redeemCollateral(): cancels the provided U with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
		// --- SETUP ---
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(310, 16)),
			extraUAmount: dec(10, 18),
			extraParams: { from: alice },
		})
		const { netDebt: B_netDebt } = await openTrove({
			ICR: toBN(dec(290, 16)),
			extraUAmount: dec(8, 18),
			extraParams: { from: bob },
		})
		...
		const redemptionAmount_Asset =
			C_netDebt_Asset.add(B_netDebt_Asset).add(partialRedemptionAmount)
		// start Dennis with a high ICR
		await openTrove({
			ICR: toBN(dec(100, 18)),
			extraUAmount: redemptionAmount,
			extraParams: { from: dennis },
		})
		await openTrove({
			asset: erc20.address,
		...
		const dennis_UBalance_Before = await UToken.balanceOf(dennis)
		...
		/* check that Dennis' redeemed 20 U has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) U debt + 50 for gas compensation. */
		th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount))
		th.assertIsApproximatelyEqual(
			alice_debt_After_Asset,
			A_totalDebt_Asset.sub(partialRedemptionAmount)
		)
		...
		const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount U to ETH, at ETH:USD price 200
		...
		th.assertIsApproximatelyEqual(
			expectedReceivedETH_Asset.div(toBN(10 ** 10)),
			receivedETH_Asset
		)

		const dennis_UBalance_After = (await UToken.balanceOf(dennis)).toString()
		assert.equal(
			dennis_UBalance_After,
			dennis_UBalance_Before.sub(redemptionAmount).sub(redemptionAmount_Asset)
		)
	})
    ...
    It leaves her with (3) U debt + 50 for gas compensation. */
		...
		const expectedTotalETHDrawn_Asset = redemptionAmount_Asset.div(toBN(200)) // convert redemptionAmount U to ETH, at ETH:USD price 200
		const expectedReceivedETH_Asset = expectedTotalETHDrawn_Asset.sub(toBN(ETHFee_Asset))
	...
	it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
		...
		// open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 U), 20000%
		const { UAmount: F_UAmount } = await openTrove({
			ICR: toBN(dec(200, 18)),
			extraUAmount: redemptionAmount.mul(toBN(2)),
			extraParams: { from: flyn },
		})
		const { UAmount: F_UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 18)),
			extraUAmount: redemptionAmount.mul(toBN(2)),
			extraParams: { from: flyn },
		...
		// Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 U
		const flynBalance = await UToken.balanceOf(flyn)
		th.assertIsApproximatelyEqual(
			flynBalance,
			F_UAmount.add(F_UAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
		)
	...
	it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
		...
		// Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 U
		const flynBalance = (await UToken.balanceOf(flyn)).toString()
		th.assertIsApproximatelyEqual(
			flynBalance,
			F_UAmount.add(F_UAmount_Asset).sub(redemptionAmount).sub(redemptionAmount_Asset)
		)
	...
	it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveUAmount(dec(20000, 18), ZERO_ADDRESS),
			B,
			B,
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveUAmount(dec(30000, 18), ZERO_ADDRESS),
			C,
			C,
		...
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveUAmount(dec(10000, 18), erc20.address),
			A,
			A,
			{ from: A }
		)
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveUAmount(dec(20000, 18), erc20.address),
			B,
			B,
			{ from: B }
		)
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveUAmount(dec(30000, 18), erc20.address),
			C,
			C,
			{ from: C }
		)
		...
		await UToken.transfer(B, await UToken.balanceOf(A), { from: A })
		await UToken.transfer(B, await UToken.balanceOf(C), { from: C })
		...
		// U redemption is 55000 US
		const URedemption = dec(55000, 18)
		await th.redeemCollateralAndGetTxObject(B, contracts, URedemption, ZERO_ADDRESS)
		await th.redeemCollateralAndGetTxObject(B, contracts, URedemption, erc20.address)
	...
	it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveUAmount(dec(6000, 18), ZERO_ADDRESS),
			A,
			A,
			{ from: A, value: dec(1000, "ether") }
		)
		...
		await borrowerOperations.openTrove(
			...
			dec(1000, "ether"),
			th._100pct,
			await getOpenTroveUAmount(dec(6000, 18), erc20.address),
			A,
			A,
			{ from: A }
		)
	...
	it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
		...
		const redemptionAmount_Asset =
			fullfilledRedemptionAmount_Asset.add(partialRedemptionAmount)
		...
		// Oops, another transaction gets in the way
		{
			...
			// Alice redeems 1 U from Carol's Trove
			await troveManager.redeemCollateral(
				ZERO_ADDRESS,
				frontRunRedepmtion,
				firstRedemptionHint,
		...
		// Since Alice already redeemed 1 U from Carol's Trove, Dennis was  able to redeem:
		//  - 9 U from Carol's
		//  - 8 U from Bob's
		// for a total of 17 U.

		// Dennis calculated his hint for redeeming 2 U from Alice's Trove, but after Alice's transaction
		// got in the way, he would have needed to redeem 3 U to fully complete his redemption of 20 U.
		...
		const expectedTotalETHDrawn = fullfilledRedemptionAmount
			...
			.div(toBN(200)) // redempted U converted to ETH, at ETH:USD price 200
	...
	it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
		...
		const amount = await getOpenTroveUAmount(dec(110, 18))
		...
		await UToken.transfer(carol, amount.mul(toBN(2)), { from: bob })
		...
		const expectedTotalETHDrawn = toBN(amount).div(toBN(100)) // convert 100 U to ETH at ETH:USD price of 100
		const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee)
		...
		const carol_UBalance_After = (await UToken.balanceOf(carol)).toString()
		assert.equal(carol_UBalance_After, "0")
	})
	it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
		...
		const { UAmount: B_UAmount, totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(133, 16)),
			extraUAmount: A_debt,
			extraParams: { from: bob },
		})
		...
		const { UAmount: B_UAmount_Asset, totalDebt: B_totalDebt_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(133, 16)),
			extraUAmount: A_debt,
			extraParams: { from: bob },
		...
		await UToken.transfer(carol, B_UAmount.add(B_UAmount_Asset), { from: bob })
	...
	it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
		...
		const { totalDebt: D_totalDebt_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(195, 16)),
			extraUAmount: redemptionAmount_Asset,
			extraParams: { from: dennis },
		})
	...
	it("redeemCollateral(): reverts when argument _amount is 0", async () => {
		...
		// Alice opens trove and transfers 500U to Erin, the would-be redeemer
		await openTrove({
			ICR: toBN(dec(200, 16)),
			extraUAmount: dec(500, 18),
			extraParams: { from: alice },
		})
		await openTrove({
			asset: erc20.address,
		...
		await UToken.transfer(erin, dec(500, 18), { from: alice })
	...
	it("redeemCollateral(): reverts if max fee > 100%", async () => {
		...
		await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(30, 18),
			extraParams: { from: C },
		})
		await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(40, 18),
			extraParams: { from: D },
		})
	...
	it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(80, 18),
			extraParams: { from: A },
		})
		const { totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(90, 18),
			extraParams: { from: B },
		})
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(100, 18),
			extraParams: { from: C },
		})
		...
		// Check total U supply
		const totalSupply = await UToken.totalSupply()
		th.assertIsApproximatelyEqual(
			totalSupply,
			expectedTotalSupply.add(expectedTotalSupply_Asset)
		)
		...
		// U redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
		const attemptedURedemption = expectedTotalSupply.div(toBN(10))
		const attemptedURedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))
	...
	it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(9500, 18),
			extraParams: { from: A },
		})
		const { totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(395, 16)),
			extraUAmount: dec(9000, 18),
			extraParams: { from: B },
		})
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(390, 16)),
			extraUAmount: dec(10000, 18),
			extraParams: { from: C },
		})
		...
		// U redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
		const attemptedURedemption = expectedTotalSupply.div(toBN(10))
		const attemptedURedemption_Asset = expectedTotalSupply_Asset.div(toBN(10))

		// Attempt with maxFee > 5.5%
		const price = await priceFeed.getPrice()
		const ETHDrawn = attemptedURedemption.mul(mv._1e18BN).div(price)
		const ETHDrawn_Asset = attemptedURedemption_Asset.mul(mv._1e18BN).div(price)

		const slightlyMoreThanFee = await troveManager.getRedemptionFeeWithDecay(
			ZERO_ADDRESS,
			ETHDrawn
		)
		const slightlyMoreThanFee_Asset = await troveManager.getRedemptionFeeWithDecay(
			erc20.address,
			ETHDrawn
		)
		...
		const exactSameFee_Asset = await troveManager.getRedemptionFeeWithDecay(
			erc20.address,
			ETHDrawn_Asset
		)
	...
	it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
		...
		const { totalDebt: C_totalDebt } = await openTrove({
			ICR: toBN(dec(195, 16)),
			extraUAmount: dec(200, 18),
			extraParams: { from: carol },
		})
		const { totalDebt: D_totalDebt } = await openTrove({
			ICR: toBN(dec(190, 16)),
			extraUAmount: dec(400, 18),
			extraParams: { from: dennis },
		})
		...
		// Alice opens trove and transfers U to Erin, the would-be redeemer
		await openTrove({
			ICR: toBN(dec(300, 16)),
			extraUAmount: redemptionAmount,
			extraParams: { from: alice },
		...
		await UToken.transfer(erin, redemptionAmount.add(redemptionAmount_Asset), {
			from: alice,
		})
		...
		const dennis_ICR_before_Asset = await troveManager.getCurrentICR(
			erc20.address,
			dennis,
			price
		)
		...
		const bob_SPDeposit_before = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
		const carol_SPDeposit_before = (
			await stabilityPool.getCompoundedUDeposit(carol)
		).toString()
		...
		const carol_SPDeposit_before_Asset = (
			await stabilityPoolERC20.getCompoundedUDeposit(carol)
		).toString()
		...
		const carol_ETHGain_before_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(carol)
		).toString()
		const dennis_ETHGain_before_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(dennis)
		).toString()

		// Check the remaining U and ETH in Stability Pool after liquidation is non-zero
		const UinSP = await stabilityPool.getTotalUDeposits()
		const ETHinSP = await stabilityPool.getAssetBalance()

		const UinSP_Asset = await stabilityPoolERC20.getTotalUDeposits()
		const ETHinSP_Asset = await stabilityPoolERC20.getAssetBalance()

		assert.isTrue(UinSP.gte(mv._zeroBN))
		assert.isTrue(ETHinSP.gte(mv._zeroBN))

		assert.isTrue(UinSP_Asset.gte(mv._zeroBN))
		...
		const bob_ICR_after_Asset = await troveManager.getCurrentICR(erc20.address, bob, price)
		const carol_ICR_after_Asset = await troveManager.getCurrentICR(erc20.address, carol, price)
		const dennis_ICR_after_Asset = await troveManager.getCurrentICR(
			erc20.address,
			dennis,
			price
		)
		...
		const bob_SPDeposit_after = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
		const carol_SPDeposit_after = (await stabilityPool.getCompoundedUDeposit(carol)).toString()
	...
	it("redeemCollateral(): caller can redeem their entire UToken balance", async () => {
		...
		// Alice opens trove and transfers 400 U to Erin, the would-be redeemer
		const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(300, 16)),
			extraUAmount: dec(400, 18),
			extraParams: { from: alice },
		...
		await UToken.transfer(erin, toBN(dec(400, 18)).mul(toBN(2)), { from: alice })

		// Check Erin's balance before
		const erin_balance_before = await UToken.balanceOf(erin)
		...
		// Check activePool debt reduced by  400 U
		const activePool_debt_after = await activePool.getUDebt(ZERO_ADDRESS)
		assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

		const activePool_debt_after_Asset = await activePool.getUDebt(erc20.address)
    ...
    */
		...
		assert.equal(
			activePool_coll_after_Asset.toString(),
			activePool_coll_before_Asset.sub(toBN(dec(2, 18)))
		)

		// Check Erin's balance after
		const erin_balance_after = (await UToken.balanceOf(erin)).toString()
		assert.equal(erin_balance_after, "0")
	})

	it("redeemCollateral(): reverts when requested redemption amount exceeds caller's U token balance", async () => {
		const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
			ICR: toBN(dec(20, 18)),
			extraParams: { from: whale },
		})
		...
		try {
			...
			assert.include(
				error.message,
				"Requested redemption amount must be <= user's U token balance"
			)
		}
		...
		// Erin tries to redeem 239482309 U
		try {
			;({ firstRedemptionHint, partialRedemptionHintNICR } =
		...
		// Erin tries to redeem 2^256 - 1 U
	...
	it("redeemCollateral(): value of issued ETH == face value of redeemed U (assuming 1 U has value of $1)", async () => {
		...
		// Alice opens trove and transfers 1000 U each to Erin, Flyn, Graham
		const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(400, 16)),
			extraUAmount: dec(4990, 18),
			extraParams: { from: alice },
		})
		const { collateral: A_coll_Asset, totalDebt: A_totalDebt_Asset } = await openTrove({
			asset: erc20.address,
		...
		await UToken.transfer(erin, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
		await UToken.transfer(flyn, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })
		await UToken.transfer(graham, toBN(dec(1000, 18)).mul(toBN(2)), { from: alice })

		// B, C, D open trove
		const { collateral: B_coll } = await openTrove({
			ICR: toBN(dec(300, 16)),
			extraUAmount: dec(1590, 18),
			extraParams: { from: bob },
		})
		const { collateral: C_coll } = await openTrove({
			ICR: toBN(dec(600, 16)),
			extraUAmount: dec(1090, 18),
			extraParams: { from: carol },
		})
		...
		const _120_U = "120000000000000000000"
		const _373_U = "373000000000000000000"
		const _950_U = "950000000000000000000"
		...
		let partialRedemptionHintNICR_Asset
			...
			// Erin redeems 120 U
		;({ firstRedemptionHint, partialRedemptionHintNICR } =
			await hintHelpers.getRedemptionHints(ZERO_ADDRESS, _120_U, price, 0))
		...
		await ({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
			await hintHelpers.getRedemptionHints(erc20.address, _120_U, price, 0))
		...
		/* 120 U redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200,
    ...
    Total active ETH = 280 - 0.6 = 279.4 ETH */
		...
		assert.equal(
			...
			activeETH_0.sub(toBN(_120_U).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_1_Asset.toString(),
			activeETH_0_Asset.sub(toBN(_120_U).mul(mv._1e18BN).div(price))
		)

		// Flyn redeems 373 U
		;({ firstRedemptionHint, partialRedemptionHintNICR } =
			await hintHelpers.getRedemptionHints(ZERO_ADDRESS, _373_U, price, 0))
		...
		;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
			await hintHelpers.getRedemptionHints(erc20.address, _373_U, price, 0))
		...
		/* 373 U redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200,
    ...
    Total active ETH = 279.4 - 1.865 = 277.535 ETH */
		...
		assert.equal(
			...
			activeETH_1.sub(toBN(_373_U).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_2_Asset.toString(),
			activeETH_1_Asset.sub(toBN(_373_U).mul(mv._1e18BN).div(price))
		)

		// Graham redeems 950 U
		;({ firstRedemptionHint, partialRedemptionHintNICR } =
			await hintHelpers.getRedemptionHints(ZERO_ADDRESS, _950_U, price, 0))
		...
		;({ 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
			await hintHelpers.getRedemptionHints(erc20.address, _950_U, price, 0))
		...
		/* 950 U redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200,
    ...
    Total active ETH = 277.535 - 4.75 = 272.785 ETH */
		...
		assert.equal(
			...
			activeETH_2.sub(toBN(_950_U).mul(mv._1e18BN).div(price))
		)
		assert.equal(
			activeETH_3_Asset.toString(),
			activeETH_2_Asset.sub(toBN(_950_U).mul(mv._1e18BN).div(price))
		)
	})
	...
	it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
		// --- SETUP --- illegally mint U to Bob
		await UToken.unprotectedMint(bob, toBN(dec(100, 18)).mul(toBN(2)))

		assert.equal(await UToken.balanceOf(bob), toBN(dec(100, 18)).mul(toBN(2)).toString())
		...
		// Bob tries to redeem his illegally obtained U
		try {
			await troveManager.redeemCollateral(
				ZERO_ADDRESS,
				dec(100, 18),
	...
	it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
		// --- SETUP --- illegally mint U to Bob
		await UToken.unprotectedMint(bob, "202000000000000000000")

		assert.equal(await UToken.balanceOf(bob), "202000000000000000000")
		...
		th.assertIsApproximatelyEqual(
			(await activePool.getUDebt(ZERO_ADDRESS)).toString(),
			totalDebt
		)
		th.assertIsApproximatelyEqual(
			(await activePool.getUDebt(erc20.address)).toString(),
			totalDebt_Asset
		)
		...
		// Bob attempts to redeem his ill-gotten 101 U, from a system that has 100 U outstanding debt
		try {
			const redemptionTx = await troveManager.redeemCollateral(
				ZERO_ADDRESS,
	...
	it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
		...
		// Check A's balance has decreased by 10 U
		assert.equal(
			await UToken.balanceOf(A),
			A_balanceBefore.sub(toBN(dec(10, 18)).mul(toBN(2))).toString()
		)
	...
	it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
		...
		const A_balanceBefore = await UToken.balanceOf(A)
		const B_balanceBefore = await UToken.balanceOf(B)
		...
		// Check B's balance has decreased by 10 U
		assert.equal(
			await UToken.balanceOf(B),
	...
	it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
		...
		// Check A's balance has decreased by 10 U
		assert.equal(
			A_balanceBefore.sub(await UToken.balanceOf(A)).toString(),
	...
	it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to YOU staking contract", async () => {
		...
		assert.isTrue(YOUStakingBalance_After_Asset.gt(YOUStakingBalance_Before_Asset))
	})
	...
	it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
		...
		// Check total U supply
		const activeU = await activePool.getUDebt(ZERO_ADDRESS)
		const defaultU = await defaultPool.getUDebt(ZERO_ADDRESS)

		const activeU_Asset = await activePool.getUDebt(erc20.address)
		const defaultU_Asset = await defaultPool.getUDebt(erc20.address)

		const totalUSupply = activeU.add(defaultU)
		const totalUSupply_Asset = activeU_Asset.add(defaultU_Asset)
		th.assertIsApproximatelyEqual(totalUSupply, totalDebt)
		th.assertIsApproximatelyEqual(totalUSupply_Asset, totalDebt_Asset)
    ...
    */
	...
	it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
		...
		// whale redeems 360 U.  Expect this to fully redeem A, B, C, and partially redeem D.
	...
	const redeemCollateral3Full1Partial = async () => {
		...
		// whale redeems U.  Expect this to fully redeem A, B, C, and partially redeem D.
		await th.redeemCollateral(whale, contracts, redemptionAmount, ZERO_ADDRESS)
    ...
    */
	...
	it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
		...
		// whale redeems U.  Expect this to fully redeem A, B, C, and partially redeem 15 U from D.
	...
	it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
		...
		const { UAmount: UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraUAmount: dec(1, 24),
			extraParams: { from: alice },
		...
		for (let i = 0; i < 2; i++) {
			...
			const { firstRedemptionHint, partialRedemptionHintNICR } =
				await hintHelpers.getRedemptionHints(ZERO_ADDRESS, UAmount, price, 0)
			...
			await borrowerOperations.adjustTrove(
				...
				alice,
				alice,
				{ from: alice, value: UAmount.mul(mv._1e18BN).div(price) }
			)
		}
		...
		for (let i = 0; i < 2; i++) {
			// Find hints for redeeming
			const { 0: firstRedemptionHint_Asset, 1: partialRedemptionHintNICR_Asset } =
				await hintHelpers.getRedemptionHints(erc20.address, UAmount_Asset, price, 0)
			...
			await borrowerOperations.adjustTrove(
				erc20.address,
				UAmount_Asset.mul(mv._1e18BN).div(price),
				th._100pct,
				0,
				UAmount_Asset,
				true,
	...
	it("getPendingUDebtReward(): Returns 0 if there is no pending UDebt reward", async () => {
		// Make some troves
		...
		const current_L_UDebt = await troveManager.L_UDebts(ZERO_ADDRESS)
		assert.equal(current_L_UDebt, 0)

		const current_L_UDebt_Asset = await troveManager.L_UDebts(erc20.address)
		assert.equal(current_L_UDebt_Asset, 0)

		const carolSnapshot_L_UDebt = (await troveManager.rewardSnapshots(carol, ZERO_ADDRESS))[1]
		assert.equal(carolSnapshot_L_UDebt, 0)

		const carolSnapshot_L_UDebt_Asset = (
			await troveManager.rewardSnapshots(carol, erc20.address)
		)[1]
		assert.equal(carolSnapshot_L_UDebt_Asset, 0)

		const carol_PendingUDebtReward = await troveManager.getPendingUDebtReward(
			ZERO_ADDRESS,
			carol
		)
		assert.equal(carol_PendingUDebtReward, 0)

		const carol_PendingUDebtReward_Asset = await troveManager.getPendingUDebtReward(
			erc20.address,
			carol
		)
		assert.equal(carol_PendingUDebtReward_Asset, 0)
	})
	...
	it("getPendingETHReward(): Returns 0 if there is no pending ETH reward", async () => {
		...
		const carol_PendingETHReward = await troveManager.getPendingAssetReward(
			ZERO_ADDRESS,
			carol
		)
		...
		const carol_PendingETHReward_Asset = await troveManager.getPendingAssetReward(
			erc20.address,
			carol
		)
	...
	it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 U", async () => {
		const price = dec(100, 18)
		const coll = dec(1, "ether")
		const debt = dec(100, 18)
	...
	it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 U", async () => {
		const price = dec(100, 18)
	...
	it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 U", async () => {
	...
	it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 U", async () => {
		const price = dec(100, 18)
		const coll = dec(1, "ether")
	...
	it("getTroveStatus(): Returns status", async () => {
		...
		// to be able to repay:
		await UToken.transfer(B, B_totalDebt, { from: A })
		await UToken.transfer(B, B_totalDebt_Asset, { from: A })
