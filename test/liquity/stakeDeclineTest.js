...
contract("TroveManager", async accounts => {
	...
	it("A given trove's stake decline is negligible with adjustments and tiny liquidations", async () => {
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveUAmount(dec(1, 31), ZERO_ADDRESS),
			ZERO_ADDRESS,
			ZERO_ADDRESS,
		...
		await borrowerOperations.openTrove(
			...
			0,
			th._100pct,
			await getOpenTroveUAmount(dec(2, 30), ZERO_ADDRESS),
			ZERO_ADDRESS,
			ZERO_ADDRESS,
		...
		for (account of tinyTroves) {
			await borrowerOperations.openTrove(
				...
				0,
				th._100pct,
				await getOpenTroveUAmount(dec(1, 22), ZERO_ADDRESS),
				ZERO_ADDRESS,
				ZERO_ADDRESS,
		...
		assert.isTrue(
			await troveManager.checkRecoveryMode(ZERO_ADDRESS, await priceFeed.getPrice())
		)
		await troveManager.liquidate(ZERO_ADDRESS, A)

		console.log(
			`totalStakesSnapshot after L1: ${await troveManager.totalStakesSnapshot(ZERO_ADDRESS)}`
		)
		console.log(
		...
		console.log(
			`B pending ETH reward after L1: ${await troveManager.getPendingAssetReward(
				ZERO_ADDRESS,
				B
			)}`
		)
		console.log(
			`B stake after L1: ${(await troveManager.Troves(B, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX]}`
		)
		...
		console.log(
			`B stake after A1: ${(await troveManager.Troves(B, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX]}`
		)
		...
		for (let [idx, trove] of tinyTroves.entries()) {
			...
			console.log(
				`B stake after L${idx + 2}: ${
					(await troveManager.Troves(B, ZERO_ADDRESS))[th.TROVE_STAKE_INDEX]
				}`
			)
			console.log(
				`Snapshots ratio after L${idx + 2}: ${await getSnapshotsRatio(ZERO_ADDRESS)}`
			)
			await borrowerOperations.adjustTrove(
				ZERO_ADDRESS,
				0,
