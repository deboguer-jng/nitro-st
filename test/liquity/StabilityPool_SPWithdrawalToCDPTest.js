...
contract(
	...
	async accounts => {
		...
		describe("Stability Pool Withdrawal", async () => {
			...
			it("withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(100000, 18), ZERO_ADDRESS),
					whale,
					whale,
				...
				await borrowerOperations.openTrove(
					...
					dec(100000, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(100000, 18), erc20.address),
					whale,
					whale,
					{ from: whale }
				)
				...
				// Whale transfers 10k U to A, B and C who then deposit it to the SP
				const depositors = [alice, bob, carol]
				for (account of depositors) {
					await UToken.transfer(account, dec(20000, 18), { from: whale })
					await stabilityPool.provideToSP(dec(10000, 18), { from: account })
					await stabilityPoolERC20.provideToSP(dec(10000, 18), { from: account })
				}

				// Defaulter opens trove with 200% ICR and 10k U net debt
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
				...
				// Check depositors' compounded deposit is 6666.66 U and ETH Gain is 33.16 ETH
				...
				const txAERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: alice }
				)
				const txBERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: bob }
				)
				const txCERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: carol }
				)
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(alice)).toString(),
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(bob)).toString(),
						"6666666666666666666666"
					),
					10000
				)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(carol)).toString(),
						"6666666666666666666666"
					),
					10000
				)
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(alice)).toString(),
						"6666666666666666666666"
					),
					10000
				)
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(carol)).toString(),
						"6666666666666666666666"
					),
					10000
				)
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "33166666666666666667"),
					10000
				)
				assert.isAtMost(th.getDifference(bob_ETHWithdrawnERC20, "33166666666666666667"), 10000)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "33166666666666666667"),
					10000
				)
			})
			...
			it("withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
				...
				// Check depositors' compounded deposit is 3333.33 U and ETH Gain is 66.33 ETH
				const txA = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
					from: alice,
				})
				const txB = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "66333333333333333333"),
					10000
				)
				assert.isAtMost(th.getDifference(bob_ETHWithdrawnERC20, "66333333333333333333"), 10000)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "66333333333333333333"),
					10000
				)
			})
			...
			it("withdrawAssetGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
				...
				// Check depositors' compounded deposit is 0 U and ETH Gain is 99.5 ETH
				const txA = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
					from: alice,
				})
				const txB = await stabilityPool.withdrawAssetGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS, {
				...
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(alice)).toString(), "0"),
					10000
				)
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(bob)).toString(), "0"),
					10000
				)
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(carol)).toString(), "0"),
					10000
				)
			...
			it("withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing U", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(5000, 18)),
					defaulter_1,
					defaulter_1,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(7000, 18)),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					th._100pct,
					await getOpenTroveUAmount(dec(5000, 18), erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				await borrowerOperations.openTrove(
					...
					th._100pct,
					await getOpenTroveUAmount(dec(7000, 18), erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
			...
			it("withdrawAssetGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing U", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(6000, 18)),
					defaulter_2,
					defaulter_2,
			...
			it("withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
				...
				// Whale transfers 10k, 20k, 30k U to A, B and C respectively who then deposit it to the SP
				await UToken.transfer(alice, dec(10000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
				await UToken.transfer(bob, dec(20000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(20000, 18), { from: bob })
				await UToken.transfer(carol, dec(30000, 18), { from: whale })
				...
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "66333333333333333333"),
					100000
				)
			...
			it("withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "49750000000000000000"),
					100000
				)
				assert.isAtMost(th.getDifference(bob_ETHWithdrawnERC20, dec(995, 17)), 100000)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "149250000000000000000"),
					100000
				)
			})
			...
			it("withdrawAssetGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(1000000, 18), ZERO_ADDRESS),
					whale,
					whale,
				...
				await borrowerOperations.openTrove(
					...
					dec(1000000, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(1000000, 18), erc20.address),
					whale,
					whale,
					{ from: whale }
				)
      ...
      Bob:  456000 U
      Carol: 13100 U */
				// Whale transfers U to  A, B and C respectively who then deposit it to the SP
				await UToken.transfer(alice, dec(2000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(2000, 18), { from: alice })
				await UToken.transfer(bob, dec(456000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(456000, 18), { from: bob })
				await UToken.transfer(carol, dec(13100, 18), { from: whale })
      ...
      Defaulter 1: 207000 U & 2160 ETH
      Defaulter 2: 5000 U & 50 ETH
      Defaulter 3: 46700 U & 500 ETH
      */
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount("207000000000000000000000"),
					defaulter_1,
					defaulter_1,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(5, 21)),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("46700000000000000000000"),
					defaulter_3,
					defaulter_3,
				...
				await borrowerOperations.openTrove(
					...
					dec(2160, 18),
					th._100pct,
					await getOpenTroveUAmount("207000000000000000000000", erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				await borrowerOperations.openTrove(
					...
					dec(50, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(5, 21), erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
				await borrowerOperations.openTrove(
					...
					dec(500, "ether"),
					th._100pct,
					await getOpenTroveUAmount("46700000000000000000000", erc20.address),
					defaulter_3,
					defaulter_3,
					{ from: defaulter_3 }
				)
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "11447463383570366500"),
					10000000000
				)
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "2610021651454043834000"),
					10000000000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "74980885162385912900"),
					10000000000
				)
			})
			...
			it("withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				await UToken.transfer(dennis, dec(20000, 18), { from: whale })
				...
				const txDERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: dennis }
				)
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "82916666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "82916666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "82916666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "49750000000000000000"),
					100000
				)
			})

			it("withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawAssetGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 1m ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
      ...
      */
				// Whale transfers U to  A, B and C respectively who then deposit it to the SP
				await UToken.transfer(alice, dec(60000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(60000, 18), { from: alice })
				await UToken.transfer(bob, dec(20000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(20000, 18), { from: bob })
				await UToken.transfer(carol, dec(15000, 18), { from: whale })
				...
				/* Defaulters open troves:
      Defaulter 1:  10000 U, 100 ETH
      Defaulter 2:  25000 U, 250 ETH
      Defaulter 3:  5000 U, 50 ETH
      Defaulter 4:  40000 U, 400 ETH
      */
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(25000, 18), ZERO_ADDRESS),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(5000, 18), ZERO_ADDRESS),
					defaulter_3,
					defaulter_3,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(40000, 18), ZERO_ADDRESS),
					defaulter_4,
					defaulter_4,
				...
				await borrowerOperations.openTrove(
					...
					th._100pct,
					await getOpenTroveUAmount(dec(25000, 18), erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
				...
				await borrowerOperations.openTrove(
					...
					dec(400, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(40000, 18), erc20.address),
					defaulter_4,
					defaulter_4,
					{ from: defaulter_4 }
				)
				...
				// Dennis provides 25000 U
				await UToken.transfer(dennis, dec(25000, 18), { from: whale })
				await UToken.transfer(dennis, dec(25000, 18), { from: whale })
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "419563467492260055900"),
					100000000000
				)
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "139854489164086692700"),
					100000000000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "104890866873065014000"),
					100000000000
				)
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "131691176470588233700"),
					100000000000
				)
			})
			...
			it("withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawAssetGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
      ...
      Dennis: 40000 U
      */
				// Whale transfers U to  A, B,C and D respectively who then deposit it to the SP
				await UToken.transfer(alice, dec(20000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(20000, 18), { from: alice })
				await UToken.transfer(bob, dec(25000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(25000, 18), { from: bob })
				await UToken.transfer(carol, dec(12500, 18), { from: whale })
				await stabilityPool.provideToSP(dec(12500, 18), { from: carol })
				await UToken.transfer(dennis, dec(40000, 18), { from: whale })
      ...
      */
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(20000, 18)),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(30000, 18)),
					defaulter_3,
					defaulter_3,
				...
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(dennis)).toString(),
				...
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "12246153846"),
					100000000000
				)
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "182361204013377919900"),
					100000000000
				)
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "227951505016722411000"),
					100000000000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "113975752508361205500"),
					100000000000
				)
			})

			// --- One deposit enters at t > 0, and another leaves later ---
			it("withdrawAssetGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 U. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				// Whale transfers 10k U to A, B and D who then deposit it to the SP
				...
				// Carol makes deposit
				await UToken.transfer(carol, dec(10000, 18), { from: whale })
				...
				assert.isAtMost(
					th.getDifference(alice_ETHWithdrawnERC20, "92866666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "92866666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "79600000000000000000"),
					100000
				)
			})
			...
			// A, B withdraw 0U & 100e
			// C, D withdraw 5000U  & 500e
			it("withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
				...
				// Whale transfers 10k U to A, B who then deposit it to the SP
				...
				// Defaulter 1 liquidated. 20000 U fully offset with pool.
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_1, { from: owner })

				// Carol, Dennis each deposit 10000 U
				...
				// Defaulter 2 liquidated. 10000 U offset
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
				...
				// Expect Alice And Bob's compounded deposit to be 0 U
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(alice)).toString(), "0"),
					10000
				)
				...
				// Expect Carol And Dennis' compounded deposit to be 50 U
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(carol)).toString(),
						"5000000000000000000000"
				...
				assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, "49750000000000000000"), 100000)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "49750000000000000000"),
					100000
				)
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "49750000000000000000"),
			...
			it("withdrawAssetGainToTrove(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
				...
				// Defaulter 1 liquidated. 10--0 U fully offset, Pool remains non-zero
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_1, { from: owner })
				...
				// Defaulter 2 liquidated. 1--00 U, empties pool
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
				...
				// Defaulter 3 liquidated. 10000 U fully offset, Pool remains non-zero
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_3, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_3, { from: owner })
				...
				// Defaulter 4 liquidated. 10000 U, empties pool
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_4, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_4, { from: owner })
			...
			// A, B withdraw 0 U & 100e
			// C, D withdraw 5000 U  & 50e
			it("withdrawAssetGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
				...
				// Carol, Dennis, Erin each deposit 10000, 20000, 30000 U respectively
				await UToken.transfer(carol, dec(10000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(10000, 18), { from: carol })
				await UToken.transfer(dennis, dec(20000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(20000, 18), { from: dennis })
				await UToken.transfer(erin, dec(30000, 18), { from: whale })
				...
				const txEERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: erin }
				)
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(erin)).toString(),
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(erin)).toString(),
						"25000000000000000000000"
					),
					100000
				)
				...
				assert.isAtMost(th.getDifference(bob_ETHWithdrawnERC20, dec(995, 17)), 100000)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "16583333333333333333"),
					100000
				)
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "33166666666666666667"),
					100000
				)
				assert.isAtMost(
					th.getDifference(erin_ETHWithdrawnERC20, "49750000000000000000"),
					100000
				)
			})

			// A deposits 10000
			// L1, L2, L3 liquidated with 10000 U each
			// A withdraws all
			...
			it("withdrawAssetGainToTrove(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation", async () => {
				...
				// Defaulter 1,2,3 withdraw 10000 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
			...
			// A,B deposit 10000 U
			// L1 cancels 20000 U, 2E
			// B,C deposits 10000 U
			// L2 cancels 20000 U, 2E
			// E,F deposit 10000 U
			// L3 cancels 20000, 200E
			// G,H deposits 10000
			// L4 cancels 20000, 200E

			// Expect all depositors withdraw 0 U and 100 ETH
			...
			it("withdrawAssetGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
				...
				// Erin, Flyn each deposit 10000 U
				...
				// Defaulter 3 liquidated. 10000 U offset
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_3, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_3, { from: owner })

				// Graham, Harriet each deposit 10000 U
				...
				// Defaulter 4 liquidated. 10k U offset
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_4, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_4, { from: owner })
				...
				const txFERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: flyn }
				)
				const txGERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: graham }
				)
				const txHERC20 = await stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: harriet }
				)
				...
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(erin)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference((await stabilityPool.getCompoundedUDeposit(flyn)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(graham)).toString(),
						"0"
					),
					100000
				)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(harriet)).toString(),
						"0"
					),
					100000
				)
				...
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(flyn)).toString(),
						"0"
					),
					100000
				)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(graham)).toString(),
						"0"
					),
					100000
				)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPoolERC20.getCompoundedUDeposit(harriet)).toString(),
						"0"
					),
					100000
				)
      ...
      should only earn ETH from the single liquidation that cancelled with their deposit */
			...
			// A withdraws all
			// B deposits 10000
			// L2 of 9900 U, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
			...
			it("withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				// Defaulter 1 withdraws 'almost' 10000 U:  9999.99991 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount("9999999910000000000000", ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					dec(100, "ether"),
					th._100pct,
					await getOpenTroveUAmount("9999999910000000000000", erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				...
				// Defaulter 2 withdraws 9900 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(9900, 18)),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					dec(60, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(9900, 18), erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
				...
				// Defaulter 2 liquidated.  9900 U liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
				await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
				await troveManager.liquidate(erc20.address, defaulter_2, { from: owner })
				...
				// Expect Bob to retain 1% of initial deposit (100 U) and all the liquidated ETH (60 ether)
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(bob)).toString(),
				...
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "59700000000000000000"),
					100000
				)
			})

			// A deposits 10000
			// L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 U
			// A withdraws all
			...
			it("withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				// Defaulter 1 withdraws 'almost' 10k U.
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
				...
				// Defaulter 2 withdraws 59400 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount("59400000000000000000000", ZERO_ADDRESS),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					dec(330, "ether"),
					th._100pct,
					await getOpenTroveUAmount("59400000000000000000000", erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
      ...
      Bob:  1000 U, 55 Ether
      Carol:  2000 U, 110 Ether
      Dennis:  3000 U, 165 Ether
     
      Total: 6000 U, 300 Ether
      */
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(bob)).toString(),
				...
				assert.isAtMost(
					th.getDifference(bob_ETHWithdrawnERC20, "54725000000000000000"),
					100000
				)
				assert.isAtMost(
					th.getDifference(carol_ETHWithdrawnERC20, "109450000000000000000"),
					100000
				)
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "164175000000000000000"),
					100000
				)
			})
			...
			// A make deposit 10000 U
			// L1 brings P to 1e-5*P. L1:  9999.9000000000000000 U
			// A withdraws
			// B makes deposit 10000 U
			// L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 U total deposits)
			// B withdraws
			...
			it("withdrawAssetGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				// Defaulter 1 and default 2 each withdraw 9999.999999999 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(99999, 17), ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				...
				await borrowerOperations.openTrove(
					...
					dec(100, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(99999, 17), erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				...
				const txL1ERC20 = await troveManager.liquidate(erc20.address, defaulter_1, {
					from: owner,
				})
				...
				// Bob should withdraw 1e-5 of initial deposit: 0.1 U and the full ETH gain of 100 ether
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(bob)).toString(),
						dec(1, 17)
			...
			// L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 U total deposits)
			// B withdraws
			// expect d(B) = d0(B) * 1e-5
			// expect B gets entire ETH gain from L2
			it("withdrawAssetGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				// Defaulter 1 and default 2 withdraw up to debt of 9999.9 U and 59999.4 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount("9999900000000000000000", ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("59999400000000000000000", ZERO_ADDRESS),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					dec(100, "ether"),
					th._100pct,
					await getOpenTroveUAmount("9999900000000000000000", erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				await borrowerOperations.openTrove(
					...
					dec(600, "ether"),
					th._100pct,
					await getOpenTroveUAmount("59999400000000000000000", erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
				...
				// B, C, D deposit 10000, 20000, 30000 U
				await UToken.transfer(bob, dec(10000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(10000, 18), { from: bob })
				await UToken.transfer(carol, dec(20000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(20000, 18), { from: carol })
				await UToken.transfer(dennis, dec(30000, 18), { from: whale })
				await stabilityPool.provideToSP(dec(30000, 18), { from: dennis })
				...
				// {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} U
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(bob)).toString(),
						dec(1, 17)
				...
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, dec(2985, 17)),
					100000000000
				)
			})

			// A make deposit 10000 U
			// L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 U
			...
			it("withdrawAssetGainToTrove(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
				...
				// Defaulters 1 withdraws 9999.9999999 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount("9999999999900000000000", ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					dec(100, "ether"),
					th._100pct,
					await getOpenTroveUAmount("9999999999900000000000", erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				...
				assert.equal((await stabilityPool.getCompoundedUDeposit(alice)).toString(), 0)
				assert.equal((await stabilityPoolERC20.getCompoundedUDeposit(alice)).toString(), 0)
			})

			// --- Serial scale changes ---

			/* A make deposit 10000 U
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 U, 1 ETH
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 U, 1 ETH
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 U, 1 ETH
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 U, 1 ETH
    expect A, B, C, D each withdraw ~100 Ether
    */
			it("withdrawAssetGainToTrove(): Several deposits of 10000 U span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				// Defaulters 1-4 each withdraw 9999.9 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
				...
				// B deposits 9999.9 U
				await UToken.transfer(bob, dec(99999, 17), { from: whale })
				...
				// C deposits 9999.9 U
				await UToken.transfer(carol, dec(99999, 17), { from: whale })
				...
				const txL3ERC20 = await troveManager.liquidate(erc20.address, defaulter_3, {
					from: owner,
				})
				...
				// D deposits 9999.9 U
				await UToken.transfer(dennis, dec(99999, 17), { from: whale })
				...
				const txL4ERC20 = await troveManager.liquidate(erc20.address, defaulter_4, {
					from: owner,
				})
				...
				assert.equal(await stabilityPool.getCompoundedUDeposit(alice), "0")
				assert.equal(await stabilityPool.getCompoundedUDeposit(alice), "0")
				assert.equal(await stabilityPool.getCompoundedUDeposit(alice), "0")

				assert.equal(await stabilityPoolERC20.getCompoundedUDeposit(alice), "0")
				assert.equal(await stabilityPoolERC20.getCompoundedUDeposit(alice), "0")
				assert.equal(await stabilityPoolERC20.getCompoundedUDeposit(alice), "0")
				// D should retain around 0.9999 U, since his deposit of 9999.9 was reduced by a factor of 1e-5
				assert.isAtMost(
					th.getDifference(
						(await stabilityPool.getCompoundedUDeposit(dennis)).toString(),
						dec(99999, 12)
			...
			it("withdrawAssetGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
				...
				// Defaulters 1-3 each withdraw 24100, 24300, 24500 U (inc gas comp)
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24100, 18), ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(200, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24300, 18), ZERO_ADDRESS),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2, value: dec(200, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24500, 18), ZERO_ADDRESS),
					defaulter_3,
					defaulter_3,
					{ from: defaulter_3, value: dec(200, "ether") }
				)
				...
				await borrowerOperations.openTrove(
					...
					dec(200, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(24100, 18), erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				await borrowerOperations.openTrove(
					...
					dec(200, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(24300, 18), erc20.address),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2 }
				)
				await borrowerOperations.openTrove(
					...
					dec(200, "ether"),
					th._100pct,
					await getOpenTroveUAmount(dec(24500, 18), erc20.address),
					defaulter_3,
					defaulter_3,
					{ from: defaulter_3 }
				)
				...
				// A, B provide 10k U
				await UToken.transfer(A, dec(10000, 18), { from: whale })
				await UToken.transfer(B, dec(10000, 18), { from: whale })
				...
				const A_deposit = await stabilityPool.getCompoundedUDeposit(A)
				const B_deposit = await stabilityPool.getCompoundedUDeposit(B)
				assert.equal(A_deposit, "0")
				assert.equal(B_deposit, "0")

				const A_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(A)
				const B_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(B)
				assert.equal(A_depositERC20, "0")
				assert.equal(B_depositERC20, "0")

				// Check SP tracker is zero
				const UinSP_1 = await stabilityPool.getTotalUDeposits()
				assert.equal(UinSP_1, "0")

				const UinSP_1ERC20 = await stabilityPoolERC20.getTotalUDeposits()
				assert.equal(UinSP_1ERC20, "0")

				// Check SP U balance is zero
				const SPUBalance_1 = await UToken.balanceOf(stabilityPool.address)
				assert.equal(SPUBalance_1, "0")

				const SPUBalance_1ERC20 = await UToken.balanceOf(stabilityPoolERC20.address)
				assert.equal(SPUBalance_1ERC20, "0")
				...
				// C, D provide 10k U
				await UToken.transfer(C, dec(10000, 18), { from: whale })
				await UToken.transfer(D, dec(10000, 18), { from: whale })
				...
				// Check compounded deposits
				const C_deposit = await stabilityPool.getCompoundedUDeposit(C)
				const D_deposit = await stabilityPool.getCompoundedUDeposit(D)
				assert.equal(C_deposit, "0")
				assert.equal(D_deposit, "0")

				const C_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(C)
				const D_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(D)
				assert.equal(C_depositERC20, "0")
				assert.equal(D_depositERC20, "0")

				// Check SP tracker is zero
				const UinSP_2 = await stabilityPool.getTotalUDeposits()
				assert.equal(UinSP_2, "0")

				const UinSP_2ERC20 = await stabilityPoolERC20.getTotalUDeposits()
				assert.equal(UinSP_2ERC20, "0")

				// Check SP U balance is zero
				const SPUBalance_2 = await UToken.balanceOf(stabilityPool.address)
				assert.equal(SPUBalance_2, "0")

				const SPUBalance_2ERC20 = await UToken.balanceOf(stabilityPoolERC20.address)
				assert.equal(SPUBalance_2ERC20, "0")
				...
				// E, F provide 10k U
				await UToken.transfer(E, dec(10000, 18), { from: whale })
				await UToken.transfer(F, dec(10000, 18), { from: whale })
				...
				// Check compounded deposits
				const E_deposit = await stabilityPool.getCompoundedUDeposit(E)
				const F_deposit = await stabilityPool.getCompoundedUDeposit(F)
				assert.equal(E_deposit, "0")
				assert.equal(F_deposit, "0")

				const E_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(E)
				const F_depositERC20 = await stabilityPoolERC20.getCompoundedUDeposit(F)
				assert.equal(E_depositERC20, "0")
				assert.equal(F_depositERC20, "0")

				// Check SP tracker is zero
				const UinSP_3 = await stabilityPool.getTotalUDeposits()
				assert.equal(UinSP_3, "0")

				const UinSP_3ERC20 = await stabilityPoolERC20.getTotalUDeposits()
				assert.equal(UinSP_3ERC20, "0")

				// Check SP U balance is zero
				const SPUBalance_3 = await UToken.balanceOf(stabilityPool.address)
				assert.equal(SPUBalance_3, "0")

				const SPUBalance_3ERC20 = await UToken.balanceOf(stabilityPoolERC20.address)
				assert.equal(SPUBalance_3ERC20, "0")
			...
			it("withdrawAssetGainToTrove(): Large liquidated coll/debt, deposits and ETH price", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(1, 36), ZERO_ADDRESS),
					defaulter_1,
					defaulter_1,
				...
				await borrowerOperations.openTrove(
					...
					dec(1, 27),
					th._100pct,
					await getOpenTroveUAmount(dec(1, 36), erc20.address),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1 }
				)
				...
				// Check U balances
				const aliceUBalance = await stabilityPool.getCompoundedUDeposit(alice)
				const aliceUBalanceERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)
				const aliceExpectedUBalance = web3.utils.toBN(dec(5, 35))

				const aliceUBalDiff = aliceUBalance.sub(aliceExpectedUBalance).abs()
				const aliceUBalDiffERC20 = aliceUBalanceERC20.sub(aliceExpectedUBalance).abs()

				assert.isTrue(aliceUBalDiff.lte(toBN(dec(1, 18)))) // error tolerance of 1e18
				assert.isTrue(aliceUBalDiffERC20.lte(toBN(dec(1, 18)))) // error tolerance of 1e18

				const bobUBalance = await stabilityPool.getCompoundedUDeposit(bob)
				const bobUBalanceERC20 = await stabilityPoolERC20.getCompoundedUDeposit(bob)
				const bobExpectedUBalance = toBN(dec(5, 35))

				const bobUBalDiff = bobUBalance.sub(bobExpectedUBalance).abs()
				const bobUBalDiffERC20 = bobUBalanceERC20.sub(bobExpectedUBalance).abs()

				assert.isTrue(bobUBalDiff.lte(toBN(dec(1, 18))))
				assert.isTrue(bobUBalDiffERC20.lte(toBN(dec(1, 18))))
			...
			it("withdrawAssetGainToTrove(): Small liquidated coll/debt, large deposits and ETH price", async () => {
				...
				// Defaulter opens trove with 50e-7 ETH and  5000 U. 200% ICR
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
				...
				const txAPromiseERC20 = stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: alice }
				)
				const txBPromiseERC20 = stabilityPoolERC20.withdrawAssetGainToTrove(
					ZERO_ADDRESS,
					ZERO_ADDRESS,
					{ from: bob }
				)
				...
				const aliceUBalance = await stabilityPool.getCompoundedUDeposit(alice)
				const aliceUBalanceERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)
				const aliceExpectedUBalance = toBN("99999999999999997500000000000000000000")
				const aliceUBalDiff = aliceUBalance.sub(aliceExpectedUBalance).abs()
				const aliceUBalDiffERC20 = aliceUBalanceERC20.sub(aliceExpectedUBalance).abs()

				assert.isTrue(aliceUBalDiff.lte(toBN(dec(1, 18))))
				assert.isTrue(aliceUBalDiffERC20.lte(toBN(dec(1, 18))))

				const bobUBalance = await stabilityPool.getCompoundedUDeposit(bob)
				const bobUBalanceERC20 = await stabilityPoolERC20.getCompoundedUDeposit(bob)
				const bobExpectedUBalance = toBN("99999999999999997500000000000000000000")
				const bobUBalDiff = bobUBalance.sub(bobExpectedUBalance).abs()
				const bobUBalDiffERC20 = bobUBalanceERC20.sub(bobExpectedUBalance).abs()

				assert.isTrue(bobUBalDiff.lte(toBN("100000000000000000000")))
				assert.isTrue(bobUBalDiffERC20.lte(toBN("100000000000000000000")))
			})
		})
	}
)
