...
contract("StabilityPool", async accounts => {
	...
	describe("Stability Pool Mechanisms", async () => {
		...
		// --- provideToSP() ---
		// increases recorded U at Stability Pool
		it("provideToSP(): increases the Stability Pool U balance", async () => {
			// --- SETUP --- Give Alice a least 200
			await openTrove({
				extraUAmount: toBN(200),
				ICR: toBN(dec(2, 18)),
			...
			// check U balances after
			assert.equal(await stabilityPool.getTotalUDeposits(), 200)
			assert.equal(await stabilityPoolERC20.getTotalUDeposits(), 200)
		})
		...
		it("provideToSP(): reduces the user's U balance by the correct amount", async () => {
			// --- SETUP --- Give Alice a least 200
			await openTrove({
				extraUAmount: toBN(200),
				ICR: toBN(dec(2, 18)),
			...
			// check user's U balance change
			const alice_UBalance_After = await vstToken.balanceOf(alice)
			assert.equal(alice_UBalance_Before.sub(alice_UBalance_After), "400")
		})

		it("provideToSP(): increases totalUDeposits by correct amount", async () => {
			// --- SETUP ---

			// Whale opens Trove with 50 ETH, adds 2000 U to StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(2000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			assert.equal(await stabilityPool.getTotalUDeposits(), dec(2000, 18))
			assert.equal(await stabilityPoolERC20.getTotalUDeposits(), dec(2000, 18))
		})
		...
		it("provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked", async () => {
			...
			const whaleU = (await vstToken.balanceOf(whale)).div(toBN(2))
			await stabilityPool.provideToSP(whaleU, { from: whale })
			await stabilityPoolERC20.provideToSP(whaleU, { from: whale })
			...
			// Alice makes Trove and withdraws 100 U
			await openTrove({
			...
			const SPU_Before = await stabilityPool.getTotalUDeposits()
			const SPU_BeforeERC20 = await stabilityPoolERC20.getTotalUDeposits()
			...
			const SPU_After = await stabilityPool.getTotalUDeposits()
			const SPU_AfterERC20 = await stabilityPoolERC20.getTotalUDeposits()
			assert.isTrue(SPU_After.lt(SPU_Before))
			assert.isTrue(SPU_AfterERC20.lt(SPU_BeforeERC20))
		...
		it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
			...
			// 3 Troves opened. Two users withdraw 160 U each
			await openTrove({
				extraUAmount: 0,
				ICR: toBN(dec(2, 18)),
			...
			// Alice makes deposit #1: 150 U
			await openTrove({
				extraUAmount: toBN(dec(250, 18)),
				ICR: toBN(dec(3, 18)),
				extraParams: { from: alice },
			})
			await openTrove({
			...
			// 2 users with Trove with 180 U drawn are closed
			await troveManager.liquidate(ZERO_ADDRESS, defaulter_1, { from: owner })
			await troveManager.liquidate(ZERO_ADDRESS, defaulter_2, { from: owner })
			...
			const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedUDeposit(alice)
			const alice_compoundedDeposit_1ERC20 = await stabilityPoolERC20.getCompoundedUDeposit(
				alice
			)
			...
			// Bob withdraws U and deposits to StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(1000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedUDeposit(alice)
			const alice_compoundedDeposit_2ERC20 = await stabilityPoolERC20.getCompoundedUDeposit(
				alice
			)
		...
		it("provideToSP(): reverts if user tries to provide more than their U balance", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: whale, value: dec(50, "ether") },
			...
			const aliceUbal = await vstToken.balanceOf(alice)
			const bobUbal = await vstToken.balanceOf(bob)

			// Alice, attempts to deposit 1 wei more than her balance

			const aliceTxPromise = stabilityPool.provideToSP(aliceUbal.add(toBN(1)), {
				from: alice,
			})
			const aliceTxPromiseERC20 = stabilityPoolERC20.provideToSP(aliceUbal.add(toBN(1)), {
				from: alice,
			})
			...
			const bobTxPromise = stabilityPool.provideToSP(bobUbal.add(toBN(dec(235534, 18))), {
				from: bob,
			})
			const bobTxPromiseERC20 = stabilityPoolERC20.provideToSP(
				bobUbal.add(toBN(dec(235534, 18))),
				{ from: bob }
			)
		...
		it("provideToSP(): reverts if user tries to provide 2^256-1 U, which exceeds their balance", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: whale, value: dec(50, "ether") },
			...
			// Alice attempts to deposit 2^256-1 U
			try {
		...
		it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
			...
			// NonPayable makes deposit #1: 150 U
			...
			// NonPayable tries to make deposit #2: 100U (which also attempts to withdraw ETH gain)
		...
		it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
			...
			const alice_UDeposit_Before = (
				await stabilityPool.getCompoundedUDeposit(alice)
			).toString()
			const bob_UDeposit_Before = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
			const carol_UDeposit_Before = (
				await stabilityPool.getCompoundedUDeposit(carol)
			).toString()

			const alice_UDeposit_BeforeERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(alice)
			).toString()
			const bob_UDeposit_BeforeERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(bob)
			).toString()
			const carol_UDeposit_BeforeERC20 = (
			...
			const carol_ETHGain_Before = (
				await stabilityPool.getDepositorAssetGain(carol)
			).toString()

			const alice_ETHGain_BeforeERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(alice)
			).toString()
			const bob_ETHGain_BeforeERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(bob)
			).toString()
			const carol_ETHGain_BeforeERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(carol)
			).toString()

			//check non-zero U and AssetGain in the Stability Pool
			const UinSP = await stabilityPool.getTotalUDeposits()
			const ETHinSP = await stabilityPool.getAssetBalance()
			const UinSPERC20 = await stabilityPoolERC20.getTotalUDeposits()
			const ETHinSPERC20 = await stabilityPoolERC20.getAssetBalance()
			assert.isTrue(UinSP.gt(mv._zeroBN))
			assert.isTrue(ETHinSP.gt(mv._zeroBN))
			assert.isTrue(UinSPERC20.gt(mv._zeroBN))
			assert.isTrue(ETHinSPERC20.gt(mv._zeroBN))

			// D makes an SP deposit
			await stabilityPool.provideToSP(dec(1000, 18), { from: dennis })
			assert.equal(
				(await stabilityPool.getCompoundedUDeposit(dennis)).toString(),
				dec(1000, 18)
			)

			await stabilityPoolERC20.provideToSP(dec(1000, 18), { from: dennis })
			assert.equal(
				(await stabilityPoolERC20.getCompoundedUDeposit(dennis)).toString(),
				dec(1000, 18)
			)

			const alice_UDeposit_After = (
				await stabilityPool.getCompoundedUDeposit(alice)
			).toString()
			const bob_UDeposit_After = (await stabilityPool.getCompoundedUDeposit(bob)).toString()
			const carol_UDeposit_After = (
				await stabilityPool.getCompoundedUDeposit(carol)
			).toString()

			const alice_UDeposit_AfterERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(alice)
			).toString()
			const bob_UDeposit_AfterERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(bob)
			).toString()
			const carol_UDeposit_AfterERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(carol)
			).toString()
			...
			const alice_ETHGain_AfterERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(alice)
			).toString()
			const bob_ETHGain_AfterERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(bob)
			).toString()
			const carol_ETHGain_AfterERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(carol)
			).toString()

			// Check compounded deposits and ETH gains for A, B and C have not changed
			assert.equal(alice_UDeposit_Before, alice_UDeposit_After)
			assert.equal(bob_UDeposit_Before, bob_UDeposit_After)
			assert.equal(carol_UDeposit_Before, carol_UDeposit_After)

			assert.equal(alice_UDeposit_BeforeERC20, alice_UDeposit_AfterERC20)
			assert.equal(bob_UDeposit_BeforeERC20, bob_UDeposit_AfterERC20)
			assert.equal(carol_UDeposit_BeforeERC20, carol_UDeposit_AfterERC20)
		...
		it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
			...
			const activeDebt_Before = (await activePool.getUDebt(ZERO_ADDRESS)).toString()
			const defaultedDebt_Before = (await defaultPool.getUDebt(ZERO_ADDRESS)).toString()
			...
			const activeDebt_BeforeERC20 = (await activePool.getUDebt(erc20.address)).toString()
			const defaultedDebt_BeforeERC20 = (await defaultPool.getUDebt(erc20.address)).toString()
			...
			const activeDebt_After = (await activePool.getUDebt(ZERO_ADDRESS)).toString()
			const defaultedDebt_After = (await defaultPool.getUDebt(ZERO_ADDRESS)).toString()
			...
			const activeDebt_AfterERC20 = (await activePool.getUDebt(erc20.address)).toString()
			const defaultedDebt_AfterERC20 = (await defaultPool.getUDebt(erc20.address)).toString()
		...
		it("provideToSP(): doesn't impact any troves, including the caller's trove", async () => {
			...
			const dennis_Debt_Before = (
				await troveManager.Troves(dennis, ZERO_ADDRESS)
			)[0].toString()

			const whale_Debt_BeforeERC20 = (
				await troveManager.Troves(whale, erc20.address)
			)[0].toString()
			const alice_Debt_BeforeERC20 = (
				await troveManager.Troves(alice, erc20.address)
			)[0].toString()
			const bob_Debt_BeforeERC20 = (
				await troveManager.Troves(bob, erc20.address)
			)[0].toString()
			const carol_Debt_BeforeERC20 = (
				await troveManager.Troves(carol, erc20.address)
			)[0].toString()
			const dennis_Debt_BeforeERC20 = (
				await troveManager.Troves(dennis, erc20.address)
			)[0].toString()

			const whale_Coll_Before = (await troveManager.Troves(whale, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const alice_Coll_Before = (await troveManager.Troves(alice, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const bob_Coll_Before = (await troveManager.Troves(bob, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const carol_Coll_Before = (await troveManager.Troves(carol, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const dennis_Coll_Before = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()

			const whale_Coll_BeforeERC20 = (await troveManager.Troves(whale, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const alice_Coll_BeforeERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const bob_Coll_BeforeERC20 = (await troveManager.Troves(bob, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const carol_Coll_BeforeERC20 = (await troveManager.Troves(carol, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const dennis_Coll_BeforeERC20 = (await troveManager.Troves(dennis, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()

			const whale_ICR_Before = (
				await troveManager.getCurrentICR(ZERO_ADDRESS, whale, price)
			).toString()
			const alice_ICR_Before = (
				await troveManager.getCurrentICR(ZERO_ADDRESS, alice, price)
			).toString()
			...
			const whale_ICR_BeforeERC20 = (
				await troveManager.getCurrentICR(erc20.address, whale, price)
			).toString()
			...
			const whale_Coll_After = (await troveManager.Troves(whale, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const alice_Coll_After = (await troveManager.Troves(alice, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const bob_Coll_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const carol_Coll_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const dennis_Coll_After = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			...
			const whale_Coll_AfterERC20 = (await troveManager.Troves(whale, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const alice_Coll_AfterERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const bob_Coll_AfterERC20 = (await troveManager.Troves(bob, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const carol_Coll_AfterERC20 = (await troveManager.Troves(carol, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const dennis_Coll_AfterERC20 = (await troveManager.Troves(dennis, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
		...
		it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
			...
			assert.equal((await stabilityPool.getCompoundedUDeposit(bob)).toString(), dec(1000, 18))
			assert.equal(
				(await stabilityPoolERC20.getCompoundedUDeposit(bob)).toString(),
				dec(1000, 18)
			)
		...
		it("provideToSP(): providing 0 U reverts", async () => {
			// --- SETUP ---
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			// A, B, C provides 100, 50, 30 U to SP
			await stabilityPool.provideToSP(dec(100, 18), { from: alice })
			...
			const bob_Deposit_BeforeERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(bob)
			).toString()
			const UinSP_BeforeERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()

			assert.equal(UinSP_Before, dec(180, 18))
			assert.equal(UinSP_BeforeERC20, dec(180, 18))

			// Bob provides 0 U to the Stability Pool
		...
		// --- YOU functionality ---
		it("provideToSP(), new deposit: when SP > 0, triggers YOU reward event - increases the sum G", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: whale, value: dec(50, "ether") },
			...
			const G_BeforeERC20 = await stabilityPoolERC20.epochToScaleToG(
				currentEpochERC20,
				currentScaleERC20
			)
			...
			const G_AfterERC20 = await stabilityPoolERC20.epochToScaleToG(
				currentEpochERC20,
				currentScaleERC20
			)
		...
		it("provideToSP(), new deposit: depositor does not receive any YOU rewards", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: whale, value: dec(50, "ether") },
		...
		it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any YOU rewards", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
		...
		it("provideToSP(), new eligible deposit: tagged System's stake increases", async () => {
			...
			assert.equal(
				Stake_DiffERC20.toString(),
				deposit_A.add(deposit_B).add(deposit_C).toString()
			)
		})
		...
		it("provideToSP(), new eligible deposit: tagged System's snapshots update", async () => {
			...
			const S_BeforeERC20 = await stabilityPoolERC20.epochToScaleToSum(
				currentEpochERC20,
				currentScaleERC20
			)
			const P_BeforeERC20 = await stabilityPoolERC20.P()
		...
		it("provideToSP(), topup: triggers YOU reward event - increases the sum G", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			...
			// Expect G has increased from the YOU reward event triggered by B's topup
			assert.isTrue(G_After.gt(G_Before))
			assert.isTrue(G_AfterERC20.gt(G_BeforeERC20))
		})

		it("provideToSP(), topup: depositor receives YOU rewards", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			...
			assert.isTrue(B_YOUBalance_After.gt(B_YOUBalance_Before))
			assert.isTrue(C_YOUBalance_After.gt(C_YOUBalance_Before))
		})
		...
		it("provideToSP(), topup: System's snapshots update", async () => {
			...
			await openTrove({
				extraUAmount: toBN(dec(400, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: B },
			})
			await openTrove({
				extraUAmount: toBN(dec(600, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: C },
			})
			...
			const G1ERC20 = await stabilityPoolERC20.epochToScaleToG(
				currentScaleERC20,
				currentEpochERC20
			)
			await stabilityPoolERC20.provideToSP(deposit_A, { from: A })
		...
		it("withdrawFromSP(): partial retrieval - retrieves correct U amount and the entire ETH Gain, and updates deposit", async () => {
			// --- SETUP ---
			// Whale deposits 185000 U in StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(1, 24)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			})
			await openTrove({
			...
			// 2 users with Trove with 170 U drawn are closed
			...
			// Alice ULoss is ((15000/200000) * liquidatedDebt), for each liquidation
			const expectedULoss_A = liquidatedDebt_1
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
				.add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

			const expectedULoss_AERC20 = liquidatedDebt_1ERC20
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
				.add(liquidatedDebt_2ERC20.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

			const expectedCompoundedUDeposit_A = toBN(dec(15000, 18)).sub(expectedULoss_A)
			const compoundedUDeposit_A = await stabilityPool.getCompoundedUDeposit(alice)

			const expectedCompoundedUDeposit_AERC20 = toBN(dec(15000, 18)).sub(expectedULoss_AERC20)
			const compoundedUDeposit_AERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)

			assert.isAtMost(
				th.getDifference(expectedCompoundedUDeposit_A, compoundedUDeposit_A),
				100000
			)
			assert.isAtMost(
				th.getDifference(expectedCompoundedUDeposit_AERC20, compoundedUDeposit_AERC20),
				100000
			)

			// Alice retrieves part of her entitled U: 9000 U
			await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice })
			await stabilityPoolERC20.withdrawFromSP(dec(9000, 18), { from: alice })

			const expectedNewDeposit_A = compoundedUDeposit_A.sub(toBN(dec(9000, 18)))
			const expectedNewDeposit_AERC20 = compoundedUDeposit_AERC20.sub(toBN(dec(9000, 18)))
		...
		it("withdrawFromSP(): partial retrieval - leaves the correct amount of U in the Stability Pool", async () => {
			// --- SETUP ---
			// Whale deposits 185000 U in StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(1, 24)),
			...
			const SP_U_Before = await stabilityPool.getTotalUDeposits()
			assert.equal(SP_U_Before, dec(200000, 18))

			const SP_U_BeforeERC20 = await stabilityPoolERC20.getTotalUDeposits()
			assert.equal(SP_U_BeforeERC20, dec(200000, 18))
			...
			const [liquidatedDebt_1ERC20] = await th.getEmittedLiquidationValues(
				liquidationTX_1ERC20
			)
			const [liquidatedDebt_2ERC20] = await th.getEmittedLiquidationValues(
				liquidationTX_2ERC20
			)
      ...
      Expect U in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
			const expectedSPU = toBN(dec(200000, 18))
				.sub(toBN(liquidatedDebt_1))
				.sub(toBN(liquidatedDebt_2))
				.sub(toBN(dec(9000, 18)))

			const expectedSPUERC20 = toBN(dec(200000, 18))
				.sub(toBN(liquidatedDebt_1ERC20))
				.sub(toBN(liquidatedDebt_2ERC20))
				.sub(toBN(dec(9000, 18)))

			const SP_U_After = (await stabilityPool.getTotalUDeposits()).toString()
			const SP_U_AfterERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()

			th.assertIsApproximatelyEqual(SP_U_After, expectedSPU)
			th.assertIsApproximatelyEqual(SP_U_AfterERC20, expectedSPUERC20)
		})

		it("withdrawFromSP(): full retrieval - leaves the correct amount of U in the Stability Pool", async () => {
			// --- SETUP ---
			// Whale deposits 185000 U in StabilityPool
			await openTrove({
			...
			const UinSPBefore = await stabilityPool.getTotalUDeposits()
			const UinSPBeforeERC20 = await stabilityPoolERC20.getTotalUDeposits()

			// Alice retrieves all of her entitled U:
			...
			const expectedUinSPAfter = UinSPBefore.sub(compoundedUDeposit_A)
			const expectedUinSPAfterERC20 = UinSPBefore.sub(compoundedUDeposit_AERC20)

			const UinSPAfter = await stabilityPool.getTotalUDeposits()
			const UinSPAfterERC20 = await stabilityPoolERC20.getTotalUDeposits()
			assert.isAtMost(th.getDifference(expectedUinSPAfter, UinSPAfter), 100000)
			assert.isAtMost(th.getDifference(expectedUinSPAfterERC20, UinSPAfterERC20), 100000)
		})
		...
		it("withdrawFromSP(): it correctly updates the user's U and ETH snapshots of entitled reward per unit staked", async () => {
			// --- SETUP ---
			// Whale deposits 185000 U in StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(1000000, 18)),
		...
		it("withdrawFromSP(): decreases StabilityPool ETH", async () => {
			...
			const stability_ETH_DifferenceERC20 = stability_ETH_BeforeERC20.sub(
				stability_ETH_AfterERC20
			)
		...
		it("withdrawFromSP(): All depositors are able to withdraw from the SP to their account", async () => {
			...
			const totalDeposits = (await stabilityPool.getTotalUDeposits()).toString()
			const totalDepositsERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()

			assert.isAtMost(th.getDifference(totalDeposits, "0"), 100000)
			assert.isAtMost(th.getDifference(totalDepositsERC20, "0"), 100000)
		})

		it("withdrawFromSP(): increases depositor's U token balance by the expected amount", async () => {
			// Whale opens trove
			await openTrove({
			...
			const defaulterDebt = (
				await troveManager.getEntireDebtAndColl(ZERO_ADDRESS, defaulter_1)
			)[0]
			const defaulterDebtERC20 = (
				await troveManager.getEntireDebtAndColl(erc20.address, defaulter_1)
			)[0]
			...
			/* From an offset of 10000 U, each depositor receives
      ULoss = 1666.6666666666666666 U

      and thus with a deposit of 10000 U, each should withdraw 8333.3333333333333333 U (in practice, slightly less due to rounding error)
      */

			// Price bounces back to $200 per ETH
			await priceFeed.setPrice(dec(200, 18))

			// Bob issues a further 5000 U from his trove
			await borrowerOperations.withdrawU(ZERO_ADDRESS, th._100pct, dec(5000, 18), bob, bob, {
				from: bob,
			})
			await borrowerOperations.withdrawU(erc20.address, th._100pct, dec(5000, 18), bob, bob, {
				from: bob,
			})

			// Expect Alice's U balance increase be very close to 8333.3333333333333333 U
			await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
			await stabilityPoolERC20.withdrawFromSP(dec(10000, 18), { from: alice })
			...
			// expect Bob's U balance increase to be very close to  13333.33333333333333333 U
			await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
			await stabilityPoolERC20.withdrawFromSP(dec(10000, 18), { from: bob })
		...
		it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
			...
			const A_pendingYOUGain = await stabilityPool.getDepositorYOUGain(A)
			assert.isTrue(A_pendingETHGain.gt(toBN("0")))
			assert.isTrue(A_pendingYOUGain.gt(toBN("0")))

			const A_pendingETHGainERC20 = await stabilityPoolERC20.getDepositorAssetGain(A)
			const A_pendingYOUGainERC20 = await stabilityPoolERC20.getDepositorYOUGain(A)
			assert.isTrue(A_pendingETHGainERC20.gt(toBN("0")))
			assert.isTrue(A_pendingYOUGainERC20.gt(toBN("0")))
			...
			assert.isAtMost(
				th.getDifference(A_YOUBalDiff, A_pendingYOUGain.add(A_pendingYOUGainERC20)),
				1000
			)
		})

		it("withdrawFromSP(): withdrawing 0 U doesn't alter the caller's deposit or the total U in the Stability Pool", async () => {
			// --- SETUP ---
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
			...
			// Bob withdraws 0 U from the Stability Pool
			await stabilityPool.withdrawFromSP(0, { from: bob })
			await stabilityPoolERC20.withdrawFromSP(0, { from: bob })

			// check Bob's deposit and total U in Stability Pool has not changed
			...
			const bob_Deposit_AfterERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(bob)
			).toString()
			const UinSP_AfterERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()

			assert.equal(bob_Deposit_Before, bob_Deposit_After)
			assert.equal(UinSP_Before, UinSP_After)

			assert.equal(bob_Deposit_BeforeERC20, bob_Deposit_AfterERC20)
			assert.equal(UinSP_BeforeERC20, UinSP_AfterERC20)
		})
		...
		it("withdrawFromSP(): withdrawing 0 ETH Gain does not alter the caller's ETH balance, their trove collateral, or the ETH  in the Stability Pool", async () => {
			...
			const dennis_Collateral_Before = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			const ETHinSP_Before = (await stabilityPool.getAssetBalance()).toString()

			const dennis_ETHBalance_BeforeERC20 = (await erc20.balanceOf(dennis)).toString()
			const dennis_Collateral_BeforeERC20 = (await troveManager.Troves(dennis, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
			const ETHinSP_BeforeERC20 = (await stabilityPoolERC20.getAssetBalance()).toString()
			...
			const dennis_Collateral_After = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			].toString()
			...
			const dennis_Collateral_AfterERC20 = (await troveManager.Troves(dennis, erc20.address))[
				th.TROVE_COLL_INDEX
			].toString()
		...
		it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
			...
			const alice_U_Balance_Before = await vstToken.balanceOf(alice)
			const bob_U_Balance_Before = await vstToken.balanceOf(bob)

			const alice_Deposit_Before = await stabilityPool.getCompoundedUDeposit(alice)
			const bob_Deposit_Before = await stabilityPool.getCompoundedUDeposit(bob)

			const alice_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)
			const bob_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedUDeposit(bob)

			const UinSP_Before = await stabilityPool.getTotalUDeposits()
			const UinSP_BeforeERC20 = await stabilityPoolERC20.getTotalUDeposits()
			...
			// Check Bob's U balance has risen by only the value of his compounded deposit
			const bob_expectedUBalance = bob_U_Balance_Before
				.add(bob_Deposit_Before)
				.add(bob_Deposit_BeforeERC20)
				.toString()
			const bob_U_Balance_After = (await vstToken.balanceOf(bob)).toString()
			assert.equal(bob_U_Balance_After, bob_expectedUBalance)

			// Alice attempts to withdraws 2309842309.000000000000000000 U from the Stability Pool
			...
			// Check Alice's U balance has risen by only the value of her compounded deposit
			const alice_expectedUBalance = alice_U_Balance_Before
				.add(alice_Deposit_Before)
				.add(alice_Deposit_BeforeERC20)
				.toString()
			const alice_U_Balance_After = (await vstToken.balanceOf(alice)).toString()
			assert.equal(alice_U_Balance_After, alice_expectedUBalance)

			// Check U in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
			const expectedUinSP = UinSP_Before.sub(alice_Deposit_Before)
				.sub(bob_Deposit_Before)
				.toString()
			const UinSP_After = (await stabilityPool.getTotalUDeposits()).toString()
			assert.equal(UinSP_After, expectedUinSP)
		})

		it("withdrawFromSP(): Request to withdraw 2^256-1 U only withdraws the caller's compounded deposit", async () => {
			// --- SETUP ---
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
			...
			// Bob attempts to withdraws maxBytes32 U from the Stability Pool
			...
			// Check U in Stability Pool has been reduced by only  Bob's compounded deposit
			const expectedUinSP = UinSP_Before.sub(bob_Deposit_Before).toString()
			const UinSP_After = (await stabilityPool.getTotalUDeposits()).toString()
			assert.equal(UinSP_After, expectedUinSP)

			const expectedUinSPERC20 = UinSP_BeforeERC20.sub(bob_Deposit_BeforeERC20).toString()
			const UinSP_AfterERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()
			assert.equal(UinSP_AfterERC20, expectedUinSPERC20)
		})
		...
		it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
			...
			// A, B, C provides 10000, 5000, 3000 U to SP
			await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
			...
			const alice_U_Balance_Before = await vstToken.balanceOf(alice)
			const bob_U_Balance_Before = await vstToken.balanceOf(bob)
			const carol_U_Balance_Before = await vstToken.balanceOf(carol)
			...
			const alice_Deposit_Before = await stabilityPool.getCompoundedUDeposit(alice)
			const bob_Deposit_Before = await stabilityPool.getCompoundedUDeposit(bob)
			const carol_Deposit_Before = await stabilityPool.getCompoundedUDeposit(carol)

			const alice_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)
			const bob_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedUDeposit(bob)
			const carol_Deposit_BeforeERC20 = await stabilityPoolERC20.getCompoundedUDeposit(carol)
			...
			// Check U balances of A, B, C have risen by the value of their compounded deposits, respectively
			const alice_expectedUBalance = alice_U_Balance_Before
				.add(alice_Deposit_Before)
				.add(alice_Deposit_BeforeERC20)
				.toString()
			const bob_expectedUBalance = bob_U_Balance_Before
				...
				.add(bob_Deposit_BeforeERC20)
				.toString()
			const carol_expectedUBalance = carol_U_Balance_Before
			...
			const alice_U_Balance_After = (await vstToken.balanceOf(alice)).toString()
			const bob_U_Balance_After = (await vstToken.balanceOf(bob)).toString()
			const carol_U_Balance_After = (await vstToken.balanceOf(carol)).toString()

			assert.equal(alice_U_Balance_After, alice_expectedUBalance)
			assert.equal(bob_U_Balance_After, bob_expectedUBalance)
			assert.equal(carol_U_Balance_After, carol_expectedUBalance)
			...
			const alice_expectedETHBalanceERC20 = alice_ETH_Balance_BeforeERC20
				...
				.toString()
			const bob_expectedETHBalanceERC20 = bob_ETH_Balance_BeforeERC20
			...
			// Check U in Stability Pool has been reduced by A, B and C's compounded deposit
			const expectedUinSP = UinSP_Before.sub(alice_Deposit_Before)
				.sub(bob_Deposit_Before)
			...
			const expectedUinSPERC20 = UinSP_BeforeERC20.sub(alice_Deposit_BeforeERC20)
		...
		it("getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
			...
			// A, B, provide 10000, 5000 U to SP
			await stabilityPool.provideToSP(dec(10000, 18), { from: alice })
			await stabilityPool.provideToSP(dec(5000, 18), { from: bob })
			...
			const UinSPERC20 = (await stabilityPoolERC20.getTotalUDeposits()).toString()
			assert.equal(UinSPERC20, "0")

			// Check Stability deposits have been fully cancelled with debt, and are now all zero
			const alice_Deposit = (await stabilityPool.getCompoundedUDeposit(alice)).toString()
			const bob_Deposit = (await stabilityPool.getCompoundedUDeposit(bob)).toString()

			const alice_DepositERC20 = (
				await stabilityPoolERC20.getCompoundedUDeposit(alice)
			).toString()
			const bob_DepositERC20 = (await stabilityPoolERC20.getCompoundedUDeposit(bob)).toString()
			...
			const alice_ETHGain_1ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(alice)
			).toString()
			const bob_ETHGain_1ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(bob)
			).toString()

			// Whale deposits 10000 U to Stability Pool
			...
			const alice_ETHGain_2ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(alice)
			).toString()
			const bob_ETHGain_2ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(bob)
			).toString()
			...
			const alice_ETHGain_3ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(alice)
			).toString()
			const bob_ETHGain_3ERC20 = (
				await stabilityPoolERC20.getDepositorAssetGain(bob)
			).toString()
		...
		// --- YOU functionality ---
		it("withdrawFromSP(): triggers YOU reward event - increases the sum G", async () => {
			await openTrove({
				extraUAmount: toBN(dec(1, 24)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
		...
		it("withdrawFromSP(), partial withdrawal: depositor receives YOU rewards", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
		...
		it("withdrawFromSP(), partial withdrawal: System's snapshots update", async () => {
			...
			await openTrove({
				extraUAmount: toBN(dec(60000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: C },
			})
		...
		it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
			...
			// Fast-forward time and make a second deposit, to trigger YOU reward and make G > 0
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
			await stabilityPool.provideToSP(dec(10000, 18), { from: E })
			await stabilityPoolERC20.provideToSP(dec(10000, 18), { from: E })
		...
		it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
			...
			await openTrove({
				extraUAmount: toBN(dec(10100, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: A },
			})
			await stabilityPool.provideToSP(dec(10000, 18), { from: A })
			...
			//  SETUP: Execute a series of operations to trigger YOU and ETH rewards for depositor A
			...
			await th.assertRevert(
				stabilityPool.withdrawFromSP(dec(10000, 18), { from: A }),
			...
			await th.assertRevert(
				stabilityPool.withdrawFromSP(dec(10000, 18), { from: C }),
				expectedRevertMessage
			)
		})
		...
		it("withdrawETHGainToTrove(): Applies ULoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
			// --- SETUP ---
			// Whale deposits 185000 U in StabilityPool
			await openTrove({
				extraUAmount: toBN(dec(1000000, 18)),
			...
			const compoundedDeposit_A = await stabilityPool.getCompoundedUDeposit(alice)

			const ETHGain_AERC20 = await stabilityPoolERC20.getDepositorAssetGain(alice)
			const compoundedDeposit_AERC20 = await stabilityPoolERC20.getCompoundedUDeposit(alice)
			...
			const expectedETHGain_A = liquidatedColl
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
			const expectedULoss_A = liquidatedDebt
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
			const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedULoss_A)

			const expectedETHGain_AERC20 = liquidatedCollERC20
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
			const expectedULoss_AERC20 = liquidatedDebtERC20
				.mul(toBN(dec(15000, 18)))
				.div(toBN(dec(200000, 18)))
			const expectedCompoundedDeposit_AERC20 = toBN(dec(15000, 18)).sub(expectedULoss_AERC20)
			...
			assert.isAtMost(
				th.getDifference(expectedCompoundedDeposit_AERC20, compoundedDeposit_AERC20),
				100000
			)
			...
			// check Alice's ULoss has been applied to her deposit expectedCompoundedDeposit_A
			...
			const Trove_ETH_IncreaseERC20 = aliceTrove_ETH_AfterERC20
				.sub(aliceTrove_ETH_BeforeERC20)
				.toString()
		...
		it("withdrawETHGainToTrove(): All depositors withdraw, each withdraw their correct ETH gain", async () => {
			...
			const collBeforeERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			] // all troves have same coll before
      ...
      */
			...
			await stabilityPoolERC20.withdrawAssetGainToTrove(alice, alice, { from: alice })
			const aliceCollAfterERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
			...
			const bobCollAfterERC20 = (await troveManager.Troves(bob, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
				th.getDifference(bobCollAfterERC20.sub(collBeforeERC20), expectedCollGainERC20),
				10000
			)

			await stabilityPool.withdrawAssetGainToTrove(carol, carol, { from: carol })
			const carolCollAfter = (await troveManager.Troves(carol, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
			...
			const carolCollAfterERC20 = (await troveManager.Troves(carol, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
			...
			const dennisCollAfter = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
			...
			const dennisCollAfterERC20 = (await troveManager.Troves(dennis, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
			...
			const erinCollAfter = (await troveManager.Troves(erin, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			...
			const erinCollAfterERC20 = (await troveManager.Troves(erin, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
				th.getDifference(erinCollAfterERC20.sub(collBeforeERC20), expectedCollGainERC20),
				10000
			)

			await stabilityPool.withdrawAssetGainToTrove(flyn, flyn, { from: flyn })
			const flynCollAfter = (await troveManager.Troves(flyn, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			...
			const flynCollAfterERC20 = (await troveManager.Troves(flyn, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			assert.isAtMost(
				th.getDifference(flynCollAfterERC20.sub(collBeforeERC20), expectedCollGainERC20),
				10000
			)
		})
		...
		it("withdrawETHGainToTrove(): caller can withdraw full deposit and ETH gain to their trove during Recovery Mode", async () => {
			...
			const alice_Collateral_Before = (await troveManager.Troves(alice, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			const bob_Collateral_Before = (await troveManager.Troves(bob, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			const carol_Collateral_Before = (await troveManager.Troves(carol, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]

			const alice_Collateral_BeforeERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			const bob_Collateral_BeforeERC20 = (await troveManager.Troves(bob, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			const carol_Collateral_BeforeERC20 = (await troveManager.Troves(carol, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			...
			const alice_Collateral_After = (await troveManager.Troves(alice, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			const bob_Collateral_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]
			const carol_Collateral_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
				th.TROVE_COLL_INDEX
			]

			const alice_Collateral_AfterERC20 = (await troveManager.Troves(alice, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			const bob_Collateral_AfterERC20 = (await troveManager.Troves(bob, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			const carol_Collateral_AfterERC20 = (await troveManager.Troves(carol, erc20.address))[
				th.TROVE_COLL_INDEX
			]
			...
			assert.equal(
				alice_expectedCollateralERC20,
				alice_Collateral_AfterERC20.div(toBN(10 ** 10))
			)
			assert.equal(bob_expectedColalteralERC20, bob_Collateral_AfterERC20.div(toBN(10 ** 10)))
			assert.equal(
				carol_expectedCollateralERC20,
				carol_Collateral_AfterERC20.div(toBN(10 ** 10))
			)
		...
		it("withdrawETHGainToTrove(): triggers YOU reward event - increases the sum G", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
		...
		it("withdrawETHGainToTrove(), eligible deposit: depositor receives YOU rewards", async () => {
			await openTrove({
				extraUAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			...
			assert.isTrue(B_YOUBalance_AfterERC20.gt(B_YOUBalance_After))
			assert.isTrue(C_YOUBalance_AfterERC20.gt(C_YOUBalance_After))
		})
		...
		it("withdrawETHGainToTrove(), eligible deposit: System's snapshots update", async () => {
			...
			// between topups, because some block.timestamp time passes (and YOU is issued) between ops
			const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch)
			await stabilityPool.withdrawAssetGainToTrove(A, A, { from: A })
		...
		it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
			...
			// fastforward time, and E makes a deposit, creating YOU rewards for all
			await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
			await openTrove({
				extraUAmount: toBN(dec(3000, 18)),
				ICR: toBN(dec(2, 18)),
