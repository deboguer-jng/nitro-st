...
contract("TroveManager - Redistribution reward calculations", async accounts => {
	...
	it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
		...
		const alice_Coll_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, alice))
			.toString()
		const carol_Coll_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
    ...
    */
		...
		assert.equal(
			...
			A_coll_Asset.add(C_coll_Asset).add(
				th.applyLiquidationFee(B_coll_Asset.add(D_coll_Asset))
			)
		)

		// check U gas compensation
		assert.equal(
			(await vstToken.balanceOf(owner)).toString(),
	...
	it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
		...
		const bob_Coll_Asset = (await troveManager.Troves(bob, erc20.address))[th.TROVE_COLL_INDEX]
			...
			.toString()
		const dennis_Coll_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, dennis))
			.toString()
		const erin_Coll_Asset = (await troveManager.Troves(erin, erc20.address))[
			th.TROVE_COLL_INDEX
		]
    ...
    */
		...
		const A_collAfterL1_Asset = A_coll_Asset.add(
			A_coll_Asset.mul(th.applyLiquidationFee(C_coll_Asset)).div(
				A_coll_Asset.add(B_coll_Asset)
			)
		)
		const B_collAfterL1_Asset = B_coll_Asset.add(
			B_coll_Asset.mul(th.applyLiquidationFee(C_coll_Asset)).div(
				A_coll_Asset.add(B_coll_Asset)
			)
		)
	...
	it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
		...
		const freddy_rawColl = (await troveManager.Troves(freddy, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].toString()
		const freddy_ETHReward = (
			await troveManager.getPendingAssetReward(ZERO_ADDRESS, freddy)
		).toString()

		const freddy_rawColl_Asset = (await troveManager.Troves(freddy, erc20.address))[
			th.TROVE_COLL_INDEX
		].toString()
		const freddy_ETHReward_Asset = (
			await troveManager.getPendingAssetReward(erc20.address, freddy)
		).toString()
    ...
    */
			...
			E_coll_Asset.add(
				th.applyLiquidationFee(
					D_coll_Asset.add(
						th.applyLiquidationFee(
							C_coll_Asset.add(
								th.applyLiquidationFee(B_coll_Asset.add(th.applyLiquidationFee(A_coll_Asset)))
							)
						)
					)
				)
		...
		assert.isAtMost(
			th.getDifference(entireSystemColl_Asset, F_coll_Asset.add(gainedETH_Asset)),
			1000
		)
		...
		assert.equal(
			(await vstToken.balanceOf(owner)).toString(),
			toBN(dec(1000, 18)).mul(toBN(2)).toString()
		)
	})
	...
	it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
		...
		const B_entireColl_1_Asset = (await th.getEntireCollAndDebt(contracts, B, erc20.address))
			.entireColl
		const C_entireColl_1_Asset = (await th.getEntireCollAndDebt(contracts, C, erc20.address))
			.entireColl
		const D_entireColl_1_Asset = (await th.getEntireCollAndDebt(contracts, D, erc20.address))
			.entireColl
		const E_entireColl_1_Asset = (await th.getEntireCollAndDebt(contracts, E, erc20.address))
			.entireColl
		...
		const totalCollAfterL1_Asset = B_coll_Asset.add(C_coll_Asset)
			.add(D_coll_Asset)
			.add(E_coll_Asset)
		const B_collAfterL1_Asset = B_coll_Asset.add(
		...
		const B_entireColl_2_Asset = (await th.getEntireCollAndDebt(contracts, B, erc20.address))
			.entireColl
		const D_entireColl_2_Asset = (await th.getEntireCollAndDebt(contracts, D, erc20.address))
			.entireColl
		const E_entireColl_2_Asset = (await th.getEntireCollAndDebt(contracts, E, erc20.address))
			.entireColl
		...
		const B_entireColl_3_Asset = (await th.getEntireCollAndDebt(contracts, B, erc20.address))
			.entireColl
		const D_entireColl_3_Asset = (await th.getEntireCollAndDebt(contracts, D, erc20.address))
			.entireColl
	...
	it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
		...
		const A_entireColl_0_Asset = (await th.getEntireCollAndDebt(contracts, A, erc20.address))
			.entireColl
		const B_entireColl_0_Asset = (await th.getEntireCollAndDebt(contracts, B, erc20.address))
			.entireColl
		const C_entireColl_0_Asset = (await th.getEntireCollAndDebt(contracts, C, erc20.address))
			.entireColl
		const D_entireColl_0_Asset = (await th.getEntireCollAndDebt(contracts, D, erc20.address))
			.entireColl
		const E_entireColl_0_Asset = (await th.getEntireCollAndDebt(contracts, E, erc20.address))
			.entireColl

		// entireSystemColl, excluding A
		const denominatorColl_1 = (await troveManager.getEntireSystemColl(ZERO_ADDRESS)).sub(
			A_entireColl_0
		)
		const denominatorColl_1_Asset = (
			await troveManager.getEntireSystemColl(erc20.address)
		).sub(A_entireColl_0_Asset)
		...
		// // Bob adds 1 ETH to his trove
		await borrowerOperations.addColl(ZERO_ADDRESS, 0, B, B, {
			from: B,
			value: dec(1, "ether"),
		})
		...
		const denominatorColl_2 = (await troveManager.getEntireSystemColl(ZERO_ADDRESS)).sub(
			C_entireColl_1
		)
		const denominatorColl_2_Asset = (
			await troveManager.getEntireSystemColl(erc20.address)
		).sub(C_entireColl_1_Asset)
		...
		const denominatorColl_3 = (await troveManager.getEntireSystemColl(ZERO_ADDRESS)).sub(
			E_entireColl_2
		)
		const denominatorColl_3_Asset = (
			await troveManager.getEntireSystemColl(erc20.address)
		).sub(E_entireColl_2_Asset)
	...
	it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
		...
		await borrowerOperations.addColl(ZERO_ADDRESS, 0, bob, bob, {
			from: bob,
			value: addedColl,
		})
		...
		// Expect Bob now holds all Ether and UDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 U (10 each for gas compensation)
		const bob_Coll = (await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
			.add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, bob))
			.toString()

		const bob_UDebt = (await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
		...
		const bob_UDebt_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		const expected_B_coll_Asset = B_coll_Asset.add(addedColl)
			...
			.add(
				th
					.applyLiquidationFee(C_coll_Asset)
					.mul(B_coll_Asset)
		...
		assert.isAtMost(
			th.getDifference(bob_UDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
			1000
		)
	...
	it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
		...
		/* Bob rewards:
     L1: 1/2*0.995 ETH, 55 U
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 UDebt
    ...
    debt: 233.77 UDebt
     ...
     Alice rewards:
    L1 1/2*0.995 ETH, 55 U
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 UDebt

    coll: 1.8705 ETH
    debt: 146.23 UDebt

    totalColl: 4.99 ETH
    totalDebt 380 U (includes 50 each for gas compensation)
    */
		const bob_Coll = (await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
			.add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, bob))
			.toString()
		...
		const alice_UDebt = (await troveManager.Troves(alice, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
		...
		const alice_UDebt_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
		assert.isAtMost(th.getDifference(bob_UDebt, expected_B_debt), 10000)

		assert.isAtMost(th.getDifference(bob_Coll_Asset, expected_B_coll_Asset), 1000)
		assert.isAtMost(th.getDifference(bob_UDebt_Asset, expected_B_debt_Asset), 10000)
		...
		assert.isAtMost(th.getDifference(alice_UDebt, expected_A_debt), 10000)

		assert.isAtMost(th.getDifference(alice_Coll_Asset, expected_A_coll_Asset), 1000)
		assert.isAtMost(th.getDifference(alice_UDebt_Asset, expected_A_debt_Asset), 10000)
	...
	it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
		...
		const alice_ETHReward_1_Asset = await troveManager.getPendingAssetReward(
			erc20.address,
			alice
		)
		const bob_ETHReward_1_Asset = await troveManager.getPendingAssetReward(erc20.address, bob)
		const carol_ETHReward_1_Asset = await troveManager.getPendingAssetReward(
			erc20.address,
			carol
		)
		...
		await borrowerOperations.addColl(ZERO_ADDRESS, 0, carol, carol, {
			from: carol,
			value: dec(1, "ether"),
		})
    ...
    */
	...
	it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
		...
		// Expect Bob now holds all Ether and UDebt in the system: 2.5 Ether and 300 U
	...
	it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
		...
		/* Bob rewards:
     L1: 0.4975 ETH, 55 U
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 UDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 UDebt 

     Alice rewards:
    L1 0.4975, 55 U
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 UDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 U Debt

    totalColl: 3.49 ETH
    totalDebt 380 U (Includes 50 in each trove for gas compensation)
    */
		const bob_Coll = (await troveManager.Troves(bob, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
			.add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, bob))
			.toString()
		...
		const entireSystemDebt = (await activePool.getUDebt(ZERO_ADDRESS)).add(
			await defaultPool.getUDebt(ZERO_ADDRESS)
		)
		const entireSystemDebt_Asset = (await activePool.getUDebt(erc20.address)).add(
	...
	it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
		...
		await borrowerOperations.withdrawColl(ZERO_ADDRESS, C_withdrawnColl, carol, carol, {
			from: carol,
		})
    ...
    */
    ...
    bringing them to 1.495, 1.495, 1990.51 total coll each. */
		const withdrawnColl = toBN(dec(500, "finney"))
		await borrowerOperations.withdrawColl(ZERO_ADDRESS, withdrawnColl, alice, alice, {
			from: alice,
		})
		await borrowerOperations.withdrawColl(ZERO_ADDRESS, withdrawnColl, bob, bob, { from: bob })
		await borrowerOperations.withdrawColl(ZERO_ADDRESS, withdrawnColl, carol, carol, {
			from: carol,
		})
		...
		const alice_Coll_1_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, alice))
			.toString()

		const bob_Coll_1_Asset = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, bob))
			.toString()

		const carol_Coll_1_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, carol))
			.toString()
		...
		assert.isAtMost(
			th.getDifference(
				alice_Coll_1,
				A_coll.add(th.applyLiquidationFee(D_coll).mul(A_coll).div(totalColl_1)).sub(
					withdrawnColl
				)
			),
			1000
		...
		assert.isAtMost(
			th.getDifference(
				bob_Coll_1,
				B_coll.add(th.applyLiquidationFee(D_coll).mul(B_coll).div(totalColl_1)).sub(
					withdrawnColl
				)
			),
			1000
		...
		assert.isAtMost(
			th.getDifference(
				carol_Coll_1,
				C_coll.add(th.applyLiquidationFee(D_coll).mul(C_coll).div(totalColl_1)).sub(
					withdrawnColl
				)
			),
			1000
		...
		assert.isAtMost(
			th.getDifference(
				alice_Coll_1_Asset,
				A_coll_Asset.add(
					th.applyLiquidationFee(D_coll_Asset).mul(A_coll_Asset).div(totalColl_1_Asset)
				).sub(withdrawnColl)
			),
			1000
		)
		assert.isAtMost(
			th.getDifference(
				bob_Coll_1_Asset,
				B_coll_Asset.add(
					th.applyLiquidationFee(D_coll_Asset).mul(B_coll_Asset).div(totalColl_1_Asset)
				).sub(withdrawnColl)
			),
			1000
		)
		assert.isAtMost(
			th.getDifference(
				carol_Coll_1_Asset,
				C_coll_Asset.add(
					th.applyLiquidationFee(D_coll_Asset).mul(C_coll_Asset).div(totalColl_1_Asset)
				).sub(withdrawnColl)
			),
			1000
		)
    ...
    */
		...
		const alice_Coll_2_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, alice))
			.toString()

		const bob_Coll_2_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, bob))
			.toString()

		const carol_Coll_2_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
			.add(await troveManager.getPendingAssetReward(erc20.address, carol))
			.toString()
	...
	it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(ZERO_ADDRESS, bob),
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(ZERO_ADDRESS, carol),
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(erc20.address, bob),
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(erc20.address, carol),
		...
		const B_collAfterL1_Asset = B_coll_Asset.add(B_pendingRewardsAfterL1_Asset).add(
			B_addedColl
		)
		const C_collAfterL1_Asset = C_coll_Asset.add(C_pendingRewardsAfterL1_Asset).sub(
			C_withdrawnColl
		)
		...
		const D_pendingRewardsAfterL2_Asset = D_coll_Asset.mul(
			th.applyLiquidationFee(B_collAfterL1_Asset)
		).div(C_collAfterL1_Asset.add(D_coll_Asset))
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(ZERO_ADDRESS, dennis),
		...
		assert.isAtMost(
			th.getDifference(
				await troveManager.getPendingAssetReward(erc20.address, dennis),
		...
		const totalStakesSnapshotAfterL2_Asset = totalStakesSnapshotAfterL1_Asset
			.add(
				D_coll_Asset.mul(totalStakesSnapshotAfterL1_Asset).div(
					totalCollateralSnapshotAfterL1_Asset
				)
			)
			.sub(B_coll_Asset)
			.sub(
				C_withdrawnColl.mul(totalStakesSnapshotAfterL1_Asset).div(
					totalCollateralSnapshotAfterL1_Asset
				)
			)
		...
		await borrowerOperations.addColl(ZERO_ADDRESS, 0, dennis, dennis, {
			from: dennis,
			value: D_addedColl,
		})
		...
		const carol_rawColl = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].toString()
		...
		const dennis_rawColl = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].toString()
		...
		const erin_rawColl = (await troveManager.Troves(erin, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].toString()
		const erin_pendingETHReward = (
			await troveManager.getPendingAssetReward(ZERO_ADDRESS, erin)
		).toString()

		const carol_rawColl_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		].toString()
		...
		const dennis_rawColl_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_COLL_INDEX
		].toString()
		...
		const erin_rawColl_Asset = (await troveManager.Troves(erin, erc20.address))[
			th.TROVE_COLL_INDEX
		].toString()
		const erin_pendingETHReward_Asset = (
			await troveManager.getPendingAssetReward(erc20.address, erin)
		).toString()
		...
		const D_collAfterL2_Asset = D_coll_Asset.add(D_pendingRewardsAfterL2_Asset).add(
			D_addedColl
		)
		...
		assert.isAtMost(
			th.getDifference(
				dennis_pendingETHReward_Asset,
				D_collAfterL3_Asset.sub(D_collAfterL2_Asset)
			),
			1000000
		)
		assert.isAtMost(
		...
		assert.isAtMost(
			th.getDifference(
				activeColl_Asset,
				C_collAfterL1_Asset.add(D_collAfterL2_Asset.add(E_coll_Asset))
			),
			1000000
		)
		assert.isAtMost(
		...
		th.assertIsApproximatelyEqual(
			totalCollateralSnapshot_Asset,
			totalCollateralSnapshotAfterL3_Asset
		)

		// check U gas compensation
		assert.equal(
			(await vstToken.balanceOf(owner)).toString(),
			toBN(dec(600, 18)).mul(toBN(2)).toString()
		)
	})
    ...
    */
		...
		const carol_Stake = (await troveManager.Troves(carol, ZERO_ADDRESS))[
		...
		const dennis_Stake = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_STAKE_INDEX
		].toString()
		...
		const erin_Stake = (await troveManager.Troves(erin, ZERO_ADDRESS))[
			th.TROVE_STAKE_INDEX
		].toString()
		...
		const carol_Stake_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_STAKE_INDEX
		].toString()
		...
		const dennis_Stake_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_STAKE_INDEX
		].toString()
		...
		const erin_Stake_Asset = (await troveManager.Troves(erin, erc20.address))[
			th.TROVE_STAKE_INDEX
		].toString()
