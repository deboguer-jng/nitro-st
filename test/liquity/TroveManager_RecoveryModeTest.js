...
contract("TroveManager - in Recovery Mode", async accounts => {
	...
	it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
		...
		// price drops to 1ETH:150U, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
	...
	it("checkRecoveryMode(): Returns true if TCR stays less than CCR", async () => {
		...
		await borrowerOperations.addColl(ZERO_ADDRESS, 0, alice, alice, {
			from: alice,
			value: "1",
		})
	...
	it("checkRecoveryMode(): returns false if TCR stays above CCR", async () => {
		...
		await borrowerOperations.withdrawColl(ZERO_ADDRESS, _1_Ether, alice, alice, {
			from: alice,
		})
	...
	it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
		...
		const bob_Stake_Before = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_STAKE_INDEX
		]
		const totalStakes_Before = await troveManager.totalStakes(ZERO_ADDRESS)

		const bob_Stake_Before_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_STAKE_INDEX
		]
		...
		const bob_Stake_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_STAKE_INDEX
		]
		const totalStakes_After = await troveManager.totalStakes(ZERO_ADDRESS)

		const bob_Stake_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_STAKE_INDEX
		]
	...
	it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
		...
		// --- TEST ---
		// price drops to 1ETH:100U, reducing TCR below 150%, and all Troves below 100% ICR
		await priceFeed.setPrice("100000000000000000000")
		...
		const totalStakesSnaphot_After_Asset = await troveManager.totalStakesSnapshot(
			erc20.address
		)
		const totalCollateralSnapshot_After_Asset = await troveManager.totalCollateralSnapshot(
			erc20.address
		)
		...
		assert.isAtMost(
			th.getDifference(
				...
				A_coll_Asset.add(A_reward_Asset).add(
					th.applyLiquidationFee(B_coll_Asset.add(B_reward_Asset))
				)
			),
			1000
		) // 3 + 4.5*0.995 + 1.5*0.995^2
	...
	it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
		...
		const bob_TroveStatus_Before = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_STATUS_INDEX
		]
		const bob_Trove_isInSortedList_Before = await sortedTroves.contains(ZERO_ADDRESS, bob)

		const bob_TroveStatus_Before_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_STATUS_INDEX
		]
		const bob_Trove_isInSortedList_Before_Asset = await sortedTroves.contains(
			erc20.address,
			bob
		)
		...
		const bob_TroveStatus_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_STATUS_INDEX
		]
		const bob_Trove_isInSortedList_After = await sortedTroves.contains(ZERO_ADDRESS, bob)

		const bob_TroveStatus_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_STATUS_INDEX
		]
		const bob_Trove_isInSortedList_After_Asset = await sortedTroves.contains(
			erc20.address,
			bob
		)
	...
	it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
		// --- SETUP ---
		//  Bob withdraws up to 2000 U of debt, bringing his ICR to 210%
		...
		// --- TEST ---
		// price drops to 1ETH:100U, reducing TCR to 117%
		await priceFeed.setPrice("100000000000000000000")
	...
	it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
		...
		//  Bob withdraws up to 20000 U of debt, bringing his ICR to 210%
		const { collateral: A_coll } = await openTrove({
			ICR: toBN(dec(150, 16)),
			extraParams: { from: alice },
		})
    ...
    but his collateral should remain in the system (*0.995). */
		...
		const totalCollateralSnapshot_3_Asset = await troveManager.totalCollateralSnapshot(
			erc20.address
		)
	...
	it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
		...
		// Alice deposits 390U to the Stability Pool
		await stabilityPool.provideToSP(spDeposit, { from: alice })
		await stabilityPoolERC20.provideToSP(spDeposit, { from: alice })
		...
		// check pool U before liquidation
		const stabilityPoolU_Before = (await stabilityPool.getTotalUDeposits()).toString()
		assert.equal(stabilityPoolU_Before, "390000000000000000000")

		const stabilityPoolU_Before_Asset = (
			await stabilityPoolERC20.getTotalUDeposits()
		).toString()
		assert.equal(stabilityPoolU_Before_Asset, "390000000000000000000")
		...
		/* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 U.
    
    With 390 U in the StabilityPool, 390 U should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    ULoss: 390U
    AssetGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 U and 4.074525 ether, the remainders - 1610 U and 16.820475 ether - should be redistributed to all active Troves.
   */
		// Liquidate Bob
		await troveManager.liquidate(ZERO_ADDRESS, bob, { from: owner })
		await troveManager.liquidate(erc20.address, bob, { from: owner })

		const aliceDeposit = await stabilityPool.getCompoundedUDeposit(alice)
		...
		const aliceDeposit_Asset = await stabilityPoolERC20.getCompoundedUDeposit(alice)
		...
		/* Now, check redistribution to active Troves. Remainders of 1610 U and 16.82 ether are distributed.
    ...
    L_UDebt = 1610 / 6 = 268.333 U
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
		const L_UDebt = (await troveManager.L_UDebts(ZERO_ADDRESS)).toString()
		const L_ETH = (await troveManager.L_ASSETS(ZERO_ADDRESS)).toString()

		const L_UDebt_Asset = (await troveManager.L_UDebts(erc20.address)).toString()
	...
	it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
		...
		const dennis_ICR = (
			await troveManager.getCurrentICR(ZERO_ADDRESS, dennis, price)
		).toString()

		const bob_ICR_Asset = (
			await troveManager.getCurrentICR(erc20.address, bob, price)
		).toString()
		const alice_ICR_Asset = (
			await troveManager.getCurrentICR(erc20.address, alice, price)
		).toString()
		const dennis_ICR_Asset = (
			await troveManager.getCurrentICR(erc20.address, dennis, price)
		).toString()
		...
		assert.equal(L_UDebt, "0")
		assert.equal(L_ETH, "0")
		assert.equal(L_UDebt_Asset, "0")
	...
	// --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool U is GREATER THAN liquidated debt ---

	it("liquidate(), with 110% < ICR < TCR, and StabilityPool U > debt to liquidate: offsets the trove entirely with the pool", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(240, 16)),
			extraUAmount: dec(250, 18),
			extraParams: { from: bob },
		})
		await openTrove({
			ICR: toBN(dec(266, 16)),
		...
		// Alice deposits U in the Stability Pool
		...
		/* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 U, Alice sole depositor.
    As liquidated debt (250 U) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240U
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
		const aliceExpectedDeposit = await stabilityPool.getCompoundedUDeposit(alice)
		const aliceExpectedETHGain = await stabilityPool.getDepositorAssetGain(alice)
		const aliceExpectedDeposit_Asset = await stabilityPoolERC20.getCompoundedUDeposit(alice)
		...
		assert.isAtMost(
			th.getDifference(
				aliceExpectedDeposit_Asset.toString(),
				spDeposit.sub(B_totalDebt_Asset)
			),
			2000
		)
		assert.isAtMost(
	...
	it("liquidate(), with ICR% = 110 < TCR, and StabilityPool U > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 220%. Bob has lowest ICR.
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraUAmount: B_totalDebt_Asset,
			extraParams: { from: alice },
		})
		await openTrove({
			asset: erc20.address,
    ...
    */
	...
	it("liquidate(), with  110% < ICR < TCR, and StabilityPool U > debt to liquidate: removes stake and updates totalStakes", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
		...
		assert.equal(bob_Stake_Before_Asset.toString(), B_coll_Asset)
		assert.equal(
			totalStakes_Before_Asset.toString(),
		...
		const bob_ICR_Asset = await troveManager.getCurrentICR(erc20.address, bob, price)
		assert.isTrue(
			bob_ICR_Asset.gt(mv._MCR) && bob_ICR_Asset.lt(await th.getTCR(contracts, erc20.address))
		)
	...
	it("liquidate(), with  110% < ICR < TCR, and StabilityPool U > debt to liquidate: updates system snapshots", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { totalDebt: B_totalDebt } = await openTrove({
		...
		assert.equal(
			totalCollateralSnapshot_After_Asset.toString(),
			A_coll_Asset.add(D_coll_Asset)
		)
	})

	it("liquidate(), with 110% < ICR < TCR, and StabilityPool U > debt to liquidate: closes the Trove", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool U > debt to liquidate: can liquidate troves out of order", async () => {
		// taking out 1000 U, CR of 200%
		...
		assert.equal(
			(await troveManager.Troves(dennis, erc20.address))[th.TROVE_STATUS_INDEX],
			"3"
		)
		...
		assert.isTrue(
			dennis_balanceAfter_Asset.eq(
				dennis_balanceBefore_Asset.add(
					th.toBN(dennis_remainingCollateral_Asset).div(toBN(10 ** 10))
				)
			)
		)
		...
		th.assertIsApproximatelyEqual(
			...
			carol_balanceBefore_Asset.add(
				th.toBN(carol_remainingCollateral_Asset).div(toBN(10 ** 10))
			)
		)
	})

	/* --- liquidate() applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool 
  U is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: Trove remains active", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		await openTrove({
			ICR: toBN(dec(266, 16)),
			extraUAmount: dec(1500, 18),
			extraParams: { from: alice },
		})
		await openTrove({
			ICR: toBN(dec(240, 16)),
		...
		// Alice deposits 1490 U in the Stability Pool
		...
		/* Since the pool only contains 100 U, and Bob's pre-liquidation debt was 250 U,
	...
	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: Trove remains in TroveOwners array", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		await openTrove({
		...
		// Alice deposits 100 U in the Stability Pool
    ...
    expect Bob's trove to only be partially offset, and remain active after liquidation */
		...
		const idxOnStruct = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		].toString()
		assert.equal(addressIdx.toString(), idxOnStruct)

		const arrayLength_Asset = (
		...
		// Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
		const idxOnStruct_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_ARRAY_INDEX
		].toString()
		assert.equal(addressIdx_Asset.toString(), idxOnStruct_Asset)
	})

	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: nothing happens", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { collateral: A_coll } = await openTrove({
		...
		/*  Since Bob's debt (250 U) is larger than all U in the Stability Pool, Liquidation won’t happen
    ...
    */
		...
		assert.equal(
			totalStakes_After_Asset.toString(),
			A_coll_Asset.add(B_coll_Asset).add(D_coll_Asset)
		)
	})

	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: updates system shapshots", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		await openTrove({
	...
	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		await openTrove({
    ...
    Redistribution rewards-per-unit-staked should be zero. */
		...
		const L_UDebt_After = (await troveManager.L_UDebts(ZERO_ADDRESS)).toString()
		const L_ETH_After = (await troveManager.L_ASSETS(ZERO_ADDRESS)).toString()

		const L_UDebt_After_Asset = (await troveManager.L_UDebts(erc20.address)).toString()
		const L_ETH_After_Asset = (await troveManager.L_ASSETS(erc20.address)).toString()

		assert.equal(L_UDebt_After, "0")
		assert.equal(L_ETH_After, "0")
		assert.equal(L_UDebt_After_Asset, "0")
		assert.equal(L_ETH_After_Asset, "0")
	})

	it("liquidate(), with ICR > 110%, and StabilityPool U < liquidated debt: ICR of non liquidated trove does not change", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		// Carol withdraws up to debt of 240 U, -> ICR of 250%.
		await openTrove({
			ICR: toBN(dec(266, 16)),
			extraUAmount: dec(1500, 18),
			extraParams: { from: alice },
		...
		const bob_ICR_Before = (
			await troveManager.getCurrentICR(ZERO_ADDRESS, bob, price)
		).toString()
		const carol_ICR_Before = (
			await troveManager.getCurrentICR(ZERO_ADDRESS, carol, price)
		).toString()
		...
		const carol_ICR_Before_Asset = (
			await troveManager.getCurrentICR(erc20.address, carol, price)
		).toString()
		...
		const bob_Coll_Before_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const bob_Debt_Before_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		//Check SP U has been completely emptied
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), dec(100, 18))
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), dec(100, 18))
		...
		const bob_Coll_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const bob_Debt_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		// Alice provides another 50 U to pool
		...
		const carol_Coll_Before = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		]
		const carol_Debt_Before = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		]

		const carol_Coll_Before_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const carol_Debt_Before_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		await assertRevert(
			troveManager.liquidate(ZERO_ADDRESS, carol),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidate(erc20.address, carol),
			"TroveManager: nothing to liquidate"
		)

		//Check SP U has been completely emptied
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), dec(150, 18))
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), dec(150, 18))

		// Check Carol's collateral and debt remains the same
		const carol_Coll_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		]
		const carol_Debt_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		]
		const carol_Coll_After_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const carol_Debt_After_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
	...
	it("liquidate() with ICR > 110%, and StabilityPool U < liquidated debt: total liquidated coll and debt is correct", async () => {
		// Whale provides 50 U to the SP
		await openTrove({
			ICR: toBN(dec(300, 16)),
			extraUAmount: dec(50, 18),
			extraParams: { from: whale },
		})
		await openTrove({
			asset: erc20.address,
		...
		assert.isTrue(
			ICR_A_Asset.gt(mv._MCR) && ICR_A_Asset.lt(await th.getTCR(contracts, erc20.address))
		)
		...
		const changeInEntireSystemColl_Asset = entireSystemCollBefore_Asset.sub(
			entireSystemCollAfter_Asset
		)
		const changeInEntireSystemDebt_Asset = entireSystemDebtBefore_Asset.sub(
			entireSystemDebtAfter_Asset
		)
	...
	it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
		// Alice creates a single trove with 0.62 ETH and a debt of 62 U, and provides 10 U to SP
		await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: alice } })
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
	...
	it("liquidate(): does nothing if trove has >= 110% ICR and the Stability Pool is empty", async () => {
		...
		// Confirm SP is empty
		const UinSP = (await stabilityPool.getTotalUDeposits()).toString()
		assert.equal(UinSP, "0")

		const UinSP_Asset = (await stabilityPoolERC20.getTotalUDeposits()).toString()
		assert.equal(UinSP_Asset, "0")
	...
	it("liquidate(): does nothing if trove ICR >= TCR, and SP covers trove's debt", async () => {
		...
		// Try to liquidate A
		await assertRevert(
			troveManager.liquidate(ZERO_ADDRESS, A),
			"TroveManager: nothing to liquidate"
		)
		await assertRevert(
			troveManager.liquidate(erc20.address, A),
			"TroveManager: nothing to liquidate"
		)
    ...
    check that Bob's raw coll and debt has not changed, and that his "raw" ICR is above the MCR */
		...
		/*  Since there is 0 U in the stability Pool, A, with ICR >110%, should stay active.
	...
	it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
		...
		assert.isAtMost(
			th.getDifference(
				dennis_ETHGain_Before_Asset,
				th.applyLiquidationFee(C_coll_Asset.div(toBN(10 ** 10)))
			),
			1000
		)
	...
	it("liquidate(): does not alter the liquidated user's token balance", async () => {
		...
		assert.equal((await vstToken.balanceOf(alice)).toString(), A_UAmount.add(A_UAmount_Asset))
		assert.equal((await vstToken.balanceOf(bob)).toString(), B_UAmount.add(B_UAmount_Asset))
		assert.equal((await vstToken.balanceOf(carol)).toString(), C_UAmount.add(C_UAmount_Asset))
	...
	it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 U of debt, resulting in ICRs of 266%.
		// Bob withdraws up to 480 U of debt, resulting in ICR of 240%. Bob has lowest ICR.
		const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
			ICR: toBN(dec(240, 16)),
			extraUAmount: dec(480, 18),
			extraParams: { from: bob },
		})
		await openTrove({
			ICR: toBN(dec(266, 16)),
		...
		// Bob re-opens the trove, price 200, total debt 80 U, ICR = 120% (lowest one)
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraUAmount: B_netDebt_2_Asset,
			extraParams: { from: dennis },
		})
	...
	it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
		// --- SETUP ---
		// Bob withdraws up to 90 U of debt, resulting in ICR of 222%
		const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
			ICR: toBN(dec(222, 16)),
			extraUAmount: dec(90, 18),
			extraParams: { from: bob },
		})
		// Dennis withdraws to 150 U of debt, resulting in ICRs of 266%.
		await openTrove({
			ICR: toBN(dec(266, 16)),
			extraUAmount: B_netDebt,
			extraParams: { from: dennis },
		})
		...
		// Bob re-opens the trove, price 200, total debt 250 U, ICR = 240% (lowest one)
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraUAmount: B_totalDebt_2_Asset,
			extraParams: { from: alice },
		})
	...
	it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
		...
		// Everyone withdraws some U from their Trove, resulting in different ICRs
		...
		// Alice deposits U to Stability Pool
		...
		// price drops
		// price drops to 1ETH:90U, reducing TCR below 150%
    ...
    */
	...
	it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(400, 16)),
			extraUAmount: liquidationAmount_Asset,
			extraParams: { from: alice },
		})
		...
		// price drops to 1ETH:85U, reducing TCR below 150%
   ...
   (see calculations in Google Sheet)
	...
	it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
		...
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, freddy, price)).lte(mv._MCR)
		)
		...
		assert.equal(
			(await troveManager.Troves(freddy, ZERO_ADDRESS))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
		...
		assert.equal(
			(await troveManager.Troves(freddy, erc20.address))[th.TROVE_STATUS_INDEX].toString(),
			"3"
		)
	})
	...
	it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
		...
		await openTrove({
			ICR: toBN(dec(198, 16)),
			extraUAmount: dec(101, 18),
			extraParams: { from: defaulter_1 },
		})
		await openTrove({
			ICR: toBN(dec(184, 16)),
			extraUAmount: dec(217, 18),
			extraParams: { from: defaulter_2 },
		})
		await openTrove({
			ICR: toBN(dec(183, 16)),
			extraUAmount: dec(328, 18),
			extraParams: { from: defaulter_3 },
		})
		await openTrove({
			ICR: toBN(dec(186, 16)),
			extraUAmount: dec(431, 18),
		...
		assert.isTrue(
			await th.ICRbetween100and110(defaulter_1, troveManager, price, erc20.address)
		)
		assert.isTrue(
			await th.ICRbetween100and110(defaulter_2, troveManager, price, erc20.address)
		)
		assert.isTrue(
			await th.ICRbetween100and110(defaulter_3, troveManager, price, erc20.address)
		)
		assert.isTrue(
			await th.ICRbetween100and110(defaulter_4, troveManager, price, erc20.address)
		)
		...
		// Check Stability Pool has 500 U
	...
	it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
		...
		assert.isAtMost(
			th.getDifference(
				TCR_Before_Asset,
				totalCollBefore_Asset.mul(price).div(totalDebtBefore_Asset)
			),
			1000
		)
		...
		const totalCollAfter_Asset = W_coll_Asset.add(A_coll_Asset)
			...
			.add(
				th.applyLiquidationFee(
					d1_coll_Asset.add(d2_coll_Asset).add(d3_coll_Asset).add(d4_coll_Asset)
				)
			)
		...
		assert.isAtMost(
			th.getDifference(
				TCR_After_Asset,
				totalCollAfter_Asset.mul(price).div(totalDebtAfter_Asset)
			),
			1000
		)
   ...
   (because his pending rewards bring his ICR < MCR) */
	...
	it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
		...
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, freddy, price)).gte(mv._MCR)
		)
		...
		// Confirm U in Stability Pool
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), spDeposit.toString())
		assert.equal(
			(await stabilityPoolERC20.getTotalUDeposits()).toString(),
			spDeposit_Asset.toString()
		)
		...
		th.assertIsApproximatelyEqual(
			...
			freddy_balanceBefore_Asset.add(
				th.toBN(freddy_remainingCollateral_Asset.div(toBN(10 ** 10)))
			)
		)
		...
		th.assertIsApproximatelyEqual(
			...
			greta_balanceBefore_Asset.add(
				th.toBN(greta_remainingCollateral_Asset).div(toBN(10 ** 10))
			)
		)
	})
	...
	it("liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
		...
		const spDeposit_Asset = F_totalDebt_Asset.add(G_totalDebt_Asset).add(
			A_totalDebt_Asset.div(toBN(2))
		)
		await openTrove({
			ICR: toBN(dec(285, 16)),
			extraUAmount: spDeposit,
		...
		const entireColl_A = (await troveManager.Troves(alice, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, alice))
		const entireDebt_A = (await troveManager.Troves(alice, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(ZERO_ADDRESS, alice))

		const entireColl_A_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(erc20.address, alice))
		const entireDebt_A_Asset = (await troveManager.Troves(alice, erc20.address))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(erc20.address, alice))
	...
	it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
		...
		const { UAmount: UAmountD } = await openTrove({
			ICR: toBN(dec(200, 16)),
			extraParams: { from: dennis },
		})
		const { UAmount: UAmountE } = await openTrove({
			ICR: toBN(dec(133, 16)),
			extraParams: { from: erin },
		})
		const { UAmount: UAmountF } = await openTrove({
			ICR: toBN(dec(111, 16)),
			extraParams: { from: freddy },
		})

		const { UAmount: UAmountD_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraParams: { from: dennis },
		})
		const { UAmount: UAmountE_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(133, 16)),
			extraParams: { from: erin },
		})
		const { UAmount: UAmountF_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(111, 16)),
			extraParams: { from: freddy },
		})
		...
		assert.equal((await vstToken.balanceOf(dennis)).toString(), UAmountD.add(UAmountD_Asset))
		assert.equal((await vstToken.balanceOf(erin)).toString(), UAmountE.add(UAmountE_Asset))
		assert.equal((await vstToken.balanceOf(freddy)).toString(), UAmountF.add(UAmountF_Asset))
	...
	it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
		// Whale provides U to the SP
		const { UAmount: W_UAmount } = await openTrove({
			ICR: toBN(dec(300, 16)),
			extraUAmount: dec(4000, 18),
			extraParams: { from: whale },
		})
		const { UAmount: W_UAmount_Asset } = await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(300, 16)),
			extraUAmount: dec(4000, 18),
			extraParams: { from: whale },
		...
		await stabilityPool.provideToSP(W_UAmount, { from: whale })
		await stabilityPoolERC20.provideToSP(W_UAmount_Asset, { from: whale })
		...
		await stabilityPool.provideToSP(A_UAmount, { from: alice })
		await stabilityPool.provideToSP(B_UAmount, { from: bob })
		await stabilityPoolERC20.provideToSP(A_UAmount_Asset, { from: alice })
		await stabilityPoolERC20.provideToSP(B_UAmount_Asset, { from: bob })

		const totalDeposit = W_UAmount.add(A_UAmount).add(B_UAmount)
		const totalDeposit_Asset = W_UAmount_Asset.add(A_UAmount_Asset).add(B_UAmount_Asset)
		...
		// Check U in Pool
		assert.equal((await stabilityPool.getTotalUDeposits()).toString(), totalDeposit)
		assert.equal((await stabilityPoolERC20.getTotalUDeposits()).toString(), totalDeposit_Asset)
    ...
    Total U in Pool: 680 U

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 U
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale U Loss: 500 * (400/680) = 294.12 U
    Alice U Loss:  500 *(40/680) = 29.41 U
    Bob U Loss: 500 * (240/680) = 176.47 U

    Whale remaining deposit: (400 - 294.12) = 105.88 U
    Alice remaining deposit: (40 - 29.41) = 10.59 U
    Bob remaining deposit: (240 - 176.47) = 63.53 U

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 U
    Total ETH gain: 5*0.995 ETH */

		// Check remaining U Deposits and ETH gain, for whale and depositors whose troves were liquidated
		const whale_Deposit_After = (await stabilityPool.getCompoundedUDeposit(whale)).toString()
		...
		const whale_Deposit_After_Asset = (
			await stabilityPoolERC20.getCompoundedUDeposit(whale)
		).toString()
		...
		const whale_ETHGain_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(whale)
		).toString()
		...
		assert.isAtMost(
			th.getDifference(
				whale_Deposit_After,
				W_UAmount.sub(liquidatedDebt.mul(W_UAmount).div(totalDeposit))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				alice_Deposit_After,
				A_UAmount.sub(liquidatedDebt.mul(A_UAmount).div(totalDeposit))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				bob_Deposit_After,
				B_UAmount.sub(liquidatedDebt.mul(B_UAmount).div(totalDeposit))
			),
			100000
		)
		...
		assert.isAtMost(
			th.getDifference(
				...
				W_UAmount_Asset.sub(liquidatedDebt_Asset.mul(W_UAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				...
				A_UAmount_Asset.sub(liquidatedDebt_Asset.mul(A_UAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				...
				B_UAmount_Asset.sub(liquidatedDebt_Asset.mul(B_UAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		...
		assert.isAtMost(
			th.getDifference(
				whale_ETHGain,
				th.applyLiquidationFee(liquidatedColl).mul(W_UAmount).div(totalDeposit)
			),
			2000
		)
		assert.isAtMost(
			th.getDifference(
				alice_ETHGain,
				th.applyLiquidationFee(liquidatedColl).mul(A_UAmount).div(totalDeposit)
			),
			2000
		)
		assert.isAtMost(
			th.getDifference(
				bob_ETHGain,
				th.applyLiquidationFee(liquidatedColl).mul(B_UAmount).div(totalDeposit)
			),
			2000
		)
		...
		assert.isAtMost(th.getDifference(total_UinSP, totalDeposit.sub(liquidatedDebt)), 1000)
		assert.isAtMost(
		...
		assert.isAtMost(
			th.getDifference(total_UinSP_Asset, totalDeposit_Asset.sub(liquidatedDebt_Asset)),
			1000
		)
		assert.isAtMost(
	...
	it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
		...
		// Check U and ETH in Pool  before
		const UinSP_Before = (await stabilityPool.getTotalUDeposits()).toString()
		const ETHinSP_Before = (await stabilityPool.getAssetBalance()).toString()
		const UinSP_Before_Asset = (await stabilityPoolERC20.getTotalUDeposits()).toString()
		const ETHinSP_Before_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

		assert.equal(UinSP_Before, dec(800, 18))
		assert.equal(ETHinSP_Before, "0")
		assert.equal(UinSP_Before_Asset, dec(800, 18))
		assert.equal(ETHinSP_Before_Asset, "0")

		// *** Check A, B, C ICRs < 100
		assert.isTrue(
			(await troveManager.getCurrentICR(ZERO_ADDRESS, alice, price)).lte(mv._ICR100)
		)
		assert.isTrue((await troveManager.getCurrentICR(ZERO_ADDRESS, bob, price)).lte(mv._ICR100))
		assert.isTrue(
			(await troveManager.getCurrentICR(ZERO_ADDRESS, carol, price)).lte(mv._ICR100)
		)

		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, alice, price)).lte(mv._ICR100)
		)
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, bob, price)).lte(mv._ICR100)
		)
		assert.isTrue(
			(await troveManager.getCurrentICR(erc20.address, carol, price)).lte(mv._ICR100)
		)
		...
		// Check U and ETH in Pool after
		const UinSP_After = (await stabilityPool.getTotalUDeposits()).toString()
		const ETHinSP_After = (await stabilityPool.getAssetBalance()).toString()
		const UinSP_After_Asset = (await stabilityPoolERC20.getTotalUDeposits()).toString()
		const ETHinSP_After_Asset = (await stabilityPoolERC20.getAssetBalance()).toString()

		assert.equal(UinSP_Before, UinSP_After)
		assert.equal(ETHinSP_Before, ETHinSP_After)
		assert.equal(UinSP_Before_Asset, UinSP_After_Asset)
		...
		const whale_ETHGain_After_Asset = (
			await stabilityPoolERC20.getDepositorAssetGain(whale)
		).toString()
	...
	it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
		...
		const spDeposit_Asset = A_totalDebt_Asset.add(B_totalDebt_Asset).add(
			C_totalDebt_Asset.div(toBN(2))
		)
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(300, 16)),
    ...
    That leaves 50 U in the Pool to absorb exactly half of Carol's debt (100) */
		await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
		await troveManager.liquidateTroves(erc20.address, 10)
    ...
    That leaves 50 U in the Pool to absorb exactly half of Carol's debt (100) */
		...
		// Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
		const idxOnStruct = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_ARRAY_INDEX
		].toString()
		assert.equal(addressIdx.toString(), idxOnStruct)
		...
		// Check TroveOwners idx on trove struct == idx of address found in TroveOwners array
		const idxOnStruct_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_ARRAY_INDEX
		].toString()
		assert.equal(addressIdx_Asset.toString(), idxOnStruct_Asset)
	})
	...
	it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraUAmount: D_totalDebt_Asset,
			extraParams: { from: carol },
		})
		await openTrove({
			asset: erc20.address,
     ...
     That leaves 97 U in the Pool that won’t be enough to absorb Carol,
	...
	it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
		...
		/* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 U in the Pool that won’t be enough to absorb any other trove */
		await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
		await troveManager.liquidateTroves(erc20.address, 10)

		// Expect system debt reduced by 203 U and system coll 2.3 ETH
		const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
		const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
		const entireSystemCollAfter_Asset = await troveManager.getEntireSystemColl(erc20.address)
		const entireSystemDebtAfter_Asset = await troveManager.getEntireSystemDebt(erc20.address)
	...
	it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
		...
		/* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 U in the Pool which won’t be enough for any other liquidation */
		const liquidationTx = await troveManager.liquidateTroves(ZERO_ADDRESS, 10)
		const liquidationTx_Asset = await troveManager.liquidateTroves(erc20.address, 10)

		const [liquidatedDebt, liquidatedColl, collGasComp, UGasComp] =
			th.getEmittedLiquidationValues(liquidationTx)
		const [liquidatedDebt_Asset, liquidatedColl_Asset, collGasComp_Asset, UGasComp_Asset] =
			th.getEmittedLiquidationValues(liquidationTx_Asset)
		...
		assert.equal(UGasComp.toString(), dec(400, 18))
		...
		th.assertIsApproximatelyEqual(
			liquidatedColl_Asset,
			th.applyLiquidationFee(equivalentColl_Asset)
		)
		th.assertIsApproximatelyEqual(
			collGasComp_Asset,
			equivalentColl_Asset.sub(th.applyLiquidationFee(equivalentColl_Asset))
		) // 0.5% of 283/120*1.1
		assert.equal(UGasComp_Asset.toString(), dec(400, 18))
		...
		th.assertIsApproximatelyEqual(
			...
			alice_balanceBefore_Asset.add(
				th.toBN(alice_remainingCollateral_Asset.div(toBN(10 ** 10)))
			)
		)
    ...
    */
    ...
    Only Alice should remain active - all others should be closed. */
		...
		await troveManager.batchLiquidateTroves(ZERO_ADDRESS, [
			alice,
			bob,
			carol,
			dennis,
		...
		await troveManager.batchLiquidateTroves(erc20.address, [
			alice,
			bob,
			carol,
			dennis,
    ...
    */
	...
	it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
		...
		await openTrove({
			asset: erc20.address,
			ICR: toBN(dec(426, 16)),
			extraUAmount: A_totalDebt_Asset,
			extraParams: { from: whale },
		})
    ...
    */
	...
	it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
		...
		// Expect system debt reduced by 203 U and system coll by 2 ETH
		const entireSystemCollAfter = await troveManager.getEntireSystemColl(ZERO_ADDRESS)
		const entireSystemDebtAfter = await troveManager.getEntireSystemDebt(ZERO_ADDRESS)
	...
	it("batchLiquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
		...
		const liquidationTx_Asset = await troveManager.batchLiquidateTroves(
			erc20.address,
			trovesToLiquidate
		)
	...
	it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool U > debt to liquidate: can liquidate troves out of order", async () => {
		const { totalDebt: A_totalDebt } = await openTrove({
			ICR: toBN(dec(200, 16)),
			extraParams: { from: alice },
		})
		...
		// Whale provides 1000 U to the SP
	...
	it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
		...
		const carolColl_Before = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		]
		const dennisColl_Before = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		]

		const bobColl_Before_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const carolColl_Before_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const dennisColl_Before_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		...
		const dennisDebt_After = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(ZERO_ADDRESS, dennis))
		const bobDebt_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(ZERO_ADDRESS, bob))
		const carolDebt_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(ZERO_ADDRESS, carol))

		const dennisColl_After = (await troveManager.Troves(dennis, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, dennis))
		const bobColl_After = (await troveManager.Troves(bob, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, bob))
		const carolColl_After = (await troveManager.Troves(carol, ZERO_ADDRESS))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(ZERO_ADDRESS, carol))

		const dennisDebt_After_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(erc20.address, dennis))
		const bobDebt_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(erc20.address, bob))
		const carolDebt_After_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_DEBT_INDEX
		].add(await troveManager.getPendingUDebtReward(erc20.address, carol))

		const dennisColl_After_Asset = (await troveManager.Troves(dennis, erc20.address))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(erc20.address, dennis))
		const bobColl_After_Asset = (await troveManager.Troves(bob, erc20.address))[
			th.TROVE_COLL_INDEX
		].add(await troveManager.getPendingAssetReward(erc20.address, bob))
		const carolColl_After_Asset = (await troveManager.Troves(carol, erc20.address))[
			th.TROVE_COLL_INDEX
		...
		th.assertIsApproximatelyEqual(
			th.toBN(bobDebt_Before_Asset).toString(),
			bobDebt_After_Asset.toString()
		)
		th.assertIsApproximatelyEqual(
	...
	it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
		...
		const G_collBefore_Asset = (await troveManager.Troves(G, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const G_debtBefore_Asset = (await troveManager.Troves(G, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		const H_collBefore_Asset = (await troveManager.Troves(H, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const H_debtBefore_Asset = (await troveManager.Troves(H, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		const I_collBefore_Asset = (await troveManager.Troves(I, erc20.address))[
			th.TROVE_COLL_INDEX
		]
		const I_debtBefore_Asset = (await troveManager.Troves(I, erc20.address))[
			th.TROVE_DEBT_INDEX
		]
		...
		assert.equal(
			G_collBefore.eq(await troveManager.Troves(G, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			G_debtBefore.eq(await troveManager.Troves(G, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
		)
		assert.equal(
			H_collBefore.eq(await troveManager.Troves(H, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			H_debtBefore.eq(await troveManager.Troves(H, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
		)
		assert.equal(
			I_collBefore.eq(await troveManager.Troves(I, ZERO_ADDRESS))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			I_debtBefore.eq(await troveManager.Troves(I, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]
		)

		assert.equal(
			G_collBefore_Asset.eq(await troveManager.Troves(G, erc20.address))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			G_debtBefore_Asset.eq(await troveManager.Troves(G, erc20.address))[th.TROVE_DEBT_INDEX]
		)
		assert.equal(
			H_collBefore_Asset.eq(await troveManager.Troves(H, erc20.address))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			H_debtBefore_Asset.eq(await troveManager.Troves(H, erc20.address))[th.TROVE_DEBT_INDEX]
		)
		assert.equal(
			I_collBefore_Asset.eq(await troveManager.Troves(I, erc20.address))[th.TROVE_COLL_INDEX]
		)
		assert.equal(
			I_debtBefore_Asset.eq(await troveManager.Troves(I, erc20.address))[th.TROVE_DEBT_INDEX]
		)
		...
		// Whale withdraws entire deposit, and re-deposits 132 U
		...
		// Expected Stability Pool to fully absorb B (92 U + 10 virtual debt),
		// but not E as there are not enough funds in Stability Pool

		const stabilityBefore = await stabilityPool.getTotalUDeposits()
		const dEbtBefore = (await troveManager.Troves(E, ZERO_ADDRESS))[th.TROVE_DEBT_INDEX]

		const stabilityBefore_Asset = await stabilityPoolERC20.getTotalUDeposits()
		...
		const stabilityAfter = await stabilityPool.getTotalUDeposits()

		const dEbtAfter_Asset = (await troveManager.Troves(E, erc20.address))[th.TROVE_DEBT_INDEX]
		const stabilityAfter_Asset = await stabilityPoolERC20.getTotalUDeposits()
	...
	it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
		...
		// Whale opens trove and adds 220 U to SP
		const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
		await openTrove({
			ICR: toBN(dec(285, 16)),
			extraUAmount: spDeposit,
