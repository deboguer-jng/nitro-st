...
contract(
	...
	async accounts => {
		...
		describe("Stability Pool Withdrawal", async () => {
			...
			it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(100000, 18)),
					whale,
					whale,
				...
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(alice)).toString(),
				...
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(bob)).toString(),
						toBN("6666666666666666666666").mul(toBN(2))
					),
					20000
				)
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(carol)).toString(),
						toBN("6666666666666666666666").mul(toBN(2))
					),
					20000
				)
			...
			it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
				...
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(alice)).toString(), "0"),
					10000
				)
				assert.isAtMost(th.getDifference((await UToken.balanceOf(bob)).toString(), "0"), 10000)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(carol)).toString(), "0"),
					10000
				)
			...
			it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing U", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing U", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(1000000, 18)),
					whale,
					whale,
      ...
      */
			...
			it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(bob)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(carol)).toString(), "0"),
				...
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(dennis)).toString(), "0"),
					100000
				)
			...
			it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 1m ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
      ...
      */
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(25000, 18)),
					defaulter_2,
					defaulter_2,
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(40000, 18)),
					defaulter_4,
					defaulter_4,
				...
				assert.isAtMost(
					th.getDifference(dennis_ETHWithdrawnERC20, "13169117647"),
					100000000000
				)
			})
			...
			it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 U.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				assert.isAtMost(th.getDifference((await UToken.balanceOf(bob)).toString(), "0"), 1000)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(carol)).toString(), "0"),
					1000
				)
			...
			it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
      ...
      */
			...
			it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 U. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct U deposit and ETH Gain", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
			...
			it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
				...
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(erin)).toString(),
			...
			it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
				...
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(erin)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(flyn)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(graham)).toString(), "0"),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(harriet)).toString(), "0"),
					100000
				)
      ...
      should only earn ETH from the single liquidation that cancelled with their deposit */
			...
			it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("9999999910000000000000"),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				...
				// Expect Bob to withdraw 1% of initial deposit (100 U) and all the liquidated ETH (60 ether)
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(bob)).toString(),
			...
			it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("59400000000000000000000"),
					defaulter_2,
					defaulter_2,
      ...
      */
			...
			it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(99999, 17)),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
			...
			it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("59999400000000000000000"),
					defaulter_2,
					defaulter_2,
				...
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(bob)).toString(), dec(1, 17)),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(carol)).toString(), dec(2, 17)),
					100000
				)
				assert.isAtMost(
					th.getDifference((await UToken.balanceOf(dennis)).toString(), dec(3, 17)),
					100000
				)
			...
			it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount("9999999999900000000000"),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(100, "ether") }
				)
				...
				const aliceDeposit = (await stabilityPool.getCompoundedUDeposit(alice)).toString()
				const aliceDepositERC20 = (
    ...
    */
			it("withdrawFromSP(): Several deposits of 10000 U span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
				// Whale opens Trove with 100k ETH
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
				...
				// D deposits 9999.9 U
				await UToken.transfer(dennis, toBN(dec(99999, 17)).mul(toBN(2)), { from: whale })
				...
				assert.equal(await UToken.balanceOf(alice), "0")
				assert.equal(await UToken.balanceOf(alice), "0")
				assert.equal(await UToken.balanceOf(alice), "0")
				// D should withdraw around 0.9999 U, since his deposit of 9999.9 was reduced by a factor of 1e-5
				assert.isAtMost(
					th.getDifference(
						(await UToken.balanceOf(dennis)).toString(),
						toBN(dec(99999, 12)).mul(toBN(2))
					),
					100000
				)
			...
			it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24100, 18)),
					defaulter_1,
					defaulter_1,
					{ from: defaulter_1, value: dec(200, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24300, 18)),
					defaulter_2,
					defaulter_2,
					{ from: defaulter_2, value: dec(200, "ether") }
				)
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(24500, 18)),
					defaulter_3,
					defaulter_3,
					{ from: defaulter_3, value: dec(200, "ether") }
				)
			...
			it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
				...
				// Defaulters 1-5 each withdraw up to debt of 9999.9999999 U
				await borrowerOperations.openTrove(
					ZERO_ADDRESS,
					0,
					th._100pct,
				...
				// E deposits 9999.9 U
				await UToken.transfer(erin, dec(99999, 17), { from: whale })
				...
				const txL5ERC20 = await troveManager.liquidate(erc20.address, defaulter_5, {
					from: owner,
				})
				...
				const alice_scaleSnapshotERC20 = (
					await stabilityPoolERC20.depositSnapshots(alice)
				)[2].toString()

				assert.equal(alice_scaleSnapshotERC20, "0")
				assert.equal(
					alice_ETHGainAt2ndScaleChangeERC20,
					alice_ETHGainAfterFurtherLiquidationERC20
				)
			})
			...
			it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
				...
				await borrowerOperations.openTrove(
					...
					0,
					th._100pct,
					await getOpenTroveUAmount(dec(1, 36)),
					defaulter_1,
					defaulter_1,
				...
				// Check U balances
				const aliceUBalance = await UToken.balanceOf(alice)
				const aliceExpectedUBalance = web3.utils.toBN(dec(5, 35))
				const aliceUBalDiff = aliceUBalance.sub(aliceExpectedUBalance.mul(toBN(2))).abs()

				assert.isTrue(aliceUBalDiff.lte(toBN(dec(1, 18)))) // error tolerance of 1e18

				const bobUBalance = await UToken.balanceOf(bob)
				const bobExpectedUBalance = toBN(dec(5, 35))
				const bobUBalDiff = bobUBalance.sub(bobExpectedUBalance.mul(toBN(2))).abs()
			...
			it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
				...
				const aliceUBalance = await UToken.balanceOf(alice)
				const aliceExpectedUBalance = toBN("99999999999999997500000000000000000000").mul(
					toBN(2)
				)
				...
				const bobUBalance = await UToken.balanceOf(bob)
				const bobExpectedUBalance = toBN("99999999999999997500000000000000000000").mul(toBN(2))
