...
contract(
	...
	async accounts => {
		...
		before(async () => {
			...
			youParameters = coreContracts.youParameters
		...
		it("Check if correct Addresses in Vault Parameters", async () => {
			assert.equal(priceFeed.address, await youParameters.priceFeed())
			assert.equal(activePool.address, await youParameters.activePool())
			assert.equal(defaultPool.address, await youParameters.defaultPool())
		})

		it("Sets the correct youParams address in TroveManager", async () => {
			assert.equal(youParameters.address, await troveManager.youParams())
		})
		...
		it("Sets the correct VestaParameters address in BorrowerOperations", async () => {
			assert.equal(youParameters.address, await borrowerOperations.youParams())
		})
